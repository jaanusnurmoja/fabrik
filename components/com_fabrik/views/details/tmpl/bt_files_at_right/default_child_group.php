...
?>
<div class="row-striped">CH ROW STRIPED START
<?php
$child = $this->child;
$rowStarted = false;
...
foreach ($this->groups as $group):
	if ($group->is_child):
		if ($group->parentgroup == $this->child->id):

			$this->group = $group;
?>
			<div class="fabrikChildGroup" id="group<?php echo $group->id;?>" style="<?php echo $group->css;?>">CH CH START <?php echo $group->id;?>
			<?php
			if ($group->showLegend) :?>
			...
			?>
			CH CH END <?php echo $group->id;?></div>
<?php
		endif;
	endif;
endforeach; 
?>
CH ROWSTRIPED END</div>
