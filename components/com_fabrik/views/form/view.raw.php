...
class FabrikViewForm extends FabrikViewFormBase
{
    ...
    public function display($tpl = null)
    {
        ...
        for ($i = 0; $i < count($gkeys); $i++)
        {
            ...
            if ($groupModel->canRepeat())
            {
                if ($groupModel->isJoin())
                {
                    ...
                    if (is_object($joinTable))
                    {
                        ...
                        $repeatGroup = count((array)ArrayHelper::getValue($model->data, $smallerElHTMLName, []));
                    }
                }
            }
            ...
            for ($c = 0; $c < $repeatGroup; $c++)
            {
                ...
                foreach ($elementModels as $elementModel)
                {
                    ...
                    $JSONHtml[$elementHTMLId] = htmlentities($elementModel->render($model->data, $c), ENT_QUOTES,
                        'UTF-8');
                }
            }
        }
        ...
        $data = [
            "id"   => $model->getId(), 'model' => 'table', "errors" => $model->errors, "data" => $JSONarray, 'html' => $JSONHtml,
            'post' => $_REQUEST
        ];
