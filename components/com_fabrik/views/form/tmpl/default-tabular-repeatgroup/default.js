...
var FbForm = new Class({
	...
	setUpAll: function () {
		...
		this.form.getElements('.fabrikGroup').each(function (group) {
			...
			//var c = group.getElements('.celldata').length;
			//if no joined repeating data then c should be 0 and not 1
			/*if (c) 
			{*/
				if (c === 1) 
				{
					if ((group.getElement('.fabrikSubGroupElements') && group.getElement('.fabrikSubGroupElements').getStyle('display') === 'none') || (group.getElement('.celldata') && group.getElement('.celldata').getStyle('display') == 'none')) 
					{
						c = 0;
					}
				}
			...
			{*/
				/*if (c === 1) 
				{
					if (group.getElement('.celldata') && group.getElement('.celldata').getStyle('display') === 'none') 
					{
						c = 0;
					}
				}
	...
	hideLastGroup : function (groupid, subGroup) {
		...
		var cd = subGroup.getElement('.celldata');
		if (!cd) {
		...
		if (!sge) {
		cd.setStyle('display', 'none');
		new Element('td', {'class': 'fabrikNotice'}).appendText(Joomla.JText._('COM_FABRIK_NO_REPEAT_GROUP_DATA')).inject(cd, 'after');
		}
	},
	...
	duplicateGroup : function (e) {
		...
		if (this.isFirstRepeatSubGroup(group)) {
			...
			if (subgroups[0].getElement('.fabrikSubGroupElements')) 
			{
			subgroups[0].getElement('.fabrikSubGroupElements').show();
			}
			if (subgroups[0].getElement('.celldata')) 
			{
			subgroups[0].getElement('.celldata').show();
			}
			this.repeatGroupMarkers.set(i, this.repeatGroupMarkers.get(i) + 1);
			return;
		}
