...
$group = $this->group;
?>
<tr class="fabrikSubGroupElements fabrikSubGroup template row">
    <?php
    foreach ($this->elements as $element) :
        $style = $element->hidden ? 'style="display:none"' : '';
        ?>
        <td class="<?php
        echo $element->containerClass; ?>" <?php
        echo $style ?>>
            <?php
    ...
    if (count($group->childs > 0)) :
        foreach ($group->childs as $cgroup) :
            $this->group = $cgroup;
            $this->elements = $cgroup->elements
            ?>
            <td>
                <?php
                echo $this->loadTemplate($cgroup->tmpl);
                ?>
            </td>
        <?php
        endforeach;
