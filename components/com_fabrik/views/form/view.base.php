...
class FabrikViewFormBase extends FabrikView
{
    ...
    public function display($tpl = null)
    {
        ...
        if ($model->isMambot)
        {
            ...
            $model->errors = $this->session->get($context . 'errors', []);
            $clearErrors = true;
        }
                ...
                if (isset($p->joinId) && !empty($p->joinId))
                {
                    ...
                    if ($p->canRepeat)
                    {
                        foreach ($p->subgroups as $sgKey => $sGroup)
                        {
                            ...
                            {
                                $dataGroup[$tableSgKey][$p->tableJoin . '.' . $sgElKey] = [];
                    ...
                    else
                    {
                        ...
                        {
                            $dataGroup[$tableSgKey][$tableSgKey . '.' . $elKey] = [];
    ...
    public function output()
    {
        ...
        if (
            $params->get('process-jplugins', 2) == 1
    ...
    protected function setTitle($w, &$params)
    {
        ...
        if (!$this->app->isAdmin())
        {
            ...
            {
                $menuParams = is_a($menu->params, 'Registry') || is_a($menu->params,
                    'JRegistry') ? $menu->params : new Registry($menu->params);
    ...
    protected function _addJavascript($listId)
    {
        ...
        if (!defined('_JOS_FABRIK_FORMJS_INCLUDED'))
        {
            ...
            $deps = new stdClass;
            $deps->deps = ['fab/fabrik', 'fab/element', 'fab/form-submit'];
        ...
        FabrikHelperHTML::tips('.hasTip', [], "$('$bKey')");
        $model->getFormCss();
        $opts = $this->jsOpts();
    ...
    protected function _loadTmplBottom(&$form)
    {
        ...
        $fields[] = '<input type="hidden" name="package" value="' . $this->app->getUserState('com_fabrik.package',
                'fabrik') . '" />';
        ...
        */
        ...
        $form->resetButton = $params->get('reset_button',
            0) && $this->editable == '1' ? $btnLayout->render($layoutData) : '';

        if ($copyIcon !== '')
        {
            $copyIcon = FabrikHelperHTML::icon($copyIcon);
            $copyLabel = $params->get('copy_icon_location',
                'before') == 'before' ? $copyIcon . '&nbsp;' . $copyLabel : $copyLabel . '&nbsp;' . $copyIcon;
        }
        ...
        $this->hasActions = ($input->get('print',
                '0') == '1' || $input->get('format') === 'pdf' || $noButtons) ? false : true;
        ...
        if ($model->hasErrors())
        {
            $origRowIds = $this->app->input->getRaw('fabrik_group_rowids', []);
        }
        ...
        foreach ($groups as $groupModel)
        {
            if ($groupModel->isJoin())
            {
                ...
                else
                {
                    ...
                    $groupPk .= '_raw';
                    $groupRowIds = (array)FArrayHelper::getValue($this->data, $groupPk, []);
