...
class FabrikViewListBase extends FabrikView
{
     ...
     * @return  void
     */
    protected function getManagementJS($data = [])
    {
        ...
        FabrikHelperHTML::jLayoutJs('modal-state-label', 'list.fabrik-filters-modal-state-label',
            $layoutData = (object)[
                'label'        => '',
                'displayValue' => '',
                'key'          => ''
        ...
        $opts->links = [
            'detail'       => $params->get('detailurl', ''), 'edit' => $params->get('editurl',
                ''), 'add' => $params->get('addurl', '')
        ];
        ...
        $opts->fabrik_show_in_list = $input->get('fabrik_show_in_list', [], 'array');
        ...
        $opts->groupByOpts->startCollapsed = (bool)$input->get('start_collapsed',
    ...
    public function display($tpl = null)
    {
        ...
        $this->table->intro = $params->get('show_into',
            1) == 0 ? '' : FabrikString::translate($w->parseMessageForPlaceHolder($item->introduction));
        $this->table->outro = $params->get('show_outro',
            1) == 0 ? '' : FabrikString::translate($w->parseMessageForPlaceHolder($params->get('outro')));
        ...
        $this->showTitle = FabrikWorker::getMenuOrRequestVar('show-title', $params->get('show-title', 1),
            $this->isMambot, 'request');
        ...
        $this->toggleFilters = in_array($this->filterMode, [2, 4, 5]);
        ...
        $this->tablePicker = $params->get('show-table-picker', $input->get('list-picker',
            true)) && $this->app->isAdmin() && $this->app->input->get('format') !== 'pdf' ? FabrikHelperHTML::tableList($this->table->id) : '';
    ...
    */
    ...
    protected function buttons()
    {
        ...
        $buttonProperties = [
            'class' => 'fabrikTip', 'opts' => '{"notice":true}',
    ...
    protected function loadTemplateBottom()
    {
        ...
        $this->hiddenFields[] = '<input type="hidden" name="option" value="' . $input->get('option',
                'com_fabrik') . '" />';
