...
echo $this->action ?>" class="advancedSearch_<?php
echo $this->listref ?>">
    <a class="addbutton advanced-search-add btn-success btn" href="#">
        <?php
        echo FabrikHelperHTML::image('plus', 'list', $this->tmpl); ?>
        <?php
    ...
    <div id="advancedSearchContainer">
        <table class="advanced-search-list table table-striped table-condensed">
            ...
            foreach ($this->rows as $row) : ?>
                <tr>
                    <td><span><?php
                            echo $row['join']; ?></span></td>
                    <td><?php
                        echo $row['element'] . $row['type'] . $row['grouped']; ?>
                    </td>
                    <td><?php
                        echo $row['condition']; ?></td>
                        ...
                        if (FabrikWorker::j3()) : ?>
                            <div class="button-group">
                                <a class="advanced-search-remove-row btn btn-danger" href="#">
                                    <?php
                                    echo FabrikHelperHTML::image('minus', 'list', $this->tmpl); ?>
                                </a>
                            </div>
                        <?php
                        else: ?>
            ...
            <tr class="fabrik___heading title">
                <th></th>
                <th><?php
                    echo FText::_('COM_FABRIK_ELEMENT') ?></th>
                <th><?php
                    echo FText::_('COM_FABRIK_CONDITION') ?></th>
                <th><?php
                    echo FText::_('COM_FABRIK_VALUE') ?></th>
                <th><?php
                    echo FText::_('COM_FABRIK_DELETE') ?></th>
            </tr>
            </thead>
        </table>
    </div>
