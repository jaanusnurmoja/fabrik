...
class FabrikFEModelList extends JModelForm
{
     ...
     * @var array
     */
    protected $elementsWithRequiredFilters = [];
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_fabrik.list', 'view', ['control' => 'jform', 'load_data' => $loadData]);
     ...
     * @return  array
     */
    public function getPluginJsClasses(&$r = [], &$shim = [])
    {
        $r = (array)$r;
        $pluginManager = FabrikWorker::getPluginManager();
        $pluginManager->getPlugInGroup('list');
    ...
    public function setLimits($limitStart_override = null, $limitlength_override = null)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $limitLength = $this->app->getUserStateFromRequest($context . 'limitlength', 'limit' . $id,
                    $rowsPerPage);

                if (!$this->randomRecords)
                {
                    $limitStart = $this->app->getUserStateFromRequest($context . 'limitstart', 'limitstart' . $id,
                        $limitStart, 'int');
                }
            }
     ...
     * @return void
     */
    public function finesseData($opts = [])
    {
        $profiler = JProfiler::getInstance('Application');
...
//echo $query;
    ...
    private function activeContextElements($groupModel)
    {
        ...
        if (
            $this->formatAll() || ($tableParams->get('group_by_template') !== '' && $this->getGroupBy() != '') || $this->outputFormat == 'csv'
            || $this->outputFormat == 'feed'
        )
        {
     ...
     * @return  void
     */
    protected function formatData(&$data, $opts = [])
    {
        $profiler = JProfiler::getInstance('Application');
                ...
                if (!empty($data) && array_key_exists($col, $data[0]))
                {
                    ...
                    else
                    {
                        ...
                        for ($i = 0; $i < $ec; $i++)
                        {
                             ...
                             */
                            if (
                                $this->app->input->get('format', '') !== 'fabrikfeed'
                                && $this->app->input->get('format', '') !== 'feed'
                            )
                            {
                                $data[$i]->$col = htmlspecialchars_decode(htmlentities($data[$i]->$col, ENT_NOQUOTES,
                                    'UTF-8'), ENT_NOQUOTES);
                            }
        ...
        if ($groupBy != '' && $this->outputFormat != 'csv')
        {
            ...
            for ($i = 0; $i < count($data); $i++)
            {
                ...
                {
                    $aGroupTitles[] = $sData;
                    $this->groupTemplates[$sData] = $w->parseMessageForPlaceHolder($groupTemplate,
                        ArrayHelper::fromObject($data[$i]));
                    $this->groupTemplatesExtra[$sData] = $w->parseMessageForPlaceHolder($groupTemplateExtra,
                        ArrayHelper::fromObject($data[$i]));
                    $groupedData[$sData] = [];
                }
     ...
     * @return  void
     */
    protected function addSelectBoxAndLinks(&$data, $opts = [])
    {
        $profiler = JProfiler::getInstance('Application');
        ...
        foreach ($data as $groupKey => $group)
        {
            ...
            for ($i = 0; $i < $cg; $i++)
            {
                ...
                foreach ($joins as $join)
                {
                    if ($join->list_id !== '0')
                    {
                        ...
                        if (isset($row->$fKey))
                        {
                            ...
                            $pkCheck[] = '<input type="checkbox" class="fabrik_joinedkey" value="' . htmlspecialchars($fKeyVal,
                                    ENT_COMPAT, 'UTF-8')
                ...
                {
                    $img = FabrikHelperHTML::image('edit.png', 'list', '', ['alt' => $editLabel]);
                ...
                {
                    $img = FabrikHelperHTML::image('search.png', 'list', '', ['alt' => $viewLabel]);
    ...
    protected function deleteButton($tpl = '', $heading = false)
    {
        ...
        else
        {
            $btn = '<a href="#" class="' . $displayData->btnClass . 'delete" data-listRef="list_' . $displayData->renderContext
                ...
                . FabrikHelperHTML::image('delete.png', 'list', $displayData->tpl,
                    ['alt' => $displayData->label, 'icon-class' => $displayData->iconClass]) . ' ' . $displayData->text . '</a>';
     ...
     */
    public function getRecordCounts(&$element, $pks = [])
    {
    ...
    public function viewDataLink($popUp = false, $element = null, $row = null, $key = '', $val = '', $count = 0, $f = null)
    {
		...
		*/
        ...
        if ($facetTable->canView())
        {
            ...
            $displayData->addLink = $linkedForm == '0' ? $this->viewFormLink($popUp, $element, $row, $key, $val, false,
                $f) : '';
    ...
    public function _addLink($data, &$elementModel, $row, $repeatCounter = 0)
    {
        ...
        $paths = [
            COM_FABRIK_FRONTEND . '/views/list/tmpl/' . $this->getTmpl() . '/layouts/element/' . $elementModel->getFullName(true,
                false)
        ];
    ...
    public function buildQuery()
    {
        ...
        if ($this->mergeJoinedData())
        {
			...
			*/
            ...
            $lookupC = 0;
            $lookUps = ['DISTINCT ' . $table->db_primary_key . ' AS __pk_val' . $lookupC];
            $lookUpNames = [$table->db_primary_key];
            ...
            $by = trim($table->order_by) === '' ? [] : (array)json_decode($table->order_by);
    ...
    private function selectSlug(&$fields)
    {
        ...
        if (in_array($this->outputFormat, ['raw', 'html', 'partial', 'feed', 'pdf', 'phocapdf']))
        {
    ...
    public function buildQuerySelect($mode = 'list', $query = false)
    {
        ...
        if (
            trim($table->db_primary_key) != '' && (in_array($this->outputFormat,
                ['partial', 'raw', 'html', 'feed', 'pdf', 'phocapdf', 'csv', 'word', 'yql', 'oai']))
        )
        {
            $sFields .= ', ';
    ...
    public function buildQueryOrder($query = false)
    {
		...
		*/
        ...
        if ($groupOrderBy != '')
        {
            ...
            $strOrder == '' ? $strOrder = "\n ORDER BY " : $strOrder .= ',';
            $orderBy = strstr($groupOrderBy,
                '_raw`') ? FabrikString::safeColNameToArrayKey($groupOrderBy) : FabrikString::safeColName($groupOrderBy);
        ...
        foreach ($joins as $join)
        {
			...
			*/
            ...
            else
            {
				...
				*/
                $statements[$join->id] = [$join->keytable, $sql];
            }
    ...
    public function buildQueryPrefilterWhere($element)
    {
        ...
        $valueKeys = array_keys(FArrayHelper::getValue($filters, 'value', []));
    ...
    public function buildQueryGroupBy($query = false)
    {
        ...
        $listGroupBy = !empty($groupByQueryList) ? explode(',', $groupByQueryList) : [];
    ...
    public function buildQueryWhere($incFilters = true, $query = false, $doJoins = true)
    {
		...
		*/
        ...
        $this->_whereSQL[$sig] = ['0' => $sqlNoFilter, '1' => $sql];
    ...
    private function groupFilterSQL(&$filters, $type = '*')
    {
        ...
        $nullElementConditions = ['IS NULL', 'IS NOT NULL'];
        ...
        foreach ($valueKeys as $vkey => $i)
        {
            ...
            if (
                FArrayHelper::getValue($filters['sqlCond'], $i, '') == '' && !in_array($condition,
            ...
            if (in_array($searchType, ['prefilter', 'menuPrefilter']) && $type == '*')
            {
                continue;
            }
...
*/
    ...
    protected function getJoinsNoCdd()
    {
        if (!isset($this->_joinsNoCdd))
        {
            $form = $this->getFormModel();
            $form->getGroupsHiarachy();
            $ignore = ['PlgFabrik_ElementCascadingdropdown'];
    ...
    public function &getJoins()
    {
        if (!isset($this->joins))
        {
            ...
            $ids = $form->getElementIds([], ['includePublised' => false, 'loadPrefilters' => true]);
            $db = FabrikWorker::getDbo(true);
            $id = (int)$this->getId();
            $query = $db->getQuery(true);
    ...
    public function shouldUpdateElement(&$elementModel, $origColName = null)
    {
		...
		*/
        ...
        $objTypeUpper = str_replace([' INTEGER', ' TINYINT', ' SMALLINT', ' MEDIUMINT', ' BIGINT'], ' INT',
            $objTypeUpper);
        ...
        $existingDef = str_replace([' INTEGER', ' TINYINT', ' SMALLINT', ' MEDIUMINT', ' BIGINT'], ' INT',
            $existingDef);
        ...
        elseif ($this->canAlterFields() === false)
        {
            ...
            $this->app->enqueueMessage(FText::_('COM_FABRIK_NOTICE_ELEMENT_SAVED_BUT_STRUCTUAL_CHANGES_NOT_APPLIED'),
                'notice');
        ...
        if (
            empty($origColName) || !in_array($origColName,
                $existingFields) || ($this->app->input->get('task') === 'save2copy' && $this->canAddFields())
        )
        {
            if (!$altered)
            {
    ...
    public function storeRequestData($request)
    {
         ...
         */
        if (
            !$this->app->input->get('fabrik_incsessionfilters', true)
        ...
        if ($input->get('option') == $option && is_object($registry))
        {
            ...
            if (isset($reg[$option]) && !in_array($input->get('view'), ['form', 'details']))
            {
    ...
    public function &getFilterArray()
    {
		...
		*/
        ...
        $indexStep = count(FArrayHelper::getValue($this->filters, 'key', []));
        ...
        $readOnlyValues = [];
        $w = new FabrikWorker;
        $noFiltersSetup = FArrayHelper::getValue($this->filters, 'no-filter-setup', []);
        ...
        $elementIds = FArrayHelper::getValue($this->filters, 'elementid', []);
			...
			*/
            ...
            if ($elid == -1)
            {
                ...
                if (count($fieldsByTable) > 1)
                {
                    ...
                    {
                        $matchSql[] = 'MATCH(' . implode(',',
                                $fields) . ')' . ' ' . $condition . ' (' . $db->q($value) . ' IN BOOLEAN MODE)';
                    }
            ...
            if ($filterEval == '1')
            {
                ...
                $value = @eval($value);
                FabrikWorker::logEval($value,
                    'Caught exception on eval of tableModel::getFilterArray() ' . $key . ': %s');
                $i = $origi;
            }
            ...
            if (!array_key_exists($i, $sqlCond) || $sqlCond[$i] == '')
            {
                ...
                {
                    $query = $elementModel->getFilterQuery($key, $condition, $value, $originalValue,
                        $this->filters['search_type'][$i], $filterEval);
                }
            ...
            if ($this->filters['no-filter-setup'][$i] == 1)
            {
                ...
                $tmpData = [$tmpName => $originalValue, $tmpName . '_raw' => $originalValue];
    ...
    public function prefilterSetting()
    {
		...
		*/
        ...
        if ($properties !== '')
        {
            ...
            if (!empty($conditions))
            {
                $fields = FArrayHelper::getValue($prefilters, 'filter-fields', []);
                $conditions = FArrayHelper::getValue($prefilters, 'filter-conditions', []);
                $values = FArrayHelper::getValue($prefilters, 'filter-value', []);
                $access = FArrayHelper::getValue($prefilters, 'filter-access', []);
        ...
        return [
            $listFields, $listConditions, $listValue, $listAccess,
        ...
        if (!isset($this->prefilters))
        {
            ...
            for ($i = 0; $i < count($filterFields); $i++)
            {
                ...
                if ($elementModel && $elementModel->getElement()->published == 0)
                {
                    ...
                    // Add the logger.
                    JLog::addLogger(['text_file' => 'fabrik.log.php']);

                    // Start logging...
                    $msg = JText::sprintf('COM_FABRIK_ERR_PREFILTER_NOT_APPLIED',
            ...
            else
            {
                ...
                $query
                    ->where(
                        ...
                        . "AND el.params like '\"%cascadingdropdown_connection\":\"" . $table->connection_id . "\"%') ",
                        "OR");
    ...
    protected function &makeFilters($container = 'listform_1', $type = 'list', $id = '', $ref = '')
    {
        ...
        )
        {
            ...
            if ($params->get('search-mode-advanced') == 1)
            {
                ...
                $displayData->searchOpts[] = JHTML::_('select.option', 'all',
                    FText::_('COM_FABRIK_ALL_OF_THESE_TERMS'));
                $displayData->searchOpts[] = JHTML::_('select.option', 'any',
                    FText::_('COM_FABRIK_ANY_OF_THESE_TERMS'));
                $displayData->searchOpts[] = JHTML::_('select.option', 'exact', FText::_('COM_FABRIK_EXACT_TERMS'));
                $displayData->searchOpts[] = JHTML::_('select.option', 'none',
                    FText::_('COM_FABRIK_NONE_OF_THESE_TERMS'));
		...
		*/
        ...
        $keys = FArrayHelper::getValue($filters, 'key', []);

        foreach ($keys as $i => $key)
        {
            if (
                $filters['no-filter-setup'][$i] == '1' && !in_array($filters['search_type'][$i],
                    ['searchall', 'advanced', 'jpluginfilters'])
            )
            {
                $o = new stdClass;
    ...
    public function getHeadings()
    {
        ...
        if (!in_array($this->outputFormat, ['pdf', 'csv']))
        {
        ...
        foreach ($groups as $groupModel)
        {
            $groupHeadingKey = $w->parseMessageForPlaceHolder($groupModel->getGroup()->label, [], false);
            ...
            foreach ($elementModels as $key => $elementModel)
            {
                ...
                $label = $w->parseMessageForPlaceHolder($label, []);
                ...
                $responsiveClass = $responsiveKey !== false ? FArrayHelper::getValue($listClasses->responsive_class,
                    $responsiveKey, '') : '';
                ...
                $headingClass[$compositeKey] = [
                    'class' => $responsiveClass . $elementModel->getHeadingClass(),
                    'style' => $elementParams->get('tablecss_header')
                ];
                $cellClass[$compositeKey] = ['class' => $responsiveClass . $elementModel->getCellClass(), 'style' => $elementParams->get('tablecss_cell')];
        ...
        if (!in_array($this->outputFormat, ['pdf', 'csv']))
        {
            ...
            if (is_null($listOrder))
            {
                ...
                $listOrder = is_object($faceted) && is_object($faceted->linkedlist) ? array_keys(ArrayHelper::fromObject($faceted->linkedlist)) : [];
            }

            if (is_null($formOrder))
            {
                // Not yet saved with order
                $formOrder = is_object($faceted) && is_object($faceted->linkedform) ? array_keys(ArrayHelper::fromObject($faceted->linkedform)) : [];
            }
            ...
            foreach ($listOrder as $key)
            {
                ...
                if (is_object($join) && isset($faceted->linkedlist->$key))
                {
                    ...
                    if ($linkedTable != '0' && $facetTable->canView())
                    {
                        ...
                        $headingClass[$prefix] = [
                            'class' => 'fabrik_ordercell related ' . $prefix,
                            'style' => ''
                        ];
                        $cellClass[$prefix] = ['class' => $prefix . ' fabrik_element related'];
                    }
                }
            }
        ...
        return [$aTableHeadings, $groupHeadings, $headingClass, $cellClass];
    }
    ...
    protected function actionHeading(&$aTableHeadings, &$headingClass, &$cellClass)
    {
        ...
        $pluginManager = FabrikWorker::getPluginManager();
        $pluginManager->runPlugins('button', $this, 'list', ['heading' => true]);
        ...
        if (!empty($pluginHeadings) || $this->canSelectRows() || $this->canEditARow() || $details || $edit || $filtersUnderHeadingsAndGo)
        {
            ...
            $headingClass['fabrik_actions'] = ['class' => 'fabrik_ordercell fabrik_actions', 'style' => ''];

            // Needed for ajax filter/nav
            $cellClass['fabrik_actions'] = ['class' => 'fabrik_actions fabrik_element'];
        }
    }
    ...
    protected function addCheckBox(&$aTableHeadings, &$headingClass, &$cellClass, $hide = false)
    {
        ...
        $headingClass['fabrik_select'] = ['class' => 'fabrik_ordercell fabrik_select', 'style' => $hidestyle];
        // Needed for ajax filter/nav
        $cellClass['fabrik_select'] = ['class' => 'fabrik_select fabrik_element', 'style' => $hidestyle];
    }
                    ...
                    foreach ($elementModels as $elementModel)
                    {
                        ...
                        if ($elementModel->recordInDatabase($data))
                        {
                            if (array_key_exists($key, $data) && !in_array($key, $noRepeatFields))
                            {
                                ...
                                if (
                                    isset($formModel->formData) && array_key_exists('fabrik_copy_from_table',
                                        $formModel->formData)
                                )
                                {
    ...
    protected function addDefaultDataFromRO(&$data, &$oRecord, $isJoin, $rowId, $joinGroupTable)
    {
        ...
        if (is_null($this->origData))
        {
			...
			*/
            ...
            else
            {
                ...
                $origData = is_array($origData) ? $origData : [];
                $this->origData = $origData;
            }
        }
        else
		...
		*/
        $repeatGroupCounts = $input->get('fabrik_repeat_group', [], 'array');
    ...
    public function doCalculations()
    {
		...
		*/
        $cache = FabrikWorker::getCache($this);
        $cache->call([get_class($this), 'cacheDoCalculations'], $this->getId());
    }
    ...
    public function getPrimaryKeyAndExtra($table = null)
    {
        ...
        if (is_array($origColNames))
        {
            foreach ($origColNames as $origColName)
            {
                ...
                if ($key == "PRI")
                {
                    $keys[] = ["key" => $key, "extra" => $extra, "type" => $type, "colname" => $colName];
                }
                else
                {
        ...
        if (empty($keys))
        {
            ...
            if (!empty($shortColName) && array_key_exists($shortColName, $origColNamesByName))
            {
                ...
                $extra = $origColName->Extra;
                $type = $origColName->Type;
                $keys[] = ['colname' => $shortColName, 'type' => $type, 'extra' => $extra, 'key' => $key];
            }
        }
     ...
     * @return  bool
     */
    public function fieldExists($field, $ignore = [], $elGroupModel)
    {
                                      ...
                                      $className = "inputbox")
    {
        ...
        $fieldDropDown = JHTML::_('select.genericlist', $fieldNames, $selectListName, $opts, 'value', 'text',
            $selected);
    ...
    public function getRSSFeedLink()
    {
        ...
        {
            $base = JURI::getInstance()->toString(['scheme', 'user', 'pass', 'host', 'port', 'path']);
    ...
    public function parseMessageForRowHolder($msg, &$row, $addSlashes = false)
    {
		...
		*/
        $msg = preg_replace_callback("/{[^}\s]+}/i", [$this, 'replaceWithRowData'], $msg);
     ...
     *                                     where - additional where filter to apply to query (@return  array  Values for the column - empty array if no results found
     * @since 3.0.8)
     *
     */
    public function getColumnData($col, $distinct = true, $opts = [])
    {
        if (!array_key_exists($col, $this->columnData))
        {
            ...
            $res = $cache->call([get_class($this), 'columnData'], $this->getId(), $col, $distinct, $opts);
     ...
     * @param bool $distinct Select distinct values only
     * @param array $opts Options: filterLimit bool - should limit to filter_list_max global param (default true)
     *                                     where - additional where filter to apply to query (@return  array  Column's values
     * @since   3.0.7
     *
     * @since 3.0.8)
     *
        ...
        if (!array_key_exists($sig, $this->elements))
        {
            ...
            foreach (array_keys($groups) as $gid)
            {
                ...
                foreach ($elementModels as $elementModel)
                {
                    ...
                    $dbKey = $key == 'filtername' ? trim($elementModel->getFilterFullName()) : trim($elementModel->getFullName(true,
                        false));
    ...
    public function viewLabel($row)
    {
        ...
        return FText::_($this->parseMessageForRowHolder($params->get('detaillabel', FText::_('COM_FABRIK_VIEW')),
            $row));
    }
    ...
    public function getAddRecordLink()
    {
        ...
        $keys = FArrayHelper::getValue($filters, 'key', []);
        $vals = FArrayHelper::getValue($filters, 'value', []);
        $types = FArrayHelper::getValue($filters, 'search_type', []);
        ...
        for ($i = 0; $i < count($keys); $i++)
        {
            ...
            if (in_array($filterType, ['jpluginfilters', 'querystring']))
            {
    ...
    public function setPluginQueryWhere($pluginName, $whereClause)
    {
		...
		*/
        if (
            !array_key_exists($pluginName,
                $this->pluginQueryWhere) || $whereClause != $this->pluginQueryWhere[$pluginName]
        )
        {
                    ...
                    if ($elementModel->getGroup()->canRepeat() || $elementModel->getGroup()->isJoin())
                    {
                        ...
                        if (!isset($canRepeatsTables[$join_table_name]))
                        {
                            ...
                            else
                            {
                                ...
                                if (!empty($joinPk))
                                {
                                    $canRepeatsTables[$join_table_name] = ['colname' => FabrikString::shortColName($joinPk)];
                                }
                            }
                        }
    ...
    protected function setElementTmpl()
    {
        ...
        {
            if (
                ($params->get('group_by_template',
                        '') !== '' && $this->getGroupBy() != '') || $this->outputFormat == 'csv'
                || $this->outputFormat == 'feed'
            )
            {
                $elementModels = $groupModel->getPublishedElements();
    ...
    public function setRenderContext($id = null)
    {
        ...
        else
        {
            ...
            if (
                (($task == 'list.view' || $task == 'list.delete' || $task === 'view' || $task == 'list.doPlugin') && $input->get('format') == 'raw')
    ...
    public function getGroupByHeadings()
    {
        ...
        $base = $base->toString(['scheme', 'user', 'pass', 'host', 'port', 'path']);
        ...
        foreach ($h as $key => $v)
        {
            if (!in_array($key, ['fabrik_select', 'fabrik_edit', 'fabrik_view', 'fabrik_delete', 'fabrik_actions']))
            {
                /**
    ...
    public function getShowFilters()
    {
        ...
        return (count($filters) > 0 && $filterMode !== 0) && $this->app->input->get('showfilters',
            1) == 1 ? true : false;
    }
    ...
    public function getHasButtons()
    {
        ...
        if (
            ($this->canAdd() && $params->get('show-table-add')) || $this->getShowFilters()
    ...
    private function getTabCategories()
    {
        ...
        {
            $this->app->enqueueMessage(sprintf(FText::_('COM_FABRIK_LIST_TABS_TABLE_ERROR'), $tableName,
                $table->db_table_name), 'error');
        ...
        $isDropdown = (is_subclass_of($elementModel,
                'PlgFabrik_ElementDropdown') || get_class($elementModel) == 'PlgFabrik_ElementDropdown');
        $is_join = (is_subclass_of($elementModel,
                'PlgFabrik_ElementDatabasejoin') || get_class($elementModel) == 'PlgFabrik_ElementDatabasejoin');
        if (!$is_join)
        {
            // Get values and count in the tab field
            $db = $this->getDb();
            $query = $db->getQuery(true);
            $query->select([$tabsField, 'Count(' . $tabsField . ') as count'])
        ...
        {
            $this->app->enqueueMessage(sprintf(FText::_('COM_FABRIK_LIST_TABS_MERGE_ERROR'), count($counts), $tabsMax),
                'notice');
        ...
        if ($tabsAll)
        {
            ...
            $tabs[] = [FText::_('COM_FABRIK_LIST_TABS_ALL'), null];
        }
        ...
        while (count($counts) > $tabsMax)
        {
            ...
            $counts[$minIndex - 1][0] = [$counts[$minIndex - 1][0][0], end($counts[$minIndex][0])];
        ...
        for ($i = 0; $i < count($counts); $i++)
        {
            ...
            {
                $tabs[] = [$counts[$i][0][0] . '-' . $counts[$i][0][1], $counts[$i][0]];
            }
            else
            {
                $tabLabel = empty($counts[$i][0]) ? '-' : $counts[$i][0];
                $tabs[] = [$tabLabel, $counts[$i][0]];
            }
        }
    ...
    public function loadTabs()
    {
        ...
        $ActiveTabLimitStart = FArrayHelper::getValue($inputArray, 'limitstart' . $listId,
            $uri->getVar('limitstart' . $listId, null));
        ...
        if (empty($cachedTabs))
        {
            ...
            foreach ($tabs as $i => $tabArray)
            {
                ...
                {
                    list($low, $high) = $range;
                    $row->mergeArray = ['value' => [urlencode($low), urlencode($high)], 'condition' => 'BETWEEN'];
    ...
    {
        $acl_types = ['allow_edit_details2', 'allow_delete2'];
        $params = $this->getParams();
    ...
    public function toggleCols()
    {
        ...
        foreach ($groups as $groupModel)
        {
            ...
            {
                $group = $groupModel->getGroup();
                $groupLabel = $w->parseMessageForPlaceHolder($group->label, [], false);
                $cols[$group->id]['name'] = $groupLabel;
                $cols[$group->id]['elements'] = [];
     ...
     * @return LayoutFile
     */
    public function getLayout($name, $paths = [], $options = [])
    {
