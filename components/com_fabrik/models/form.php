...
class FabrikFEModelForm extends FabModelForm
{
     ...
     * @var array
     */
    protected $pluginUpdatedElements = [];
    ...
    public function getJsActions()
    {
        ...
        foreach ($groups as $groupModel)
        {
            ...
            foreach ($elementModels as $elementModel)
            {
				 ...
				 * element on the form.
				 */
                $aJsActions[$elementModel->getElement()->id] = [];
        ...
        if ($form->record_in_database)
        {
            ...
            if (is_object($listModel) && $listId !== 0)
            {
                ...
                $query->select('g.id, j.id AS joinid')->from('#__{package}_joins AS j')
                    ->join('INNER',
                        '#__{package}_groups AS g ON g.id = j.group_id')->where('list_id = ' . $listId . ' AND g.published = 1');
    ...
    public function &setFormData()
    {
        ...
        if (!array_key_exists('__pk_val', $data))
        {
            ...
            $data['__pk_val'] = FArrayHelper::getValue($data, $primaryKey . '_raw',
                FArrayHelper::getValue($data, $primaryKey, ''));
        }
        ...
        array_walk_recursive($data, [$this, '_clean']);
    ...
    protected function _clean(&$item)
    {
        if (is_array($item))
        {
            array_walk_recursive($item, [$this, '_clean']);
        }
        else
        {
            if ($this->dofilter)
    ...
    {
        $input = $this->app->input;
        $repeatTotals = $input->get('fabrik_repeat_group', [0], 'array');
        $groups = $this->getGroupsHiarachy();
    ...
    public function validate()
    {
        ...
        foreach ($groups as $groupModel)
        {
            ...
            foreach ($elementModels as $elementModel)
            {
                ...
                $elName = $input->getBool('fabrik_ajax') ? $elementModel->getHTMLId(0) : $elementModel->getFullName(true,
                    false);
                $this->errors[$elName] = [];
                $elName2 = $elementModel->getFullName(true, false);
    ...
    public function getErrors()
    {
        ...
        if (empty($this->errors))
        {
            if ($this->isMambot)
            {
                $errors = $this->session->get($this->getSessionContext() . 'errors', []);
            }
        }
        else
        {
    ...
    {
        $data = ['modified' => $this->modifiedValidationData, 'errors' => $this->errors];
     ...
     *                          Property 'loadPrefilters' @return  array  int ids
     * @since 3.0.7.1 - used to ensure that pre-filter elements are loaded in inline edit
     *
     */
    public function getElementIds($ignore = [], $opts = [])
    {
        $aEls = [];
        $groups = $this->getGroupsHiarachy();
    ...
    public function getElementOptions($useStep = false, $key = 'name', $show_in_list_summary = false, $incRaw = false,
                                      $filter = [], $labelMethod = '', $noJoins = false, $translate = true)
    {
        $groups = $this->getGroupsHiarachy();
        $aEls = [];
    ...
    public function hasErrors()
    {
        ...
        $allErrors = $this->isMambot ? $this->session->get($this->getSessionContext() . 'errors', []) : $this->errors;
            ...
            else
            {
                ...
                if (!$sessionLoaded)
                {
                    ...
                    if (!empty($useKey) || $this->rowId !== '')
                    {
                         ...
                         */
                        $opts = $input->get('task') == 'form.inlineedit' ? ['ignoreOrder' => true] : [];
                        ...
                        if (!empty($rows))
                        {
                            ...
                            foreach ($rows as &$row)
                            {
                                ...
                                $row = empty($row) ? [] : ArrayHelper::fromObject($row);
                        ...
                        FabrikHelperHTML::debug($data,
                            'form:getData from querying rowid= ' . $this->rowId . ' (form not in Mambot and no errors)');
                        ...
                        if (empty($rows) && $this->rowId != '')
                        {
                            ...
                            else
                            {
                                ...
                                if (
                                    empty($useKey) && !$this->isMambot && in_array($input->get('view'),
                                        ['form', 'details'])
                                )
                                {
                                    $this->rowId = '';
     ...
     *
     */
    public function _buildQuery($opts = [])
    {
     ...
     * @return  string  query
     */
    public function buildQuery($opts = [])
    {
        if (isset($this->query))
        {
            return $this->query;
    ...
    public function getFormPluginHTML()
    {
        ...
        return [$pluginTop, $pluginBottom, $pluginEnd];
    }
    ...
    public function parseIntroOutroPlaceHolders($text)
    {
        ...
        if (!$this->isEditable())
        {
            ...
            $text = preg_replace($remove, '', $text);
            $match = "/{details:\s*.*?}/is";
            $text = preg_replace_callback($match, [$this, '_getIntroOutro'], $text);
        }
        else
        {
    ...
    public function getRelatedTables()
    {
        ...
        foreach ($joinsToThisKey as $joinKey => $element)
        {
            ...
            if (isset($linkedLists->$key) && $linkedLists->$key != 0)
            {
                ...
                $links[$element->list_id][] = $label . ': ' . $referringTable->viewDataLink($popUpLink, $element, null,
                        $linkKey, $val, $count, $f);
            }
            ...
            if ($element !== false)
            {
                ...
                if ($linkedForm !== '0')
                {
                    if (is_object($element))
                    {
                        ...
                        $links[$element->list_id][] = $referringTable->viewFormLink($popUpLink, $element, null,
                                $linkKey, $val, false, $f) . $label;
                    }
                }
    ...
    public function getGroupView($tmpl = '')
    {
        ...
        foreach ($groups as $gkey => $groupModel)
        {
			...
			*/
            ...
            {
                if (
                    ($this->isEditable() && $groupModel->labelPosition('form') !== 1)
            ...
            if (count($aElements) != 0 && $groupModel->canView() !== false)
            {
                ...
                if ($showGroup !== 0)
                {
                    ...
                    if (
                        !($showGroup == 2 && $this->isEditable()) && !($showGroup == 3 && $input->get('view',
                                'form') == 'details')
    ...
    public function getRedirectURL($incSession = true, $isMambot = false)
    {
        ...
        if ($this->app->isAdmin())
        {
            ...
            {
                $url = 'index.php?option=com_fabrik&task=form.view&formid=' . $input->getInt('formid') . '&rowid=' . $input->getString('rowid',
                        '', 'string');
            }
            else
            {
        ...
        else
        {
            if (array_key_exists('apply', $this->formData))
            {
                $url = 'index.php?option=com_' . $this->package . '&view=form&formid=' . $input->getInt('formid') . '&rowid=' . $input->getString('rowid',
                        '', 'string')
            ...
            else
            {
                if ($isMambot)
                {
                    ...
                    $url = filter_var(ArrayHelper::getValue($_SERVER, 'HTTP_REFERER', 'index.php'),
                        FILTER_SANITIZE_URL);
                }
                else
                {
        ...
        if (!$incSession)
        {
            return ['url' => $url, 'baseRedirect' => $baseRedirect];
        }
        ...
        $sUrl = $this->session->get($context . 'url', []);
    ...
    public function aclMessage($force = false)
    {
        ...
        $input = $this->app->input;
        $msg = $input->get('rowid', '',
            'string') == 0 ? 'COM_FABRIK_NOTICE_CANT_ADD_RECORDS' : 'COM_FABRIK_NOTICE_CANT_EDIT_RECORDS';
    ...
    public function applyMsgOnce()
    {
        ...
        $context = $this->getRedirectContext();
        $msg = $this->session->get($context . 'msg', []);
    ...
    public function getRedirectMessage()
    {
        ...
        $context = $this->getRedirectContext();
        $sMsg = $this->session->get($context . 'msg', [$msg]);
        ...
        $showMsg = (array)$this->session->get($context . 'showsystemmsg', [true]);
