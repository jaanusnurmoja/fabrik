...
class FabrikFEModelGroup extends FabModel
{
    ...
    public function canEdit()
    {
        ...
        if (in_array($params->get('repeat_group_show_first', '1'), ['2', '5']))
        {
            $this->canEdit = false;
    ...
    public function getPublishedElements()
    {
        ...
        $ids = (array)$this->app->input->get('elementid', [], 'array');
        $sig = implode('.', $ids);
        if ($sig === '')
        {
            $sig = 'default';
    ...
    public function setColumnCss(&$element, $rowIx)
    {
        ...
        $element->span = $element->hidden ? '' : FArrayHelper::getValue($spans, $spanKey,
            FabrikHelperHTML::getGridSpan((int)floor(12 / $colCount)));
        ...
        if (!array_key_exists($sig, $this->listQueryElements))
        {
            ...
            foreach ($elements as $elementModel)
            {
                ...
                if ($element->published == 1)
                {
                    ...
                    if (
                        $input->get('view') == 'list' && ((!$this->getListModel()->isUserDoElement($full_name) && !$elementModel->canView('list')) || $groupParams->get('list_view_and_query',
                                1) == 0)
                    )
                    {
                        continue;
                    ...
                    $showThisInList = $element->primary_key || $params->get('include_in_list_query', 1) == 1
                        || (empty($showInList) && $element->show_in_list_summary) || in_array($element->id,
                            $showInList);
    ...
    public function getPublishedListElements()
    {
        ...
        {
            $this->publishedListElements[$sig] = [];
            $elements = $this->getMyElements();
            ...
            foreach ($elements as $elementModel)
            {
                ...
                if (
                    $params->get('list_view_and_query',
                        1) == 1 && $element->published == 1 && $elementModel->canView('list')
                )
                {
                    if (empty($showInList))
                    {
    ...
    public function getGroupProperties(&$formModel)
    {
        if (!isset($this->groupProperties))
        {
            ...
            if (
                ...
                && !in_array($groupTable->id,
                    $pages[$startPage]) || $showGroup == -1 || $showGroup == 0 || ($view == 'form' && $showGroup == -2) || ($view == 'details' && $showGroup == -3)
            )
            {
                $groupTable->css .= ";display:none;";
            }

            $group->css = trim(str_replace(["<br />", "<br>"], "", $groupTable->css));
            ...
            $group->intro = $formModel->parseIntroOutroPlaceHolders($intro, $group->editable,
                $formModel->isNewRecord());
            $outro = FText::_($params->get('outro'));
            $group->outro = $formModel->parseIntroOutroPlaceHolders($outro, $group->editable,
                $formModel->isNewRecord());
            $group->columns = $params->get('group_columns', 1);
            $group->splitPage = $this->isSplitPage();
            $group->showLegend = $this->showLegend($group);
            $group->start = in_array($params->get('repeat_table_part', 0), [0, 1]);
            $group->end = in_array($params->get('repeat_table_part', 0), [0, 3]);
    ...
    {
        $input = $this->app->input;
        $repeatTotals = $input->get('fabrik_repeat_group', [0], 'post', 'array');
     ...
     * @return  bool
     */
    private function deleteRepeatGroups($usedKeys = [])
    {
        if (!$this->canRepeat())
        {
            /*
    ...
    public function getOrigGroupRowsIds()
    {
        $groupId = $this->getId();
        $formModel = $this->getFormModel();
        $origGroupRowsIds = FArrayHelper::getValue($formModel->formData, 'fabrik_group_rowids', []);
