...
class getid3_shorten extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        {
            $info['error'][] = 'Expecting "' . getid3_lib::PrintHexBytes($magic) . '" at offset ' . $info['avdataoffset'] . ', found "' . getid3_lib::PrintHexBytes(substr($ShortenHeader,
                    0, 4)) . '"';
            return false;
        }
        $info['fileformat'] = 'shn';
        ...
        if (!empty($output) && (substr($output, 12, 4) == 'fmt '))
        {
            ...
            if (substr($output, 20 + $fmt_size, 4) == 'data')
            {
                ...
                $info['playtime_seconds'] = getid3_lib::LittleEndian2Int(substr($output, 20 + 4 + $fmt_size,
                        4)) / $DecodedWAVFORMATEX['raw']['nAvgBytesPerSec'];
