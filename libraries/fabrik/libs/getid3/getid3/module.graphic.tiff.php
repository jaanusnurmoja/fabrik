    ...
    public function Analyze()
    {
        ...
        switch (substr($TIFFheader, 0, 2))
        {
            ...
            case 'MM':
                ...
                break;
            default:
                $info['error'][] = 'Invalid TIFF byte order identifier (' . substr($TIFFheader, 0,
                        2) . ') at offset ' . $info['avdataoffset'];
                return false;
                break;
        }
        ...
        while ($nextIFDoffset > 0)
        {
            ...
            {
                $CurrentIFD['fields'][$i]['raw']['tag'] = $this->TIFFendian2Int($this->fread(2),
                    $info['tiff']['byte_order']);
                $CurrentIFD['fields'][$i]['raw']['type'] = $this->TIFFendian2Int($this->fread(2),
                    $info['tiff']['byte_order']);
                $CurrentIFD['fields'][$i]['raw']['length'] = $this->TIFFendian2Int($this->fread(4),
                    $info['tiff']['byte_order']);
                ...
                switch ($CurrentIFD['fields'][$i]['raw']['type'])
                {
                    case 1: // BYTE  An 8-bit unsigned integer.
                        ...
                        {
                            $CurrentIFD['fields'][$i]['value'] = $this->TIFFendian2Int(substr($CurrentIFD['fields'][$i]['raw']['offset'],
                                0, 1), $info['tiff']['byte_order']);
                        }
                        else
                        {
                            $CurrentIFD['fields'][$i]['offset'] = $this->TIFFendian2Int($CurrentIFD['fields'][$i]['raw']['offset'],
                                $info['tiff']['byte_order']);
                        }
                        break;
                    ...
                    case 4: // LONG  A 32-bit (4-byte) unsigned integer.
                        ...
                        {
                            $CurrentIFD['fields'][$i]['value'] = $this->TIFFendian2Int($CurrentIFD['fields'][$i]['raw']['offset'],
                                $info['tiff']['byte_order']);
                        }
                        else
                        {
                ...
                switch ($fieldarray['raw']['tag'])
                {
                    ...
                    case 258: // BitsPerSample
                        ...
                        else
                        {
                            ...
                            {
                                $info['video']['bits_per_sample'] += $this->TIFFendian2Int(substr($info['tiff']['ifd'][$IFDid]['fields'][$key]['raw']['data'],
                                    $i * $FieldTypeByteLength[$fieldarray['raw']['type']],
                                    $FieldTypeByteLength[$fieldarray['raw']['type']]), $info['tiff']['byte_order']);
                            }
                        }
                        break;
