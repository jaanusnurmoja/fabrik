        ...
        switch ($RIFFsubtype)
        {
            ...
            case 'WAVE':
                ...
                if (isset($thisfile_riff_WAVE['bext'][0]['data']))
                {
                    ...
                    if (
                    preg_match('#^([0-9]{4}).([0-9]{2}).([0-9]{2})$#', $thisfile_riff_WAVE_bext_0['origin_date'],
                        $matches_bext_date)
                    )
                    {
                        if (
                        preg_match('#^([0-9]{2}).([0-9]{2}).([0-9]{2})$#', $thisfile_riff_WAVE_bext_0['origin_time'],
                            $matches_bext_time)
                        )
                        {
...
*/
        ...
        switch ($RIFFsubtype)
        {
            ...
            case 'AIFC':
                ...
                {
                    if (
                        isset($thisfile_riff[$RIFFsubtype][$ID3v2_key_bad]) && !array_key_exists($ID3v2_key_good,
                    ...
                    default:
                        ...
                        switch ($chunkname)
                        {
                            case 'data':
                                ...
                                if (
                                preg_match('/^\xFF[\xE2-\xE7\xF2-\xF7\xFA-\xFF][\x00-\xEB]/s', substr($testData, 0, 4))
                                )
                                {
                                ...
                                elseif (
                                    ($isRegularAC3 = (substr($testData, 0,
                                ...
                                elseif (
                                preg_match('/^(' . implode('|', array_map('preg_quote', getid3_dts::$syncwords)) . ')/',
                                    $testData)
                                )
                                {
