...
class getid3_write_id3v2
{
    ...
    public function WriteID3v2()
    {
        ...
        if (!empty($this->filename) && (is_writeable($this->filename) || (!file_exists($this->filename) && is_writeable(dirname($this->filename)))))
        {
            ...
            $this->paddedlength = (isset($OldThisFileInfo['id3v2']['headerlength']) ? max($OldThisFileInfo['id3v2']['headerlength'],
                $this->paddedlength) : $this->paddedlength);
            ...
            if ($NewID3v2Tag = $this->GenerateID3v2Tag())
            {
                ...
                if (file_exists($this->filename) && is_writeable($this->filename) && isset($OldThisFileInfo['id3v2']['headerlength']) && ($OldThisFileInfo['id3v2']['headerlength'] == strlen($NewID3v2Tag)))
                {
                    ...
                    if (file_exists($this->filename))
                    {
                        ...
                        if (
                            is_readable($this->filename) && is_writable($this->filename) && is_file($this->filename) && ($fp = fopen($this->filename,
                                'r+b'))
                        )
                        {
                            rewind($fp);
                    ...
                    else
                    {
                        ...
                        if (
                            is_writable($this->filename) && is_file($this->filename) && ($fp = fopen($this->filename,
                                'wb'))
                        )
                        {
                            rewind($fp);
                ...
                else
                {
                    ...
                    {
                        if (
                            is_readable($this->filename) && is_file($this->filename) && ($fp_source = fopen($this->filename,
                                'rb'))
                        )
                        {
                            if (
                                is_writable($tempfilename) && is_file($tempfilename) && ($fp_temp = fopen($tempfilename,
                                    'wb'))
                            )
                            {
    ...
    public function RemoveID3v2()
    {
        ...
        if (is_writeable(dirname($this->filename)))
        {
            ...
            if (is_readable($this->filename) && is_file($this->filename) && ($fp_source = fopen($this->filename, 'rb')))
            {
                ...
                if (
                    is_writable($this->filename) && is_file($this->filename) && ($fp_temp = fopen($this->filename . 'getid3tmp',
                        'w+b'))
                )
                {
                    while ($buffer = fread($fp_source, $this->fread_buffer_size))
        ...
        elseif (is_writable($this->filename))
        {
            ...
            if (is_readable($this->filename) && is_file($this->filename) && ($fp_source = fopen($this->filename, 'rb')))
            {
                ...
                if ($fp_temp = tmpfile())
                {
                    ...
                    fclose($fp_source);
                    if (
                        is_writable($this->filename) && is_file($this->filename) && ($fp_source = fopen($this->filename,
                            'wb'))
                    )
                    {
                        rewind($fp_temp);
            ...
            switch ($frame_name)
            {
                case 'UFID':
                    ...
                    else
                    {
                        ...
                        $framedata .= substr($source_data_array['data'], 0,
                            64); // max 64 bytes - truncate anything longer
                    }
                    break;
                ...
                case 'WXXX':
                    ...
                    elseif (
                        !isset($source_data_array['data']) || !$this->IsValidURL($source_data_array['data'], false,
                            false)
                    )
                    {
                ...
                case 'MLLT':
                    ...
                    {
                        $framedata .= getid3_lib::BigEndian2String($source_data_array['framesbetweenreferences'], 2,
                            false);
                    }
                    else
                    {
                    ...
                    {
                        $framedata .= getid3_lib::BigEndian2String($source_data_array['bytesbetweenreferences'], 3,
                            false);
                    }
                    else
                    {
                    ...
                    foreach ($source_data_array as $key => $val)
                    {
                        ...
                        {
                            $unwrittenbitstream .= str_pad(getid3_lib::Dec2Bin($val['bytedeviation']),
                                $source_data_array['bitsforbytesdeviation'], '0', STR_PAD_LEFT);
                            $unwrittenbitstream .= str_pad(getid3_lib::Dec2Bin($val['msdeviation']),
                                $source_data_array['bitsformsdeviation'], '0', STR_PAD_LEFT);
                        }
                    }
                ...
                case 'RVA2':
                    ...
                    foreach ($source_data_array as $key => $val)
                    {
                        if ($key != 'description')
                        {
                            ...
                            $framedata .= getid3_lib::BigEndian2String($val['volumeadjust'], 2, false,
                                true); // signed 16-bit
                            if (!$this->IsWithinBitRange($source_data_array['bitspeakvolume'], 8, false))
                            {
                                ...
                                {
                                    $framedata .= getid3_lib::BigEndian2String($val['peakvolume'],
                                        ceil($val['bitspeakvolume'] / 8), false, false);
                                }
                            }
                            else
                            {
                ...
                case 'RVAD':
                    ...
                    else
                    {
                        ...
                        $framedata .= getid3_lib::BigEndian2String($source_data_array['volumechange']['right'],
                            ceil($source_data_array['bitsvolume'] / 8), false);
                        $framedata .= getid3_lib::BigEndian2String($source_data_array['volumechange']['left'],
                            ceil($source_data_array['bitsvolume'] / 8), false);
                        $framedata .= getid3_lib::BigEndian2String($source_data_array['peakvolume']['right'],
                            ceil($source_data_array['bitsvolume'] / 8), false);
                        $framedata .= getid3_lib::BigEndian2String($source_data_array['peakvolume']['left'],
                            ceil($source_data_array['bitsvolume'] / 8), false);
                        if (
                            $source_data_array['volumechange']['rightrear'] || $source_data_array['volumechange']['leftrear'] ||
                        ...
                        {
                            $framedata .= getid3_lib::BigEndian2String($source_data_array['volumechange']['rightrear'],
                                ceil($source_data_array['bitsvolume'] / 8), false);
                            $framedata .= getid3_lib::BigEndian2String($source_data_array['volumechange']['leftrear'],
                                ceil($source_data_array['bitsvolume'] / 8), false);
                            $framedata .= getid3_lib::BigEndian2String($source_data_array['peakvolume']['rightrear'],
                                ceil($source_data_array['bitsvolume'] / 8), false);
                            $framedata .= getid3_lib::BigEndian2String($source_data_array['peakvolume']['leftrear'],
                                ceil($source_data_array['bitsvolume'] / 8), false);
                        }
                        if (
                            $source_data_array['volumechange']['center'] || $source_data_array['peakvolume']['center'] ||
                        ...
                        {
                            $framedata .= getid3_lib::BigEndian2String($source_data_array['volumechange']['center'],
                                ceil($source_data_array['bitsvolume'] / 8), false);
                            $framedata .= getid3_lib::BigEndian2String($source_data_array['peakvolume']['center'],
                                ceil($source_data_array['bitsvolume'] / 8), false);
                        }
                        if ($source_data_array['volumechange']['bass'] || $source_data_array['peakvolume']['bass'])
                        {
                            $framedata .= getid3_lib::BigEndian2String($source_data_array['volumechange']['bass'],
                                ceil($source_data_array['bitsvolume'] / 8), false);
                            $framedata .= getid3_lib::BigEndian2String($source_data_array['peakvolume']['bass'],
                                ceil($source_data_array['bitsvolume'] / 8), false);
                        }
                    }
                    break;
                        ...
                        foreach ($source_data_array as $key => $val)
                        {
                            if ($key != 'bitsvolume')
                            {
                                ...
                                else
                                {
                                    ...
                                    $framedata .= getid3_lib::BigEndian2String($val,
                                        ceil($source_data_array['adjustmentbits'] / 8), false);
                                }
                            }
                        }
                    }
                ...
                case 'APIC':
                    ...
                    elseif (
                        ($source_data_array['mime'] == '-->') && (!$this->IsValidURL($source_data_array['data'], false,
                            false))
                    )
                    {
                        //$this->errors[] = 'Invalid URL in '.$frame_name.' ('.$source_data_array['data'].')';
                ...
                case 'LINK':
                    ...
                    elseif (
                        ($source_data_array['frameid'] == 'USER') && (getid3_id3v2::LanguageLookup($source_data_array['additionaldata'],
                                true) == '')
                    )
                    {
                    ...
                    elseif (
                        (($source_data_array['frameid'] == 'COMM') || ($source_data_array['frameid'] == 'SYLT') || ($source_data_array['frameid'] == 'USLT')) && ((getid3_id3v2::LanguageLookup(substr($source_data_array['additionaldata'],
                                    0, 3), true) == '') || (substr($source_data_array['additionaldata'], 3) == ''))
                    )
                    {
                    ...
                    else
                    {
                        ...
                        switch ($source_data_array['frameid'])
                        {
                            ...
                            default:
                                if (
                                    (substr($source_data_array['frameid'], 0,
                                            1) == 'T') || (substr($source_data_array['frameid'], 0, 1) == 'W')
                                )
                                {
                                    // no additional data required
                                }
                ...
                case 'ASPI':
                    ...
                    else
                    {
                        ...
                        {
                            $framedata .= getid3_lib::BigEndian2String($val,
                                ceil($source_data_array['bitsperpoint'] / 8), false);
                        }
                    }
                    break;
                ...
                case 'RGAD':
                    ...
                    else
                    {
                        ...
                        $framedata .= getid3_lib::RGADgainString($source_data_array['raw']['track_name'],
                            $source_data_array['raw']['track_originator'], $source_data_array['track_adjustment']);
                        $framedata .= getid3_lib::RGADgainString($source_data_array['raw']['album_name'],
                            $source_data_array['raw']['album_originator'], $source_data_array['album_adjustment']);
                    }
                    break;
        ...
        if ($this->majorversion == 4)
        {
            switch ($frame_name)
            {
                ...
                case 'COMM':
                    ...
                    elseif (
                    in_array($frame_name . $source_data_array['language'] . $source_data_array['description'],
                        $PreviousFrames)
                    )
                    {
                ...
                case 'SIGN':
                    ...
                    elseif (
                    in_array($frame_name . $source_data_array['ownerid'] . $source_data_array['data'], $PreviousFrames)
                    )
                    {
    ...
    public function GenerateID3v2Tag($noerrorsonly = true)
    {
        ...
        if (is_array($this->tag_data))
        {
            foreach ($this->tag_data as $frame_name => $frame_rawinputdata)
            {
                foreach ($frame_rawinputdata as $irrelevantindex => $source_data_array)
                {
                    if (getid3_id3v2::IsValidID3v2FrameName($frame_name, $this->majorversion))
                    {
                        ...
                        {
                            if (
                                array_key_exists('description', $source_data_array) && array_key_exists('encodingid',
                                    $source_data_array) && array_key_exists('encoding', $this->tag_data)
                            )
                            {
                                $source_data_array['description'] = getid3_lib::iconv_fallback($this->tag_data['encoding'],
                                    $source_data_array['encoding'], $source_data_array['description']);
                            }
                            if ($frame_data = $this->GenerateID3v2FrameData($frame_name, $source_data_array))
                            {
                                ...
                                $frame_flags = $this->GenerateID3v2FrameFlags($this->ID3v2FrameFlagsLookupTagAlter($frame_name),
                                    $this->ID3v2FrameFlagsLookupFileAlter($frame_name), false, false, false, false,
                                    $FrameUnsynchronisation, false);
                            }
                        }
                        else
            ...
            if (!$footer && ($this->paddedlength > (strlen($tagstring) + getid3_id3v2::ID3v2HeaderLength($this->majorversion))))
            {
                ...
                {
                    $tagstring .= str_repeat("\x00",
                        $this->paddedlength - strlen($tagstring) - getid3_id3v2::ID3v2HeaderLength($this->majorversion));
                }
            }
    ...
    public function array_join_merge($arr1, $arr2)
    {
        ...
        if (is_array($arr1) && is_array($arr2))
        {
            ...
            if ($this->is_hash($arr1) && $this->is_hash($arr2))
            {
                ...
                foreach ($keys as $key)
                {
                    $new_array[$key] = $this->array_join_merge((isset($arr1[$key]) ? $arr1[$key] : ''),
