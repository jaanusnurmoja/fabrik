...
class getid3_writetags
{
            ...
            if ($this->remove_other_tags)
            {
                foreach ($AllowedTagFormats as $AllowedTagFormat)
                {
                    switch ($AllowedTagFormat)
                    {
                        ...
                        case 'id3v2.4':
                            if (
                                !in_array('id3v2', $TagFormatsToRemove) && !in_array('id3v2.2',
                                    $this->tagformats) && !in_array('id3v2.3',
                                    $this->tagformats) && !in_array('id3v2.4', $this->tagformats)
                            )
                            {
            ...
            switch ($tag_key)
            {
                ...
                default:
                    foreach ($valuearray as $key => $value)
                    {
                        ...
                        {
                            $ape_tag_data[$tag_key][$key] = getid3_lib::iconv_fallback($this->tag_encoding, 'UTF-8',
                                $value);
                        }
                        else
                        {
    ...
    public function FormatDataForID3v1()
    {
        ...
        $tag_data_id3v1['title'] = getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1',
            implode(' ', (isset($this->tag_data['TITLE']) ? $this->tag_data['TITLE'] : [])));
        $tag_data_id3v1['artist'] = getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1',
            implode(' ', (isset($this->tag_data['ARTIST']) ? $this->tag_data['ARTIST'] : [])));
        $tag_data_id3v1['album'] = getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1',
            implode(' ', (isset($this->tag_data['ALBUM']) ? $this->tag_data['ALBUM'] : [])));
        $tag_data_id3v1['year'] = getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1',
            implode(' ', (isset($this->tag_data['YEAR']) ? $this->tag_data['YEAR'] : [])));
        $tag_data_id3v1['comment'] = getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1',
            implode(' ', (isset($this->tag_data['COMMENT']) ? $this->tag_data['COMMENT'] : [])));
        $tag_data_id3v1['track'] = intval(getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1',
            implode(' ', (isset($this->tag_data['TRACKNUMBER']) ? $this->tag_data['TRACKNUMBER'] : []))));
                            ...
                            if ($id3v2_majorversion < 4)
                            {
                                ...
                                if (!$ID3v2_tag_data_converted)
                                {
                                    ...
                                    $tag_data_id3v2[$ID3v2_framename][$key]['data'] = "\xFF\xFE" . getid3_lib::iconv_fallback($this->tag_encoding,
                                            'UTF-16LE', $value); // force LittleEndian order version of UTF-16
                                    $ID3v2_tag_data_converted = true;
                                }
    ...
    public function FormatDataForVorbisComment()
    {
        ...
        foreach ($tag_data_vorbiscomment as $tag_key => $valuearray)
        {
            foreach ($valuearray as $key => $value)
            {
                ...
                {
                    $tag_data_vorbiscomment[$tag_key][$key] = getid3_lib::iconv_fallback($this->tag_encoding, 'UTF-8',
                        $value);
                }
                else
                {
    ...
    {
        $tag_data_real['title'] = getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1',
            implode(' ', (isset($this->tag_data['TITLE']) ? $this->tag_data['TITLE'] : [])));
        $tag_data_real['artist'] = getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1',
            implode(' ', (isset($this->tag_data['ARTIST']) ? $this->tag_data['ARTIST'] : [])));
        $tag_data_real['copyright'] = getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1',
            implode(' ', (isset($this->tag_data['COPYRIGHT']) ? $this->tag_data['COPYRIGHT'] : [])));
        $tag_data_real['comment'] = getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1',
            implode(' ', (isset($this->tag_data['COMMENT']) ? $this->tag_data['COMMENT'] : [])));
