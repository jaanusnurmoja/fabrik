...
class getid3_szip extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        {
            $info['error'][] = 'Expecting "53 5A 0A 04" at offset ' . $info['avdataoffset'] . ', found "' . getid3_lib::PrintHexBytes(substr($SZIPHeader,
                    0, 4)) . '"';
            return false;
        }
            ...
            switch ($NextBlockID)
            {
                ...
                case 'BH':
                    ...
                    while (strpos($BHheaderdata, "\x00", $BHheaderoffset) > 0)
                    {
                        ...
                        $BHdataArray['filelength'] = getid3_lib::BigEndian2Int(substr($BHheaderdata, $BHheaderoffset,
                            3));
                        $BHheaderoffset += 3;

                        $BHdataArray['access_flags'] = getid3_lib::BigEndian2Int(substr($BHheaderdata, $BHheaderoffset,
                            2));
                        $BHheaderoffset += 2;

                        $BHdataArray['creation_time'] = getid3_lib::BigEndian2Int(substr($BHheaderdata, $BHheaderoffset,
                            4));
                        $BHheaderoffset += 4;

                        $BHdataArray['modification_time'] = getid3_lib::BigEndian2Int(substr($BHheaderdata,
                            $BHheaderoffset, 4));
                        $BHheaderoffset += 4;

                        $BHdataArray['access_time'] = getid3_lib::BigEndian2Int(substr($BHheaderdata, $BHheaderoffset,
                            4));
                        $BHheaderoffset += 4;
