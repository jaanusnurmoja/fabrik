...
class getid3_jpg extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        list($width, $height, $type) = getimagesize($info['filenamepath'],
            $imageinfo); // http://www.getid3.org/phpBB3/viewtopic.php?t=1474
...
//return false;
                        ...
                        {
                            $info['warning'][] = 'exif_read_data() cannot parse non-EXIF data in APP1 (expected "Exif", found "' . substr($imageinfo['APP1'],
                                    0, 4) . '")';
                        }
                    }
                    else
        ...
        if (isset($info['jpg']['exif']['GPS']))
        {
            ...
            if (isset($info['jpg']['exif']['GPS']['GPSDateStamp']))
            {
                ...
                $info['jpg']['exif']['GPS']['computed']['timestamp'] = gmmktime($computed_time[0], $computed_time[1],
                    $computed_time[2], $computed_time[3], $computed_time[4], $computed_time[5]);
            }
