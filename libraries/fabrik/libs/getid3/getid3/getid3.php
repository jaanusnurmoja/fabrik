...
class getID3
{
    ...
    public function __construct()
    {
        ...
        elseif (preg_match('#([0-9]+)G#i', $this->memory_limit, $matches))
                    ...
                    if (strpos($value, ' ') !== false)
                    {
                        if (!empty($path_so_far))
                        {
                            ...
                            foreach ($lines as $line)
                            {
                                $line = trim($line);
                                if (
                                preg_match('#^([0-9/]{10}) +([0-9:]{4,5}( [AP]M)?) +(<DIR>|[0-9,]+) +([^ ]{0,11}) +(.+)$#',
                                    $line, $matches)
                                )
                                {
    ...
    public function openfile($filename, $filesize = null)
    {
        try
        {
            ...
            if (
                (is_readable($filename) || file_exists($filename)) && is_file($filename) && ($this->fp = fopen($filename,
                    'rb'))
            )
            {
                // great
    ...
    public function analyze($filename, $filesize = null, $original_filename = '')
    {
        try
        {
            ...
            if (
                isset($determined_format['fail_id3']) && (in_array('id3v1', $this->info['tags']) || in_array('id3v2',
                        $this->info['tags']))
            )
            {
    ...
    public function GetFileFormat(&$filedata, $filename = '')
    {
        ...
        elseif (
            preg_match('/\.cue$/i', $filename) && preg_match('#FILE "[^"]+" (BINARY|MOTOROLA|AIFF|WAVE|MP3)#',
                $filedata)
        )
        {
                            ...
                            foreach ($this->info['tags'][$tag_name] as $tag_key => $valuearray)
                            {
                                foreach ($valuearray as $key => $value)
                                {
                                    if (preg_match('#^[\\x80-\\xFF]+$#', $value))
                                    {
                                        ...
                                        {
                                            if (
                                                @mb_convert_encoding($value, $id3v1_bad_encoding,
                                                    $id3v1_bad_encoding) === $value
                                            )
                                            {
    ...
    public function saveAttachment($name, $offset, $length, $image_mime = null)
    {
        try
        {
            ...
            else
            {
                ...
                $dir = rtrim(str_replace(['/', '\\'], DIRECTORY_SEPARATOR, $this->getid3->option_save_attachments),
                    DIRECTORY_SEPARATOR);
                ...
                {
                    if (
                        ($buffer = $this->fread(min($buffersize,
