...
class getid3_mp3 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if (
            !empty($CurrentDataLAMEversionString) && (substr($CurrentDataLAMEversionString, 0,
        ...
        )
        {
            ...
            if (
                ($PossiblyLongerLAMEversion_String = strstr($PossiblyLongerLAMEversion_Data,
                    $CurrentDataLAMEversionString)) !== false
            )
            {
                if (
                    substr($PossiblyLongerLAMEversion_String, 0,
    ...
    public function GuessEncoderOptions()
    {
        ...
        elseif (
            !empty($thisfile_mpeg_audio_lame['preset_used']) && (!in_array($thisfile_mpeg_audio_lame['preset_used_id'],
                $NamedPresetBitrates))
        )
        {
    ...
    public function decodeMPEGaudioHeader($offset, &$info, $recursivesearch = true, $ScanAsCBR = false, $FastMPEGheaderScan = false)
    {
        ...
        else
        {
            ...
            if (
                (substr($headerstring, $VBRidOffset, strlen('Xing')) == 'Xing') || (substr($headerstring, $VBRidOffset,
                        strlen('Info')) == 'Info')
            )
            {
    ...
    public function FreeFormatFrameLength($offset, $deepscan = false)
    {
        ...
        if ($deepscan)
        {
            ...
            while ($nextoffset < ($info['avdataend'] - 6))
            {
                ...
                if (
                    (substr($NextSyncPattern, 1, strlen($SyncPattern1)) == $SyncPattern1) || (substr($NextSyncPattern,
                ...
                elseif (
                    (substr($NextSyncPattern, 0, strlen($SyncPattern1)) == $SyncPattern1) || (substr($NextSyncPattern,
                            0, strlen($SyncPattern2)) == $SyncPattern2)
                )
                {
                ...
                elseif (
                    (substr($NextSyncPattern, 2, strlen($SyncPattern1)) == $SyncPattern1) || (substr($NextSyncPattern,
                            2, strlen($SyncPattern2)) == $SyncPattern2)
                )
                {
                        ...
                        for ($current_segment = 0; $current_segment < $max_scan_segments; $current_segment++)
                        {
                            ...
                            if ($current_segment > 0)
                            {
                                ...
                                for ($j = 0; $j < (strlen($buffer_4k) - 4); $j++)
                                {
                                    ...
                                    { // synch detected
                                        if (
                                        $this->decodeMPEGaudioHeader($scan_start_offset[$current_segment] + $j, $dummy,
                                            false, false, $FastMode)
                                        )
                                        {
                                            $calculated_next_offset = $scan_start_offset[$current_segment] + $j + $dummy['mpeg']['audio']['framelength'];
                                            if (
                                            $this->decodeMPEGaudioHeader($calculated_next_offset, $dummy, false, false,
                                                $FastMode)
                                            )
                                            {
