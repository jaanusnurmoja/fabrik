...
class getid3_id3v2 extends getid3_handler
{
    ...
    public function Analyze()
    {
            ...
            { // cycle through until no more frame data is left to parse
                ...
                elseif ($id3v2_majorversion > 2)
                {
                    ...
                    if ($frame_size < (strlen($framedata) + 4))
                    {
                        ...
                        elseif (
                            ($id3v2_majorversion == 4) && ($this->IsValidID3v2FrameName(substr($framedata,
                ...
                if ($iTunesBrokenFrameNameFixed = self::ID3v22iTunesBrokenFrameName($frame_name))
                {
                    $info['warning'][] = 'error parsing "' . $frame_name . '" (' . $framedataoffset . ' bytes into the ID3v2.' . $id3v2_majorversion . ' tag). (ERROR: IsValidID3v2FrameName("' . str_replace("\x00",
                            ' ',
                            $frame_name) . '", ' . $id3v2_majorversion . '))). [Note: this particular error has been known to happen with tags edited by iTunes (versions "X v2.0.3", "v3.0.1", "v7.0.0.70" are known-guilty, probably others too)]. Translated frame name from "' . str_replace("\x00",
                            ' ', $frame_name) . '" to "' . $iTunesBrokenFrameNameFixed . '" for parsing.';
                    $frame_name = $iTunesBrokenFrameNameFixed;
                }
                if (
                    ($frame_size <= strlen($framedata)) && ($this->IsValidID3v2FrameName($frame_name,
                        $id3v2_majorversion))
                )
                {
        ...
        if (
            !isset($thisfile_id3v2['comments']['year']) && !empty($thisfile_id3v2['comments']['recording_time'][0]) && preg_match('#^([0-9]{4})#',
    ...
    public function ParseID3v2Frame(&$parsedFrame)
    {
        ...
        {    // 4.2.2 TXX  User defined text information frame
            ...
            if (
                ord(substr($parsedFrame['data'], $frame_terminatorpos + strlen($frame_textencoding_terminator),
                    1)) === 0
            )
            {
            ...
            if (!empty($parsedFrame['framenameshort']) && !empty($parsedFrame['data']))
            {
                ...
                if (
                    !isset($info['id3v2']['comments'][$parsedFrame['framenameshort']]) || !array_key_exists($commentkey,
        ...
        {     // 4.4  IPL  Involved people list (ID3v2.2 only)
            ...
            if (strpos($parsedFrame['data_raw'], "\x00") !== false)
            {
                $IPLS_parts_unsorted = [];
                if (
                    ((strlen($parsedFrame['data_raw']) % 2) == 0) && ((substr($parsedFrame['data_raw'], 0,
        ...
        {     // 4.10  SLT  Synchronised lyric/text
            ...
            while (strlen($frame_remainingdata))
            {
                ...
                {
                    if (
                        ord(substr($frame_remainingdata, $frame_terminatorpos + strlen($frame_textencoding_terminator),
                            1)) === 0
                    )
                    {
                        $frame_terminatorpos++; // strpos() fooled because 2nd byte of Unicode chars are often 0x00
