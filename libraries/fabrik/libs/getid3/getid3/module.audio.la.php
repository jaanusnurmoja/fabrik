    ...
    public function Analyze()
    {
        ...
        switch (substr($rawdata, $offset, 4))
        {
                ...
                if ($info['la']['footerstart'] < $info['avdataend'])
                {
                    if ($RIFFtempfilename = tempnam(GETID3_TEMP_DIR, 'id3'))
                    {
                        if ($RIFF_fp = fopen($RIFFtempfilename, 'w+b'))
                        {
                            ...
                            $RIFFdata = 'RIFF' . getid3_lib::LittleEndian2String(strlen($RIFFdata), 4,
                                    false) . $RIFFdata;
            ...
            default:
                ...
                {
                    $info['error'][] = 'This version of getID3() [' . $this->getid3->version() . '] does not support LA version ' . substr($rawdata,
                            $offset + 2, 1) . '.' . substr($rawdata, $offset + 3,
                            1) . ' which this appears to be - check http://getid3.sourceforge.net for updates.';
                }
                else
                {
