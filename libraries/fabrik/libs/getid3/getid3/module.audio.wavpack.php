...
class getid3_wavpack extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        while (true)
        {
            ...
            if (
                empty($info['wavpack']['blockheader']['block_samples']) ||
            ...
            )
            {
                ...
                $info['wavpack']['blockheader']['total_samples'] = getid3_lib::LittleEndian2Int(substr($wavpackheader,
                    12, 4));
                $info['wavpack']['blockheader']['block_index'] = getid3_lib::LittleEndian2Int(substr($wavpackheader, 16,
                    4));
                $info['wavpack']['blockheader']['block_samples'] = getid3_lib::LittleEndian2Int(substr($wavpackheader,
                    20, 4));
                $info['wavpack']['blockheader']['flags_raw'] = getid3_lib::LittleEndian2Int(substr($wavpackheader, 24,
                    4));
            ...
            while (!feof($this->getid3->fp) && ($this->ftell() < ($blockheader_offset + $blockheader_size + 8)))
            {
                ...
                $metablock['size'] = getid3_lib::LittleEndian2Int(substr($metablockheader,
                        1)) * 2; // size is stored in words
                $metablock['data'] = null;
                ...
                if ($metablock['size'] > 0)
                {
                    ...
                    switch ($metablock['function_id'])
                    {
                        ...
                        default:
                            $info['warning'][] = 'Unexpected metablock type "0x' . str_pad(dechex($metablock['function_id']),
                                    2, '0', STR_PAD_LEFT) . '" at offset ' . $metablock['offset'];
                            $this->fseek($metablock['offset'] + ($metablock['large_block'] ? 4 : 2) + $metablock['size']);
                            break;
                    }
                    ...
                    {
                        case 0x21: // ID_RIFF_HEADER
                            getid3_lib::IncludeDependency(GETID3_INCLUDEPATH . 'module.audio-video.riff.php', __FILE__,
                                true);
                        ...
                        case 0x22: // ID_RIFF_TRAILER
                            ...
                            $getid3_riff = new getid3_riff($getid3_temp);
                            $metablock['riff'] = $getid3_riff->ParseRIFF($startoffset,
                                $startoffset + $metablock['size']);
                        ...
                        case 0x26: // ID_MD5_CHECKSUM
                            ...
                            {
                                $info['md5_data_source'] = strtolower(getid3_lib::PrintHexBytes($metablock['data'],
                                    true, false, false));
                            }
                            else
                            {
        ...
        $info['audio']['encoder'] = 'WavPack v' . $info['wavpack']['blockheader']['major_version'] . '.' . str_pad($info['wavpack']['blockheader']['minor_version'],
                2, '0', STR_PAD_LEFT);
