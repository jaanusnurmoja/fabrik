...
class Html
{
     ...
     *
     */
    public static function mocha($selector = '', $params = [])
    {
...
EOD;
    ...
    public static function emailSent()
    {
        ...
        if (!$j3)
        {
            ...
            <a href='javascript:window.close();'> <span class="small"><?php
                    echo Text::_('COM_FABRIK_CLOSE_WINDOW'); ?>
</span>
            </a>
            <?php
        }
    ...
    public static function conditionList($listId, $sel = '')
    {
        ...
        return JHTML::_('select.genericlist', $conditions, $name, 'class="inputbox input-mini" size="1" ', 'value',
            'text', $sel);
    }
    ...
    public static function tableList($sel = '')
    {
        ...
        return JHTML::_('select.genericlist', $rows, 'fabrik__swaptable', 'class="inputbox" size="1" ', 'id', 'label',
            $sel);
    }
    ...
    public static function calendar()
    {
        ...
        $attribs = ['title' => JText::_('JLIB_HTML_BEHAVIOR_GREEN'), 'media' => 'all'];
        JHtml::_('stylesheet', 'system/calendar-jos.css', ['version' => 'auto', 'relative' => true], $attribs);
        JHtml::_('script', $tag . '/calendar.js', ['version' => 'auto', 'relative' => true]);
        JHtml::_('script', $tag . '/calendar-setup.js', ['version' => 'auto', 'relative' => true]);
    ...
    public static function framework()
    {
        if (!self::$framework)
        {
            ...
            $bootstrapped = in_array($app->getTemplate(), ['bootstrap', 'fabrik4']) || $version->RELEASE > 2.5;
     ...
     *
     */
    public static function iniRequireJs($shim = [], $paths = [])
    {
        ...
        $newShim = $session->get('fabrik.js.shim', []);
        ...
        self::addRequireJsShim($framework, 'fab/autocomplete-bootstrap', ['fab/fabrik'], false);
        self::addRequireJsShim($framework, 'jQueryUI', ['jquery'], false);
     ...
     */
    public static function jLayoutJs($name, $layoutName, stdClass $data = null, $paths = [], $options = [])
    {
    ...
    public static function script($file, $onLoad = '', $minSuffix = '-min.js')
    {
        ...
        $names = array_keys($files) !== range(0, count($files) - 1) ? array_keys($files) : [];
     ...
     * @return  void
     */
    public static function tips($selector = '.hasTip', $params = [], $selectorPrefix = 'document')
    {
        $sig = md5(serialize([$selector, $params]));
    ...
    public static function folderAjaxSelect($folders, $path = '', $tpl = '')
    {
        ...
        $str[] = self::image('orderneutral.png', 'form', $tpl,
            ['alt' => Text::_('COM_FABRIK_BROWSE_FOLDERS'), 'icon-class' => 'icon-menu-2']);
        $str[] = '</a>';
     ...
     * @return  void
     */
    public static function autoComplete($htmlId, $elementId, $formId, $plugin = 'field', $opts = [])
    {
        /*
		$input = JFactory::getApplication()->input;
     ...
     */
    public static function autoCompleteOptions($htmlId, $elementId, $formId, $plugin = 'field', $opts = [])
    {
        $json = new stdClass;
    ...
    public static function atWho($selector, $placeHolders = [])
    {
     ...
     * @return  string
     */
    public static function facebookGraphAPI($appId, $locale = 'en_US', $meta = [])
    {
...
EOT;
     ...
     *
     */
    public static function image($file, $type = 'form', $tmpl = '', $properties = [], $srcOnly = false, $opts = [])
    {
    ...
    public static function gridItems($values, $labels, $selected, $name, $type = 'checkbox',
                                     $elementBeforeLabel = true, $classes = [], $buttonGroup = false, $inputDataAttributes = [])
    {
        $items = [];
    ...
    public static function grid($values, $labels, $selected, $name, $type = 'checkbox',
                                $elementBeforeLabel = true, $optionsPerRow = 4, $classes = [], $buttonGroup = false, $dataAttributes = [],
                                $inputDataAttributes = [])
    {
        if (Worker::j3())
        {
     ...
     */
    public static function getPHPTemplate($tmpl, $data = [], $model = null)
    {
        ...
        foreach ($data as $key => $d)
        {
            ...
            if ($d != '')
            {
                if (trim($baseUrl) == '')
                {
                    ...
                    {
                        $thisUrl = strstr($url,
                            '?') ? $url . '&' . $name . '[value]=' . urlencode($d) : $url . '?' . $name . '[value]=' . urlencode($d);
                    }
     ...
     *
     */
    public static function a($href, $lbl = '', $opts = [], $normalize = false)
    {
    ...
    public static function getSmartLinkType($link)
    {
		...
		*/
        $ret = ['width' => '800', 'height' => '600', 'type' => 'mediabox'];
     ...
     * @return LayoutFile
     */
    public static function getLayout($name, $paths = [], $options = [])
    {
        $defaultOptions = [
            'debug'     => false,
    ...
    public static function modalJLayouts()
    {
        ...
        Html::jLayoutJs('icon-expand', 'fabrik-icon', (object)['icon' => 'icon-expand']);
        Html::jLayoutJs('icon-full-screen', 'fabrik-icon', (object)['icon' => 'icon-out-2 icon-fullscreen']);
    }
