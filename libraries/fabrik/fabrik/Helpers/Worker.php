...
class Worker
{
    ...
    public static function strToDateTime($date, $format)
    {
        ...
        $shortMonths = [
            Text::_('Jan'), Text::_('Feb'), Text::_('Mar'), Text::_('Apr'), Text::_('May'), Text::_('Jun'), Text::_('Jul'),
         ...
         */
        $dateTime = ['sec' => 0, 'min' => 0, 'hour' => 0, 'day' => 1, 'mon' => 0, 'year' => 0, 'timestamp' => 0];
        ...
        $dateTime['timestamp'] = mktime($dateTime['hour'], $dateTime['min'], $dateTime['sec'], $dateTime['mon'],
    ...
    public static function specialStrToMySQL($date, $gmt = true)
    {
        ...
        // E.g. next Wednesday
        preg_match("/[next|last]* (monday\b|tuesday\b|wednesday\b|thursday\b|friday\b|saturday\b|sunday\b)/i", $date,
            $matches3);
    ...
    public static function str2Time($date, $format)
    {
        ...
        $search = ['%d', '%e', '%D', '%j', '%m', '%b', '%Y', '%y', '%g', '%H', '%h', '%i', '%s', '%S', '%M'];

        $replace = [
            '(\d{2})', '(\d{1,2})', '(\w{3})', '(\d{1,2})', '(\d{2})', '(\w{3})', '(\d{4})', '(\d{2})', '(\d{1,2})', '(\d{2})',
            '(\d{2})', '(\d{2})', '(\d{2})', '(\d{2})', '(\d{2})'
        ];
    ...
    {
        $reservedWords = [
            "task", "view", "layout", "option", "formid", "submit", "ul_max_file_size"
         ...
         */
        $strictWords = ["listid", 'rowid', 'itemid'];
        ...
        if (strstr($msg, '{') && !empty($searchData))
        {
            ...
            if ($groupModel->canRepeat())
            {
                ...
                foreach ($elementModels as $elementModel)
                {
                    ...
                    foreach ([$repeatElName, $repeatElName . '_raw'] as $tmpElName)
                    {
                        if (strstr($msg, '{' . $tmpElName . '}'))
                        {
                            if (
                                array_key_exists($tmpElName,
                                    $searchData) && is_array($searchData[$tmpElName]) && array_key_exists($repeatCounter,
                                    $searchData[$tmpElName])
                            )
                            {
    ...
    public function parseMessageForPlaceHolder($msg, $searchData = null, $keepPlaceholders = true, $addSlashes = false, $theirUser = null, $unsafe = true)
    {
        ...
        foreach ($messages as &$msg)
        {
            ...
            if (!($msg == '' || is_array($msg) || StringHelper::strpos($msg, '{') === false))
            {
                ...
                $msg = preg_replace_callback("/{([^}\s]+(\|\|[\w|\s]+|<\?php.*\?>)*)}/i",
                    [$this, 'replaceWithFormData'], $msg);
    ...
    public static function replaceWithUserData($msg, $user = null, $prefix = 'my')
    {
        ...
        {
            $bits = explode('->', str_replace(['{', '}'], '', $match));
    ...
    protected function replaceWithFormData($matches)
    {
        ...
        if (count($bits) == 2)
        {
            ...
            if (in_array($match, ['', '<ul></ul>', '<ul><li></li></ul>']))
            {
     ...
     */
    public static function fabrikReadDirectory($path, $filter = '.', $recurse = false, $fullPath = false, $aFolderFilter = [],
                                               $foldersOnly = false)
    {
        $arr = [];
        ...
        while ($file = readdir($handle))
        {
            ...
            if ($file != "." && $file != "..")
            {
                ...
                if ($recurse && $isDir)
                {
                    ...
                    if ($goDown)
                    {
                        $arr2 = self::fabrikReadDirectory($dir, $filter, $recurse, $fullPath, $aFolderFilter,
                            $foldersOnly);
    ...
    public static function getContentFilter()
    {
        ...
        else
        {
            ...
            if ($blackList)
            {
                ...
                $filter = JFilterInput::getInstance($tags, array_diff($blackListAttributes, $whiteListAttributes), 1,
                    1);
            }
    ...
    public static function getDbo($loadJoomlaDb = false, $cnnId = null)
    {
        ...
        if (!array_key_exists($sig, self::$database))
        {
            ...
            $driver .= '_fab';
            $options = [
                'driver' => $driver, 'host' => $host, 'user' => $user, 'password' => $password, 'database' => $database,
                'prefix' => $dbPrefix
            ];
    ...
    public static function getConnection($item = null)
    {
        ...
        if (is_object($item))
        {
            $item = is_null($item->connection_id) ? ArrayHelper::getValue($jForm, 'connection_id',
                -1) : $item->connection_id;
        }
    ...
    {
        $db = self::getDbo(true);
        $aNullDates = ['0000-00-000000-00-00', '0000-00-00 00:00:00', '0000-00-00', '', $db->getNullDate()];
    ...
    public static function sendMail($from, $fromName, $recipient, $subject, $body, $mode = false,
                                    $cc = null, $bcc = null, $attachment = null, $replyTo = null, $replyToName = null, $headers = [])
    {
        ...
        try
        {
            $mailer->setSender([$from, $fromName, $autoReplyTo]);
        }
        catch (\Exception $e)
        {
     ...
     * @return  string
     */
    public static function getMenuOrRequestVar($name, $val = '', $mambot = false, $priority = 'menu', $opts = [])
    {
        $app = JFactory::getApplication();
        $input = $app->input;
            ...
            if (!$app->isAdmin())
            {
                if (!$mambot)
                {
                    ...
                    if (is_object($menu))
                    {
                        ...
                        elseif (array_key_exists('formid', $opts))
                        {
    ...
    public static function getCache($listModel = null, $group = null, $ttl = null)
    {
        ...
        $opts = ['defaultgroup' => 'com_' . $package, 'cachebase' => $base, 'lifetime' => $time, 'language' => 'en-GB', 'storage' => 'file'];
    ...
    public static function useCache($listModel = null, $noGuest = true, $excludedFormats = null)
    {
        ...
        {
            $excludedFormats = ['raw', 'csv', 'pdf', 'json', 'fabrikfeed', 'feed'];
        }
    ...
    public static function formDefaults($form)
    {
        ...
        $form = JForm::getInstance('com_fabrik.' . $form, $form, ['control' => '', 'load_data' => true]);
        $fs = $form->getFieldset();
        $json = ['params' => []];
    ...
    public static function getID3Instance()
    {
        ...
        if (JFile::exists(COM_FABRIK_LIBRARY . '/libs/getid3/getid3/getid3.php'))
        {
            ...
            \getid3_lib::IncludeDependency(COM_FABRIK_LIBRARY . '/libs/getid3/getid3/extension.cache.mysqli.php',
                __FILE__, true);
            $config = JFactory::getConfig();
