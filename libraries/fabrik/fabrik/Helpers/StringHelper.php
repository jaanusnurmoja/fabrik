...
class StringHelper extends \Joomla\String\StringHelper
{
        ...
        if ($considerHtml)
        {
            ...
            foreach ($lines as $lineMatchings)
            {
                ...
                if ($totalLength + $contentLength > $length)
                {
                    ...
                    if (
                    preg_match_all('/&[0-9a-z]{2,8};|&#[0-9]{1,7};|[0-9a-f]{1,6};/i', $lineMatchings[2], $entities,
                        PREG_OFFSET_CAPTURE)
                    )
                    {
     ...
     * @return  string
     */
    public static function truncate($text, $opts = [])
    {
        $origText = $text;
        ...
        elseif (ArrayHelper::getValue($opts, 'chars', false))
        {
            if (!empty($wordCount))
            {
