...
final class BaseWriter
{
    ...
    public function utf8ToUtf16BigEndian($str, $setbom = true) // UTF8ToUTF16BE
    {
        ...
        {
            if (
            !in_array($this->mpdf->currentfontfamily, [
                'gb', 'big5', 'sjis', 'uhc', 'gbB', 'big5B', 'sjisB', 'uhcB', 'gbI', 'big5I', 'sjisI', 'uhcI',
    ...
    private function endPage($s, $ln)
    {
        ...
        elseif ($this->mpdf->ColActive && !$this->mpdf->processingHeader && !$this->mpdf->processingFooter)
        {
            ...
            elseif (
                $this->mpdf->tableLevel > 0 && preg_match('/\d+\.\d\d+ \d+\.\d\d+ \d+\.\d\d+ ([\-]{0,1}\d+\.\d\d+) re/',
                    $s, $m)
            )
            { // Rect in table
                ...
                $h = ($m[1] / Mpdf::SCALE);
                // Update/overwrite the lowest bottom of printing y value for a column
                $this->mpdf->ColDetails[$this->mpdf->CurrCol]['bottom_margin'] = max($this->mpdf->ColDetails[$this->mpdf->CurrCol]['bottom_margin'],
                    $this->mpdf->y + $h);
