...
class MetadataWriter implements \Psr\Log\LoggerAwareInterface
{
    ...
    public function writeMetadata() // _putmetadata
    {
        ...
        $uuid = sprintf('%04x%04x-%04x-%04x-%04x-%04x%04x%04x', random_int(0, 0xffff), random_int(0, 0xffff),
	 ...
	 <rdf:Seq>
	  <rdf:li>' . $this->mpdf->author . '</rdf:li>
        ...
        elseif ($this->mpdf->PDFA)
        {
            ...
            {
                throw new \Mpdf\MpdfException(sprintf('PDFA version (%s) is not valid. (Use: 1-B, 3-B, etc.)',
                    $this->mpdf->PDFAversion));
            }
    ...
    public function writeCatalog() //_putcatalog
    {
        ...
        if ($this->mpdf->DisplayPreferences || $this->mpdf->directionality === 'rtl' || $this->mpdf->mirrorMargins)
        {
            ...
            if (
                !$this->mpdf->PDFA && !$this->mpdf->PDFX && is_int(strpos($this->mpdf->DisplayPreferences,
                    'NoPrintScaling'))
            )
            {
                ...
                if (isset($this->mpdf->PageLinks[$n]))
                {
                    ...
                    foreach ($this->mpdf->PageLinks[$n] as $key => $pl)
                    {
                        ...
                        else
                        {
                            ...
                            $annot .= sprintf(' /Dest [%d 0 R /XYZ 0 %.3F null]>>', 1 + 2 * $l[0],
                                $htarg - $l[1] * Mpdf::SCALE);
                        }
                ...
                if (isset($this->mpdf->PageAnnots[$n]))
                {
                    ...
                    foreach ($this->mpdf->PageAnnots[$n] as $key => $pl)
                    {
                        ...
                        if (isset($pl['opt']['c']) && $pl['opt']['c'])
                        {
                            ...
                            if ($col{0} == 3 || $col{0} == 5)
                            {
                                $annotcolor .= sprintf('%.3F %.3F %.3F', ord($col{1}) / 255, ord($col{2}) / 255,
                                    ord($col{3}) / 255);
                            }
                            elseif ($col{0} == 1)
                            {
                            ...
                            {
                                $annotcolor .= sprintf('%.3F %.3F %.3F %.3F', ord($col{1}) / 100, ord($col{2}) / 100,
