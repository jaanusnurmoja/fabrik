        ...
        foreach ($this->mpdf->FontFiles as $fontkey => $info)
        {
            ...
            if (isset($info['type']) && $info['type'] === 'TTF' && !$info['sip'] && !$info['smp'])
            {
                ...
                if ($used && !$asSubset)
                {
                    ...
                    if ($this->mpdf->repackageTTF || $this->mpdf->fonts[$fontkey]['TTCfontID'] > 0 || $this->mpdf->fonts[$fontkey]['useOTL'] > 0)
                    { // mPDF 5.7.1
                        ...
                        else
                        {
                            ...
                            $font = $ttf->repackageTTF($this->mpdf->FontFiles[$fontkey]['ttffile'],
                                $this->mpdf->fonts[$fontkey]['TTCfontID'], $this->mpdf->debugfonts,
                                $this->mpdf->fonts[$fontkey]['useOTL']); // mPDF 5.7.1
        ...
        foreach ($this->mpdf->fonts as $k => $font)
        {
            ...
            elseif ($type === 'TTF' && ($font['sip'] || $font['smp']))
            {
                ...
                for ($sfid = 0; $sfid < $subsetCount; $sfid++)
                {
                    ...
                    unset($subset[0]);
                    $ttfontstream = $ttf->makeSubsetSIP($font['ttffile'], $subset, $font['TTCfontID'],
                        $this->mpdf->debugfonts, $font['useOTL']); // mPDF 5.7.1
                    ...
                    foreach ($font['subsets'][$sfid] as $cp => $u)
                    {
                        ...
                        if ($u > 65535)
                        {
                            ...
                            $l2 = ord($utf16[2]);
                            $h2 = ord($utf16[3]);
                            $toUnistring .= sprintf("<%02s> <%02s%02s%02s%02s>\n", strtoupper(dechex($cp)),
                                strtoupper(dechex($l1)), strtoupper(dechex($h1)), strtoupper(dechex($l2)),
                                strtoupper(dechex($h2)));
                        }
                        else
                        {
            ...
            {  // TrueType embedded SUBSETS or FULL
                ...
                if ($asSubset)
                {
                    ...
                    $subset = $font['subset'];
                    unset($subset[0]);
                    $ttfontstream = $ttf->makeSubset($font['ttffile'], $subset, $font['TTCfontID'],
                ...
                else
                {
                    ...
                    {
                        $ttf = new TTFontFile($this->fontCache, $this->fontDescriptor);
                        $charToGlyph = $ttf->getCTG($font['ttffile'], $font['TTCfontID'], $this->mpdf->debugfonts,
                            $font['useOTL']);
                        $cidtogidmap = str_pad('', 256 * 256 * 2, "\x00");
