    ...
    public function writePatterns() // _putpatterns
    {
        ...
        for ($i = 1; $i <= $patternCount; $i++)
        {
            ...
            if ($itype === 'svg' || $itype === 'wmf')
            {
                ...
                $s = sprintf('q %.3F 0 0 %.3F %.3F %.3F cm /FO%d Do Q', $orig_w / $fo_w, -$orig_h / $fo_h,
                    -($orig_w / $fo_w) * $wmf_x, ($orig_w / $fo_w) * $wmf_y, $image_id);
            }
            else
            {
    ...
    public function writeShaders() // _putshaders
    {
        ...
        foreach ($this->mpdf->gradients as $id => $grad)
        {
            ...
            if (empty($grad['is_mask']))
            {
                ...
                if ($grad['type'] == 2)
                {
                    $this->writer->write(sprintf('/Coords [%.3F %.3F %.3F %.3F]', $grad['coords'][0],
                        $grad['coords'][1], $grad['coords'][2], $grad['coords'][3]));
                ...
                elseif ($grad['type'] == 3)
                {
                    ...
                    $this->writer->write(sprintf('/Coords [%.3F %.3F %.3F %.3F %.3F %.3F]', $grad['coords'][0],
                        $grad['coords'][1], $ir, $grad['coords'][2], $grad['coords'][3], $grad['coords'][4]));
                    $this->writer->write('/Function ' . $f1 . ' 0 R');
                    $this->writer->write('/Extend [' . $grad['extend'][0] . ' ' . $grad['extend'][1] . '] ');
                    $this->writer->write('>>');
                }
