    ...
    public function writePages() // _putpages
    {
        ...
        for ($n = 1; $n <= $nb; $n++)
        {
            ...
            foreach ($unused as $fk)
            {
                ...
                {
                    $thispage = preg_replace('/\s\/F' . $this->mpdf->fonts[$fk]['i'] . ' \d[\d.]* Tf\s/is', ' ',
                        $thispage);
                }
            }
            ...
            if (isset($this->mpdf->OrientationChanges[$n]))
            {
                ...
                $this->writer->write(sprintf('/TrimBox [%.3F %.3F %.3F %.3F]', $owidthPt_TB, $owidthPt_LR,
                    $hPt - $owidthPt_TB, $wPt - $owidthPt_LR));
            ...
            { // elseif($wPt != $defwPt || $hPt != $defhPt) {
                ...
                $this->writer->write(sprintf('/TrimBox [%.3F %.3F %.3F %.3F]', $owidthPt_LR, $owidthPt_TB,
                    $wPt - $owidthPt_LR, $hPt - $owidthPt_TB));
            }
