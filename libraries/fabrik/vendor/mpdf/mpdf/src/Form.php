...
class Form
{
    ...
    function print_ob_textarea($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
    {
        ...
        else
        {
            ...
            $ClipPath = sprintf('q %.3F %.3F %.3F %.3F re W n ', $this->mpdf->x * Mpdf::SCALE,
                ($this->mpdf->h - $this->mpdf->y) * Mpdf::SCALE, $w * Mpdf::SCALE, -$h * Mpdf::SCALE);
    ...
    function print_ob_imageinput($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir, $is_table)
    {
        ...
        else
        {
            ...
            $this->writer->write(sprintf('q %.3F 0 0 %.3F %.3F %.3F cm /I%d Do Q',
                $objattr['INNER-WIDTH'] * Mpdf::SCALE, $objattr['INNER-HEIGHT'] * Mpdf::SCALE,
                $objattr['INNER-X'] * Mpdf::SCALE,
                ($this->mpdf->h - ($objattr['INNER-Y'] + $objattr['INNER-HEIGHT'])) * Mpdf::SCALE, $objattr['ID']));
    ...
    function _putform_bt($form, $hPt)
    {
        ...
        $this->writer->write('/M ' . $this->writer->string('D:' . date('YmdHis')));
        $this->writer->write('/Rect [ ' . $this->_form_rect($form['x'], $form['y'], $form['w'], $form['h'],
                $hPt) . ' ]');
