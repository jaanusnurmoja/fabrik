...
class Otl
{
    ...
    function applyOTL($str, $useOTL)
    {
        ...
        for ($sch = 0; $sch <= $subchunk; $sch++)
        {
            ...
            if (count($ScriptLang))
            {
                ...
                if (
                    $this->mpdf->fontLanguageOverride && strpos($ScriptLang[$GSUBscriptTag],
            ...
            if (
                $GSUBscriptTag && $GSUBlangsys && isset($ScriptLang[$GSUBscriptTag]) && strpos($ScriptLang[$GSUBscriptTag],
                    $GSUBlangsys) !== false
            )
            {
            ...
            elseif (count($ScriptLang))
            {
                ...
                if (
                    $GPOSscriptTag && $this->mpdf->fontLanguageOverride && strpos($ScriptLang[$GPOSscriptTag],
                        $this->mpdf->fontLanguageOverride) !== false
                )
                {
            ...
            {
                $this->ttfOTLdata = $this->GDEFdata[$this->fontkey]['GSUBGPOStables'] = $this->fontCache->load($this->fontkey . '.GSUBGPOStables.dat',
                    'rb');
                if (!$this->ttfOTLdata)
                {
            ...
            if (($useOTL & 0xFF) && $GSUBscriptTag && $GSUBlangsys && $GSUBFeatures)
            {
                ...
                elseif ($this->shaper == 'E')
                {
                    ...
                    Sea::initial_reordering($this->OTLdata, $this->GSUBdata[$this->GSUBfont], $broken_syllables,
                        $scriptblock, $dottedcircle);
                ...
                { // DEFAULT
                    ...
                    if ($scriptblock == Ucdn::SCRIPT_TIBETAN)
                    {
                        ...
                        {
                            if (
                                Indic::in_range($this->OTLdata[$ptr]['uni'], 0x0F20,
                    ...
                    if (
                    !(($this->mpdf->textvar & TextVars::FC_SMALLCAPS) && $scriptblock == Ucdn::SCRIPT_LATIN && strpos($useGSUBtags,
                            'smcp') === false)
                    )
                    {
            ...
            foreach ($LookupList as $lu => $tag)
            {
                ...
                { // whilst there is another glyph ..0064
                    ...
                    foreach ($this->GSUBLookups[$lu]['Subtables'] as $c => $subtable_offset)
                    {
                        ...
                        elseif (
                            static::_OTL_OLD_SPEC_COMPAT_1 && $Type == 4 && !$is_old_spec && strpos('0094D 009CD 00A4D 00ACD 00B4D 00BCD 00C4D 00CCD 00D4D',
                                $currGlyph) !== false
                        )
                        {
    ...
    function _applyGSUBsubtable($lookupID, $subtable, $ptr, $currGlyph, $currGID, $subtable_offset, $Type, $Flag, $MarkFilteringSet, $LuCoverage, $level, $currentTag, $is_old_spec, $tagInt)
    {
                ...
                if ($SubRuleSetOffset[$GlyphPos] > 0)
                {
                    ...
                    {  // EACH RULE
                        ...
                        if ($matched)
                        {
                            if ($this->debugOTL)
                            {
                                $this->_dumpproc('GSUB', $lookupID, $subtable, $Type, $SubstFormat, $ptr, $currGlyph,
                                    $level);
                            }
                            ...
                            for ($p = 0; $p < $SubstCount; $p++)
                            {
                                ...
                                {
                                    $shift = $this->_applyGSUBsubtable($lu, $luc, $luptr, $lucurrGlyph, $lucurrGID,
                                        ($lusubtable_offset - $this->GSUB_offset), $luType, $luFlag,
                                        $luMarkFilteringSet, $this->GSLuCoverage[$lu][$luc], 1, $currentTag,
                                        $is_old_spec, $tagInt);
                                    if ($shift)
                                    {
                                        break;
                ...
                { // $SubClassSet is ordered by input class-may be NULL
                    ...
                    if ($SubClassSetOffset[$s] > 0 && isset($InputClasses[$s][$currGID]))
                    {
                        ...
                        {  // EACH RULE
                            ...
                            $matched = $this->checkContextMatchMultipleUni($inputGlyphs, $backtrackGlyphs,
                                $lookaheadGlyphs, $ignore, $ptr, $class0excl);
                            if ($matched)
                            {
                                if ($this->debugOTL)
                                {
        ...
        elseif ($Type == 6)
        {
                ...
                for ($s = 0; $s < $ChainSubRuleCount; $s++)
                {
                    ...
                    if ($matched)
                    {
                        ...
                        for ($p = 0; $p < $SubstCount; $p++)
                        {
                            ...
                            {
                                $shift = $this->_applyGSUBsubtable($lu, $luc, $luptr, $lucurrGlyph, $lucurrGID,
                                    ($lusubtable_offset - $this->GSUB_offset), $luType, $luFlag, $luMarkFilteringSet,
                                    $this->GSLuCoverage[$lu][$luc], 1, $currentTag, $is_old_spec, $tagInt);
                                if ($shift)
                                {
                                    break;
                                }
                ...
                { // $ChainSubClassSet is ordered by input class-may be NULL
                    ...
                    if ($ChainSubClassSetOffset[$s] > 0 && isset($InputClasses[$s][$currGID]))
                    {
                        ...
                        {  // EACH RULE
                            ...
                            $matched = $this->checkContextMatchMultipleUni($inputGlyphs, $backtrackGlyphs,
                                $lookaheadGlyphs, $ignore, $ptr, $class0excl, $bclass0excl, $lclass0excl);
                            if ($matched)
                            {
                                if ($this->debugOTL)
                                {
            ...
            elseif ($SubstFormat == 3)
            {
                ...
                $matched = $this->checkContextMatchMultiple($CoverageInputGlyphs, $CoverageBacktrackGlyphs,
                    $CoverageLookaheadGlyphs, $ignore, $ptr);
                if ($matched)
                {
                    if ($this->debugOTL)
                    {
    ...
    private function get_arab_glyphs($char, $type, &$chars, $i, $scriptTag, $usetags)
    {
        ...
        if (
            ($type === 0 && strpos($usetags, 'isol') === false) || ($type === 1 && strpos($usetags,
        ...
        if ($scriptTag == 'syrc' && $char == '00710')
        {
            ...
            if (isset($chars[$n]) && isset($this->arabLeftJoining[hexdec($chars[$n])]))
            {
                ...
                if (
                    isset($chars[$i + 1]) && preg_match('/[\x{0700}-\x{0745}]/u',
                        UtfString::code2utf(hexdec($chars[$n]))) && preg_match('/[\x{0700}-\x{0745}]/u',
                        UtfString::code2utf(hexdec($chars[$i + 1]))) && isset($this->arabGlyphs[$char][4])
                )
                {
                    $retk = 4;
                ...
                elseif (
                    !isset($chars[$i + 1]) || !preg_match('/[\x{0700}-\x{0745}]/u',
                        UtfString::code2utf(hexdec($chars[$i + 1])))
                )
                {
...
//echo "DICT_INTERMEDIATE_MATCH: ".dechex($c).'<br />';
                ...
                if (
                    isset($this->OTLdata[$ptr]['uni']) && strpos($this->GlyphClassMarks,
...
//echo "PROBLEM: ".($x).'<br />';
    ...
    private function _applyGPOSsubtable($lookupID, $subtable, $ptr, $currGlyph, $currGID, $subtable_offset, $Type, $Flag, $MarkFilteringSet, $LuCoverage, $tag, $level, $is_old_spec)
    {
        ...
        elseif ($Type == 4)
        {
            ...
            if (
                $this->shaper == 'I' && $is_old_spec && strpos($this->GlyphClassMarks,
                    $this->OTLdata[$ptr]['hex']) === false
            )
            {
                return;
                        ...
                        {  // EACH RULE
                            ...
                            if ($matched)
                            {
                                ...
                                for ($p = 0; $p < $PosCount; $p++)
                                {
                                    ...
                                    {
                                        $shift = $this->_applyGPOSsubtable($lu, $luc, $luptr, $lucurrGlyph, $lucurrGID,
                                            ($lusubtable_offset - $this->GPOS_offset + $this->GSUB_length), $luType,
                                            $luFlag, $luMarkFilteringSet, $this->LuCoverage[$lu][$luc], $tag, 1,
                                            $is_old_spec);
                                        if ($this->debugOTL && $shift)
                                        {
                                            $this->_dumpproc('GPOS', $lookupID, $subtable, $Type, $PosFormat, $ptr,
            ...
            elseif ($PosFormat == 3)
            {
                ...
                if ($matched)
                {
                    ...
                    for ($p = 0; $p < $PosCount; $p++)
                    {
                        ...
                        {
                            $shift = $this->_applyGPOSsubtable($lu, $luc, $luptr, $lucurrGlyph, $lucurrGID,
                                ($lusubtable_offset - $this->GPOS_offset + $this->GSUB_length), $luType, $luFlag,
                                $luMarkFilteringSet, $this->LuCoverage[$lu][$luc], $tag, 1, $is_old_spec);
                            if ($this->debugOTL && $shift)
                            {
                                $this->_dumpproc('GPOS', $lookupID, $subtable, $Type, $PosFormat, $ptr, $currGlyph,
                                    $level);
    ...
    private function checkContextMatchMultiple($Input, $Backtrack, $Lookahead, $ignore, $ptr, $class0excl = '', $bclass0excl = '', $lclass0excl = '')
    {
        ...
        for ($i = 0; $i < count($Backtrack); $i++)
        {
            ...
            elseif (
                !$Backtrack[$i] && isset($this->OTLdata[$checkpos]) && strpos($bclass0excl,
                    $this->OTLdata[$checkpos]['hex']) !== false
            )
            {
                return false;
            ...
            elseif (
                !isset($this->OTLdata[$checkpos]) || strpos($Backtrack[$i], $this->OTLdata[$checkpos]['hex']) === false
            )
            {
                return false;
            }
        ...
        { // Start at 1 - already matched the first InputGlyph
            ...
            elseif (
                !$Input[$i] && isset($this->OTLdata[$checkpos]) && strpos($class0excl,
                    $this->OTLdata[$checkpos]['hex']) === false
            )
            {
                $matched[] = $checkpos;
        ...
        for ($i = 0; $i < count($Lookahead); $i++)
        {
            ...
            elseif (
                !$Lookahead[$i] && isset($this->OTLdata[$checkpos]) && strpos($lclass0excl,
                    $this->OTLdata[$checkpos]['hex']) !== false
            )
            {
                return false;
            ...
            elseif (
                !isset($this->OTLdata[$checkpos]) || strpos($Lookahead[$i], $this->OTLdata[$checkpos]['hex']) === false
            )
            {
                return false;
            }
        ...
        for ($i = 0; $i < $numchars; ++$i)
        {
            ...
            { // PDF
                ...
                if (count($remember))
                {
                    ...
                    if (
                        ($remember[$last]['num'] == 8235) || ($remember[$last]['num'] == 8234) || ($remember[$last]['num'] == 8238) ||
