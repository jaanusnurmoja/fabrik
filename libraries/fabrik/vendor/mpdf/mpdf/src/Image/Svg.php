...
class Svg
{
    ...
    function svgImage($attribs)
    {
        ...
        $w = $this->sizeConverter->convert($wset, $this->svg_info['w'] * (25.4 / $this->mpdf->dpi),
            $this->mpdf->FontSize, false);
        $h = $this->sizeConverter->convert($hset, $this->svg_info['h'] * (25.4 / $this->mpdf->dpi),
            $this->mpdf->FontSize, false);
        if ($w == 0 || $h == 0)
        {
            return;
        ...
        { // need to add a clipping path to reference rectangle
    ...
    function svgGradient($gradient_info, $attribs, $element)
    {
        ...
        for ($i = 0; $i < (count($gradient_info['color'])); $i++)
        {
            ...
            if (
                isset($gradient_info['color'][($i + 1)]['offset']) && stristr($gradient_info['color'][($i + 1)]['offset'],
                    '%') !== false
            )
            {
...
///*
        // mPDF 5.0.005
        if (isset($this->svg_info['w']) && $this->svg_info['w'])
        { // if 'w' set by viewBox
            if ($svg_w)
            { // if width also set, use these values to determine to set size of "pixel"
...
//*/
    ...
    function svgDefineStyle($critere_style)
    {
        ...
        {
            if (preg_match('/fill:\s*rgb\((\d+),\s*(\d+),\s*(\d+)\)/i', $critere_style['style'], $m))
            { // mPDF 5.7.2
...
// If rX = 0 or rY = 0 then this arc is treated as a straight line
    ...
    function svgText()
    {
        ...
        if (isset($this->txt_data[2]))
        {
            ...
            $aixextra = sprintf(' /F%d %.3F Tf %s %s Tr %s %s ', $this->mpdf->CurrentFont['i'], $this->mpdf->FontSizePt,
    ...
    function svgDefineTxtStyle($critere_style)
    {
        ...
        if (isset($critere_style['font-family']) && $critere_style['font-family'] != 'inherit')
        {
            ...
            foreach ($aux_fontlist as $f)
            {
                ...
                if (
                    (!$this->mpdf->usingCoreFont && in_array($v, $this->mpdf->available_unifonts)) ||
                    ($this->mpdf->usingCoreFont && in_array($v, ['courier', 'times', 'helvetica', 'arial'])) ||
                    in_array($v, ['sjis', 'uhc', 'big5', 'gb']) || isset($this->svg_font[$v][$svgfontstyle])
                )
                { // mPDF 6
            ...
            if (!$found)
            {
                foreach ($aux_fontlist as $f)
                {
                    ...
                    if (
                        in_array($v, $this->mpdf->sans_fonts) || in_array($v, $this->mpdf->serif_fonts) || in_array($v,
                            $this->mpdf->mono_fonts) || isset($this->svg_font[$v][$svgfontstyle])
                    )
                    {  // mPDF 6
                        $current_style['font-family'] = $v;
                        break;
    ...
    function ImageSVG($data)
    {
        ...
        if (preg_match('/xlink:href\s*=/si', $data))
        {
            ...
            for ($i = 0; $i < count($links[0]); $i++)
            {
                ...
                {
                    if (
                    preg_match('/<' . $m[1] . '[^>]*id\s*=\s*["\']' . $links[2][$i] . '["\'][^>]*>.*?<\/' . $m[1] . '>/si',
                        $data, $m)
                    )
                    {
                        $insert = $m[0];
            ...
            if ($i % 2 == 0)
            {
                ...
                for ($sch = 0; $sch <= $subchunk; $sch++)
                {
                    if (isset($chardata[$sch]))
                    {
                        ...
                        if (
                            $scriptblocks[$sch] == Ucdn::SCRIPT_LATIN && $this->mpdf->autoVietnamese && preg_match("/([" . $this->scriptToLanguage->getLanguageDelimiters('viet') . "])/u",
                                $s)
                        )
                        {
                            $lang = "vi";
                        ...
                        elseif ($scriptblocks[$sch] == Ucdn::SCRIPT_ARABIC && $this->mpdf->autoArabic)
                        {
                            ...
                            elseif (
                            preg_match("/[" . $this->scriptToLanguage->getLanguageDelimiters('urdu') . "]/u", $s)
                            )
                            {
                                $lang = "ur";
                            }
                            elseif (
                            preg_match("/[" . $this->scriptToLanguage->getLanguageDelimiters('pashto') . "]/u", $s)
                            )
                            {
                                $lang = "ps";
                            }
                            elseif (
                            preg_match("/[" . $this->scriptToLanguage->getLanguageDelimiters('persian') . "]/u", $s)
                            )
                            {
                                $lang = "fa";
                            }
    ...
    function xml_svg2pdf_start($parser, $name, $attribs)
    {
        ...
        elseif (strtolower($name) == 'stop')
        {
            ...
            { // CMYK
                $color_final = sprintf('%.3F %.3F %.3F %.3F', ord($col{1}) / 100, ord($col{2}) / 100,
                    ord($col{3}) / 100, ord($col{4}) / 100);
            ...
            case 'text':
                ...
                if ($this->mpdf->svgAutoFont && isset($attribs['lang']) && $attribs['lang'])
                {
                    if (!$this->mpdf->usingCoreFont)
                    {
                        ...
                        {
                            list ($coreSuitable, $mpdf_unifont) = $this->languageToFont->getLanguageOptions($attribs['lang'],
            ...
            case 'tspan':
                ...
                if (isset($attribs['x']) || isset($attribs['y']))
                {
                    ...
                    while (preg_match('/mPDF-AXS\((.*?)\)/', $this->textoutput, $m))
                    {
                        ...
                        {
                            $txk = $m[1] + ($tx * $this->kp);
                            $this->textoutput = preg_replace('/mPDF-AXS\((.*?)\)/', sprintf('%.4F', $txk),
                                $this->textoutput, 1);
                        }
                        else
                        {
