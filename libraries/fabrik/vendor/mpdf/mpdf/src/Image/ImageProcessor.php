    ...
    public function getImage(&$file, $firsttime = true, $allowvector = true, $orig_srcpath = false, $interpolation = false)
    {
        ...
        if (empty($data))
        {
            ...
            if ($orig_srcpath && $this->mpdf->basepathIsLocal && $check = @fopen($orig_srcpath, 'rb'))
            {
                fclose($check);
                $file = $orig_srcpath;
                $this->logger->debug(sprintf('Fetching (file_get_contents) content of file "%s" with local basepath',
                    $file), ['context' => LogContext::REMOTE_CONTENT]);
            ...
            {
                fclose($check);
                $this->logger->debug(sprintf('Fetching (file_get_contents) content of file "%s" with non-local basepath',
                    $file), ['context' => LogContext::REMOTE_CONTENT]);
                $data = file_get_contents($file);
                $type = $this->guesser->guess($data);
            }
        ...
        if ($type === 'svg')
        {
            $svg = new Svg($this->mpdf, $this->otl, $this->cssManager, $this, $this->sizeConverter,
                $this->colorConverter, $this->languageToFont, $this->scriptToLanguage);
        ...
        if ($type === 'jpeg' || $type === 'jpg')
        {
            ...
            if ($j)
            {
                ...
                if ($unitSp > 0)
                {
                    $ppUx = $this->twoBytesToInt(substr($data, $j + 8,
            ...
            if ($a[2] === 'DeviceCMYK' && ($this->mpdf->restrictColorSpace === 2 || ($this->mpdf->PDFA && $this->mpdf->restrictColorSpace !== 3)))
            {
                ...
                {
                    $this->mpdf->PDFAXwarnings[] = sprintf('JPG image may not use CMYK color space - %s - (Image converted to RGB. NB This will alter the colour profile of the image.)',
                        $file);
                }
                ...
                if ($im)
                {
                    ...
                    {
                        return $this->imageError($file, $firsttime,
                            'Error creating temporary file (' . $tempfile . ') whilst using GD library to parse JPG(CMYK) image');
                    }
                    $info = $this->getImage($tempfile, false);
                    if (!$info)
                    {
                        return $this->imageError($file, $firsttime,
                            'Error parsing temporary file (' . $tempfile . ') created with GD library to parse JPG(CMYK) image');
                    }
                    imagedestroy($im);
                    unlink($tempfile);
                    $info['type'] = 'jpg';
            ...
            if ($a[2] === 'DeviceRGB' && ($this->mpdf->PDFX || $this->mpdf->restrictColorSpace === 3))
            {
                ...
                {
                    $this->mpdf->PDFAXwarnings[] = sprintf('JPG image may not use RGB color space - %s - (Image converted to CMYK. NB This will alter the colour profile of the image.)',
                        $file);
                }
        ...
        if ($type === 'png')
        {
            ...
            if ($j)
            {
                ...
                if ($unitSp === 1)
                {
                    $ppUx = $this->fourBytesToInt(substr($data, $j + 4,
            ...
            if ($firsttime && ($colspace === 'DeviceRGB' || $colspace === 'Indexed') && ($this->mpdf->PDFX || $this->mpdf->restrictColorSpace === 3))
            {
                ...
                // Convert to CMYK image stream - nominally returned as type='png'
                $info = $this->convertImage($data, $colspace, 'DeviceCMYK', $w, $h, $ppUx, $pngalpha, $gamma,
                    $ct); // mPDF 5.7.2 Gamma correction
                if (($this->mpdf->PDFA && !$this->mpdf->PDFAauto) || ($this->mpdf->PDFX && !$this->mpdf->PDFXauto))
                {
                    $this->mpdf->PDFAXwarnings[] = sprintf('PNG image may not use RGB color space - %s - (Image converted to CMYK. NB This will alter the colour profile of the image.)',
                        $file);
                }
            ...
            elseif ($firsttime && ($colspace === 'DeviceRGB' || $colspace === 'Indexed') && $this->mpdf->restrictColorSpace === 1)
            {
                ...
                // the alpha channel is sent through as secondtime as Indexed and should not be converted to CMYK
                // Convert to Grayscale image stream - nominally returned as type='png'
                $info = $this->convertImage($data, $colspace, 'DeviceGray', $w, $h, $ppUx, $pngalpha, $gamma,
                    $ct); // mPDF 5.7.2 Gamma correction
            ...
            elseif (($this->mpdf->PDFA || $this->mpdf->PDFX) && $pngalpha)
            {
                ...
                { // RGB
                    $info = $this->convertImage($data, $colspace, 'DeviceRGB', $w, $h, $ppUx, $pngalpha, $gamma,
                        $ct); // mPDF 5.7.2 Gamma correction
                }
                if (($this->mpdf->PDFA && !$this->mpdf->PDFAauto) || ($this->mpdf->PDFX && !$this->mpdf->PDFXauto))
                {
                    $this->mpdf->PDFAXwarnings[] = sprintf('Transparency (alpha channel) not permitted in PDFA or PDFX files - %s - (Image converted to one without transparency.)',
                        $file);
                }
            ...
            { // mPDF 5.7.2 Gamma correction
                ...
                {
                    return $this->imageError($file, $firsttime,
                        sprintf('GD library with PNG support required for image (%s)', $errpng));
                }
                ...
                {
                    return $this->imageError($file, $firsttime,
                        sprintf('Error creating GD image from PNG file (%s)', $errpng));
                }
                ...
                if ($pngalpha)
                {
                    if ($this->mpdf->PDFA)
                    {
                        throw new \Mpdf\MpdfException(sprintf('PDFA1-b does not permit images with alpha channel transparency (%s).',
                            $file));
                    }
                    ...
                    $tempfile_alpha = $this->cache->tempFilename('_tempMskPNG' . md5($file) . random_int(1,
                            10000) . '.png');
                    ...
                    {
                        return $this->imageError($file, $firsttime,
                            'Failed to create temporary image file (' . $tempfile_alpha . ') parsing PNG image with alpha channel (' . $errpng . ')');
                    }
                    ...
                    {
                        return $this->imageError($file, $firsttime,
                            'Failed to create temporary image file (' . $tempfile . ') parsing PNG image with alpha channel (' . $errpng . ')');
                    }
                    ...
                    {
                        return $this->imageError($file, $firsttime,
                            'Error parsing temporary file (' . $tempfile_alpha . ') created with GD library to parse PNG image');
                    }
                    ...
                    {
                        return $this->imageError($file, $firsttime,
                            'Error parsing temporary file (' . $tempfile . ') created with GD library to parse PNG image');
                    }
                ...
                {
                    return $this->imageError($file, $firsttime,
                        'Failed to create temporary image file (' . $tempfile . ') parsing PNG image (' . $errpng . ')');
                }
                imagedestroy($im);
                $info = $this->getImage($tempfile, false);
                unlink($tempfile);
            ...
            if (isset($gd['GIF Read Support']) && $gd['GIF Read Support'])
            {
                ...
                if ($im)
                {
                    ...
                    else
                    {
                        ...
                        {
                            return $this->imageError($file, $firsttime,
                                'Error creating temporary file (' . $tempfile . ') whilst using GD library to parse GIF image');
                        }
                        $info = $this->getImage($tempfile, false);
                        if (!$info)
                        {
                            return $this->imageError($file, $firsttime,
                                'Error parsing temporary file (' . $tempfile . ') created with GD library to parse GIF image');
                        }
                        imagedestroy($im);
                        unlink($tempfile);
                    }
        ...
        { // UNKNOWN TYPE - try GD imagecreatefromstring
            ...
            if (isset($gd['PNG Support']) && $gd['PNG Support'])
            {
                ...
                {
                    return $this->imageError($file, $firsttime,
                        'Error creating temporary file (' . $tempfile . ') whilst using GD library to parse unknown image type');
                }
                ...
                {
                    return $this->imageError($file, $firsttime,
                        'Error parsing temporary file (' . $tempfile . ') created with GD library to parse unknown image type');
                }
