...
class Hr extends Tag
{
    ...
    public function open($attr, &$ahtml, &$ihtml)
    {
        ...
        {
            $objattr['width'] = $this->sizeConverter->convert($properties['WIDTH'],
                $this->mpdf->blk[$this->mpdf->blklvl]['inner_width']);
        }
        elseif (isset($attr['WIDTH']) && $attr['WIDTH'] != '')
        {
            $objattr['width'] = $this->sizeConverter->convert($attr['WIDTH'],
                $this->mpdf->blk[$this->mpdf->blklvl]['inner_width']);
        }
        ...
        if (isset($properties['MARGIN-RIGHT']) && strtolower($properties['MARGIN-RIGHT']) === 'auto')
        {
            $objattr['align'] = 'L';
            if (
                isset($properties['MARGIN-RIGHT']) && strtolower($properties['MARGIN-RIGHT']) === 'auto'
