...
abstract class BlockTag extends Tag
{
    ...
    public function open($attr, &$ahtml, &$ihtml)
    {
        ...
        if ($this->mpdf->tableLevel)
        {
            ...
            if ($tag === 'LI')
            {
                ...
                switch ($this->mpdf->listtype[$this->mpdf->listlvl])
                {
                    ...
                    case 'lower-latin':
                    case 'a':
                        $blt = $decToAlpha->convert($this->mpdf->listcounter[$this->mpdf->listlvl],
                                false) . $this->mpdf->list_number_suffix;
                        break;
                    case 'upper-roman':
                    case 'I':
                    ...
                    default:
                        $blt = '-';
                        if (
                            $this->mpdf->listlvl % 3 == 1 && $this->mpdf->_charDefined($this->mpdf->CurrentFont['cw'],
                                8226)
                        )
                        {
                            $blt = "\xe2\x80\xa2";
                        ...
                        elseif (
                            $this->mpdf->listlvl % 3 == 2 && $this->mpdf->_charDefined($this->mpdf->CurrentFont['cw'],
                                9900)
                        )
                        {
                            $blt = "\xe2\x9a\xac";
                        ...
                        elseif (
                            $this->mpdf->listlvl % 3 == 0 && $this->mpdf->_charDefined($this->mpdf->CurrentFont['cw'],
                                9642)
                        )
                        {
                            $blt = "\xe2\x96\xaa";
        ...
        if (
            !$this->mpdf->tableLevel && (($pagesel && (!isset($this->mpdf->page_box['current'])
        ...
        if (isset($properties['FLOAT']) && strtoupper($properties['FLOAT']) === 'RIGHT' && !$this->mpdf->ColActive)
        {
            ...
            if (
                ($setwidth + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) > $maxw
                || ($maxw - ($currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr)) < (2 * $this->mpdf->GetCharWidth('W',
                        false))
            )
            {
        ...
        elseif (isset($properties['FLOAT']) && strtoupper($properties['FLOAT']) === 'LEFT' && !$this->mpdf->ColActive)
        {
            ...
            $doubleCharWidth = (2 * $this->mpdf->GetCharWidth('W', false));
            if (
                ($setwidth + $currblk['margin_left'] + $bdl + $pdl + $bdr + $pdr) > $maxw
                || ($maxw - ($currblk['margin_left'] + $bdl + $pdl + $bdr + $pdr)) < (2 * $this->mpdf->GetCharWidth('W',
                        false))
            )
            {
                // Too narrow to fit - try to move down past L or R float
        ...
        else
        {
            ...
            $doubleCharWidth = (2 * $this->mpdf->GetCharWidth('W', false));
            if (
                ($setwidth + $currblk['margin_left'] + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) > $maxw
        ...
        if ($tag === 'LI')
        {
            ...
            // Listitem-type
            $this->mpdf->_setListMarker($currblk['list_style_type'], $currblk['list_style_image'],
                $currblk['list_style_position']);
        }
    ...
    public function close(&$ahtml, &$ihtml)
    {
        ...
        if (count($this->mpdf->textbuffer) && $this->mpdf->textbuffer[count($this->mpdf->textbuffer) - 1])
        {
            ...
            { // not special content
                ...
                if (preg_match('/[ ]+$/', $this->mpdf->textbuffer[count($this->mpdf->textbuffer) - 1][0], $m))
                {
                    ...
                    {
                        $this->otl->trimOTLdata($this->mpdf->textbuffer[count($this->mpdf->textbuffer) - 1][18],
                            false); // mPDF 6  ZZZ99K
                    }
                    /* -- END OTL -- */
                }
        ...
        if ($this->mpdf->blk[$this->mpdf->blklvl]['float'] === 'R')
        {
            ...
            for ($i = ($this->mpdf->blklvl - 1); $i >= 0; $i--)
            {
                ...
                {
                    $this->mpdf->blk[$i]['float_endpos'] = max($this->mpdf->blk[$i]['float_endpos'],
                        $this->mpdf->page * 1000 + $this->mpdf->y);
                }
                else
                {
