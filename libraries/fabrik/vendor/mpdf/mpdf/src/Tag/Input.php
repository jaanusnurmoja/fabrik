...
class Input extends Tag
{
    ...
    public function open($attr, &$ahtml, &$ihtml)
    {
        ...
        if ($this->mpdf->useActiveForms)
        {
            ...
            {
                $objattr['border-col'] = $this->colorConverter->convert($properties['BORDER-TOP-COLOR'],
                    $this->mpdf->PDFAXwarnings);
            }
            if (isset($properties['BACKGROUND-COLOR']))
            {
                $objattr['background-col'] = $this->colorConverter->convert($properties['BACKGROUND-COLOR'],
                    $this->mpdf->PDFAXwarnings);
            }
        }
        ...
        {
            $w = $this->sizeConverter->convert($properties['WIDTH'],
        ...
        switch (strtoupper($attr['TYPE']))
        {
            case 'HIDDEN':
                ...
                if ($this->mpdf->useActiveForms)
                {
                    $this->form->SetFormText(0, 0, $objattr['fieldname'], $objattr['value'], $objattr['value'], '', 0,
                        '', true);
                }
            ...
            case 'IMAGE': // Draw an Image button
                if (isset($attr['SRC']))
                {
                    ...
                    {
                        $h = $this->sizeConverter->convert($properties['HEIGHT'],
                            $this->mpdf->blk[$this->mpdf->blklvl]['inner_width']);
                    }
                    ...
                    if ($w == 0 && $h == 0)
                    {
                        ...
                        {
                            if (
                                preg_match('/from-image/i',
                                    $properties['IMAGE-RESOLUTION']) && isset($info['set-dpi']) && $info['set-dpi'] > 0
                            )
                            {
