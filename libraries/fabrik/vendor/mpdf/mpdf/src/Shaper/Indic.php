...
class Indic
{
    ...
    public static function set_syllables(&$o, $s, &$broken_syllables)
    {
        ...
        while ($ptr < strlen($s))
        {
            ...
            // From OT spec:
            if (
            preg_match('/^([CR]m*[N]?(H[ZJ]?|[ZJ]H))*[CR]m*[N]?[A]?(H[ZJ]?|[M]*[N]?[H]?)?[S]?[v]{0,2}/',
                substr($s, $ptr), $ma)
            )
            {
                // From HarfBuzz:
            ...
            // From OT spec:
            elseif (
            preg_match('/^(RH|r)?V[N]?([ZJ]?H[CR]m*|J[CR]m*)?([M]*[N]?[H]?)?[S]?[v]{0,2}/', substr($s, $ptr), $ma)
            )
            {
                // From HarfBuzz:
    ...
    public static function set_syllables_khmer(&$o, $s, &$broken_syllables)
    {
        ...
        while ($ptr < strlen($s))
        {
            ...
            // CONSONANT_SYLLABLE Consonant syllable
            if (
            preg_match('/^r?([CR]J?((Z?F)?[N]{0,2})?[ZJ]?G(JN?)?){0,4}[CR]J?((Z?F)?[N]{0,2})?A?((([ZJ]?G(JN?)?)|GZ)|(GJ)?([ZJ]{0,3}MN?(H|JHJR)?){0,4})?(G([CR]J?((Z?F)?[N]{0,2})?|V))?(SZ?)?[v]{0,2}/',
                substr($s, $ptr), $ma)
            )
            {
                $syllable_length = strlen($ma[0]);
            ...
            elseif (
            preg_match('/^(RH|r)?V((Z?F)?[N]{0,2})?(J|([ZJ]?G(JN?)?[CR]J?((Z?F)?[N]{0,2})?){0,4}((([ZJ]?G(JN?)?)|GZ)|(GJ)?([ZJ]{0,3}MN?(H|JHJR)?){0,4})?(G([CR]J?((Z?F)?[N]{0,2})?|V))?(SZ?)?[v]{0,2})/',
                substr($s, $ptr), $ma)
            )
            {
                $syllable_length = strlen($ma[0]);
            ...
            elseif (
            preg_match('/^(RH|r)?((Z?F)?[N]{0,2})?(([ZJ]?G(JN?)?)[CR]J?((Z?F)?[N]{0,2})?){0,4}((([ZJ]?G(JN?)?)|GZ)|(GJ)?([ZJ]{0,3}MN?(H|JHJR)?){0,4})(G([CR]J?((Z?F)?[N]{0,2})?|V))?(SZ?)?[v]{0,2}/',
                substr($s, $ptr), $ma)
            )
            {
                if (strlen($ma[0]))
    ...
    public static function initial_reordering_syllable(&$info, $GSUBdata, $indic_config, $scriptblock, $is_old_spec, $start, $end)
    {
        ...
        if ($scriptblock != Ucdn::SCRIPT_KHMER)
        {
             ...
             * 	base consonants. */
            if (
                count($GSUBdata['rphf']) /* ?? $indic_plan->mask_array[RPHF] */ && $start + 3 <= $end &&
                (
                    ...
                    for ($i = $start; $i < ($base - 2); $i++)
                    {
                        if (self::is_consonant($info[$i]))
                        {
                            if (self::is_halant_or_coeng($info[$i + 1]) && self::is_ra($info[$i + 2]['uni']))
                            {
                                ...
                                elseif (self::is_halant_or_coeng($info[$i + 3]))
                                {
        ...
        if ($is_old_spec && $scriptblock == Ucdn::SCRIPT_DEVANAGARI)
        {
            ...
            {
                if (
                    $info[$i]['indic_category'] == self::OT_RA && $info[$i + 1]['indic_category'] == self::OT_H &&
        ...
        if ($scriptblock != Ucdn::SCRIPT_KHMER)
        {
            if (count($GSUBdata['pref']) && $base + 2 < $end)
            {
                ...
                for ($i = $base + 1; $i + 1 < $end; $i++)
                {
                    ...
                    if (
                        (isset($GSUBdata['pref'][$info[$i + 1]['uni']]) && self::is_halant_or_coeng($info[$i]) && self::is_ra($info[$i + 1]['uni'])) ||
    ...
    public static function final_reordering_syllable(&$info, $GSUBdata, $indic_config, $scriptblock, $is_old_spec, $start, $end)
    {
        ...
        if (
            $info[$start]['indic_position'] == self::POS_PRE_M &&
            ($start == 0 ||
