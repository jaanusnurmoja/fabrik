...
class Myanmar
{
    ...
    public static function set_syllables(&$o, $s, &$broken_syllables)
    {
        ...
        while ($ptr < strlen($s))
        {
            ...
            // From OT spec:
            if (
            preg_match('/^(RaH)?([C|R]|V|d|D)[s]?(H([C|R|V])[s]?)*(H|[a]*[n]?[l]?((m[k]?|k)[a]?)?[e]*[v]*[b]*[A]*(N[a]?)?(t[k]?[a]*[v]*[A]*(N[a]?)?)*(p[A]*(N[a]?)?)*S*[J|Z]?)/',
                substr($s, $ptr), $ma)
            )
            {
            ...
            elseif (
            preg_match('/^(RaH)?s?(H|[a]*[n]?[l]?((m[k]?|k)[a]?)?[e]*[v]*[b]*[A]*(N[a]?)?(t[k]?[a]*[v]*[A]*(N[a]?)?)*(p[A]*(N[a]?)?)*S*[J|Z]?)/',
                substr($s, $ptr), $ma)
            )
            {
                if (strlen($ma[0]))
    ...
    public static function reordering_syllable(&$info, $GSUBdata, $start, $end)
    {
        ...
        for (; $i < $end; $i++)
        {
            if ($info[$i]['myanmar_category'] == self::OT_MR)
            { /* Pre-base reordering */
                $info[$i]['myanmar_position'] = self::POS_PRE_C;
                continue;
            }
            if ($info[$i]['myanmar_position'] < self::POS_BASE_C)
            { /* Left matra */
                continue;
            }
