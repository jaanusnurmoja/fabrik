...
class TTFontFile
{
    ...
    function checksumTables()
    {
        ...
        foreach ($this->tables as $t)
        {
            ...
            { // 1.02
                ...
                {
                    throw new \Mpdf\MpdfException(sprintf('TTF file "%s": invalid checksum %s table: %s (expected %s)',
                        $this->filename, dechex($checksum[0]) . dechex($checksum[1]), $t['tag'],
    ...
    function extractInfo($debug = false, $BMPonly = false, $useOTL = 0)
    {
        ...
        { // if (_FONT_DESCRIPTOR == 'win') {	// default
                ...
                for ($c = 0; $c < $Lookup[$i]['SubtableCount']; $c++)
                {
                    ...
                    if ($Lookup[$i]['Type'] == 1)
                    {
                        ...
                        for ($g = 0; $g < count($glyphs); $g++)
                        {
                            ...
                            // Flag = Ignore
                            if (
                            $this->_checkGSUBignore($Lookup[$i]['Flag'], $replace[0], $Lookup[$i]['MarkFilteringSet'])
                            )
                            {
                                continue;
                            }
                    ...
                    elseif ($Lookup[$i]['Type'] == 4)
                    {
                        ...
                        for ($s = 0; $s < $LigSetCount; $s++)
                        {
                            for ($g = 0; $g < $Lookup[$i]['Subtable'][$c]['LigSet'][$s]['LigCount']; $g++)
                            {
                                ...
                                // Flag = Ignore
                                if (
                                $this->_checkGSUBignore($Lookup[$i]['Flag'], $replace[0],
                                    $Lookup[$i]['MarkFilteringSet'])
                                )
                                {
                                    continue;
                                ...
                                for ($l = 1; $l < $Lookup[$i]['Subtable'][$c]['LigSet'][$s]['Ligature'][$g]['CompCount']; $l++)
                                {
                                    ...
                                    // Flag = Ignore
                                    if (
                                    $this->_checkGSUBignore($Lookup[$i]['Flag'], $rpl, $Lookup[$i]['MarkFilteringSet'])
                                    )
                                    {
                                        continue 2;
                                    }
                            ...
                            else
                            {
                                if (isset($v['context']) && $v['context'])
                                {
                                    foreach ($v['rules'] as $vs)
                                    {
                                        ...
                                        {
                                            if (
                                                isset($vs['replace'][$i]) && preg_match('/^0[A-F0-9]{4}$/',
                                                    $vs['match'][$i])
                                            )
                                            {
...
//print_r($rtl); exit;
                        ...
                        foreach ($rtl as $base => $variants)
                        {
                            ...
                            { // i.e. final form
                                if (
                                    strpos('0FE8E 0FE94 0FEA2 0FEAA 0FEAE 0FEC2 0FEDA 0FEDE 0FB93 0FECA 0FED2 0FED6 0FEEE 0FEF0 0FEF2',
                                        $variants[1]) === false
                                )
...
//print_r($rtlpua); exit;
                    ...
                    $pstf = [];
                    if (
                        strpos('dev2 bng2 gur2 gjr2 ory2 tml2 tel2 knd2 mlm2 deva beng guru gujr orya taml telu knda mlym',
                            $st) !== false
                    )
                                ...
                                if (isset($v['context']) && $v['context'] && $v['nBacktrack'] == 0 && $v['nLookahead'] == 0)
                                {
                                    foreach ($v['rules'] as $vs)
                                    {
                                        if (count($vs['match']) == 2 && count($vs['replace']) == 1)
                                        {
                                            ...
                                            if (
                                                strpos('0094D 009CD 00A4D 00ACD 00B4D 00BCD 00C4D 00CCD 00D4D',
            ...
            for ($c = 0; $c < $Lookup[$i]['SubtableCount']; $c++)
            {
                ...
                elseif ($Lookup[$i]['Type'] == 4)
                {
                    for ($s = 0; $s < count($Lookup[$i]['Subtable'][$c]['subs']); $s++)
                    {
                        ...
                        $subs = $this->_makeGSUBinputReplacement(count($inputGlyphs), $substitute, $ignore, 0,
                            count($inputGlyphs), 0);
                                    ...
                                    for ($lus = 0; $lus < $Lookup[$lup]['SubtableCount']; $lus++)
                                    {
                                        if (count($Lookup[$lup]['Subtable'][$lus]['subs']))
                                        {
                                            foreach ($Lookup[$lup]['Subtable'][$lus]['subs'] as $luss)
                                            {
                                                ...
                                                if (
                                                    strpos("isol fina fin2 fin3 medi med2 init ",
                            ...
                            for ($lus = 0; $lus < $Lookup[$lup]['SubtableCount']; $lus++)
                            {
                                if (count($Lookup[$lup]['Subtable'][$lus]['subs']))
                                {
                                    foreach ($Lookup[$lup]['Subtable'][$lus]['subs'] as $luss)
                                    {
                                        ...
                                        // Returns e.g. ¦(0612)¦(ignore) (0613)¦(ignore) (0614)¦
                                        $contextInputMatch = $this->_makeGSUBcontextInputMatch($inputGlyphs, $ignore,
                                            $lookupGlyphs, $seqIndex);
                                        $REPL = implode(" ", $luss['substitute']);
