...
class CssManager
{
    ...
    function ReadCSS($html)
    {
        ...
        {
            if (
                $this->mpdf->CSSselectMedia && !preg_match('/(' . trim($this->mpdf->CSSselectMedia) . '|all)/i',
                    $m[1][$i])
            )
            {
                ...
                if ($xem)
                {
                    ...
                    for ($i = 0; $i < $count_cxtem; $i++)
                    {
                        ...
                        { // mPDF 5.5.13
                            ...
                            $CSSextblock = str_replace($cxtem[0][$i], ($cxtem[1][$i] . $embedded . $cxtem[3][$i]),
                                $CSSextblock);
                        }
                    }
                }
            ...
            if ($xem)
            {
                ...
                for ($i = 0; $i < $count_cxtem; $i++)
                {
                    ...
                    { // mPDF 5.5.13
                        ...
                        $tmpCSSstr = str_replace($cxtem[0][$i], ($cxtem[1][$i] . $embedded . $cxtem[3][$i]),
                            $tmpCSSstr);
                    }
                }
            }
        ...
        if ($count_idata)
        {
            ...
            {
                $file = $this->cache->write('_tempCSSidata' . random_int(1, 10000) . '_' . $i . '.' . $idata[2][$i],
                    base64_decode($idata[3][$i]));
        ...
        if ($CSSstr)
        {
            ...
            for ($i = 0; $i < $styles_count; $i++)
            {
                ...
                foreach ($stylearr as $sta)
                {
                    if (trim($sta))
                    {
                        ...
                        if ($property && ($value || $value === '0'))
                        {
                            ...
                            if (
                                (strtoupper($property) === 'BACKGROUND-IMAGE' || strtoupper($property) === 'BACKGROUND') && false !== stripos($value,
                                    '-webkit-gradient')
                            )
                            {
                                continue;
                ...
                foreach ($tagarr as $tg)
                {
                    ...
                    if (
                    preg_match('/NTH-CHILD\((\s*(([\-+]?\d*)N(\s*[\-+]\s*\d+)?|[\-+]?\d+|ODD|EVEN)\s*)\)/', $tg, $m)
                    )
                    {
                    ...
                    {  // e.g. p or .class or #id or p.class or p#id
                        ...
                        if ($t)
                        {
                            ...
                            elseif (
                            preg_match('/^(' . $this->mpdf->allowedCSStags . ')\s*:NTH-CHILD\((.*)\)$/', $t, $m)
                            )
                            {
                            ...
                            elseif (
                            preg_match('/^(' . $this->mpdf->allowedCSStags . ')\[LANG=[\'\"]{0,1}([A-Z\-]{2,11})[\'\"]{0,1}\]$/',
                                $t, $m)
                            )
                            {
    ...
    function readInlineCSS($html)
    {
        ...
        for ($i = 0; $i < $properties_count; $i++)
        {
            ...
            // Ignores -webkit-gradient so doesn't override -moz-
            if (
                (strtoupper($properties[$i]) === 'BACKGROUND-IMAGE' || strtoupper($properties[$i]) === 'BACKGROUND') && false !== stripos($values[$i],
                    '-webkit-gradient')
            )
            {
                continue;
    ...
    function fixCSS($prop)
    {
        ...
        foreach ($prop as $k => $v)
        {
            ...
            elseif ($k === 'FONT-FAMILY')
            {
                ...
                if (!$found)
                {
                    foreach ($aux_fontlist as $f)
                    {
                        ...
                        if (
                            in_array($v, $this->mpdf->sans_fonts) || in_array($v,
            ...
            elseif ($k === 'FONT-VARIANT')
            {
                ...
                {
                    if (
                    preg_match_all('/(no-common-ligatures|\bcommon-ligatures|no-discretionary-ligatures|\bdiscretionary-ligatures|no-historical-ligatures|\bhistorical-ligatures|no-contextual|\bcontextual)/i',
                        $v, $m)
                    )
                    {
                    ...
                    if (
                    preg_match('/(all-small-caps|\bsmall-caps|all-petite-caps|\bpetite-caps|unicase|titling-caps)/i',
                        $v, $m)
                    )
                    {
                        $newprop['FONT-VARIANT-CAPS'] = $m[1];
                    ...
                    if (
                    preg_match_all('/(lining-nums|oldstyle-nums|proportional-nums|tabular-nums|diagonal-fractions|stacked-fractions)/i',
                        $v, $m)
                    )
                    {
            ...
            elseif ($k === 'BACKGROUND-POSITION')
            {
                ...
                if (isset($bg['p']))
                {
                    ...
                    if (
                    !preg_match('/[\-]{0,1}\d+(in|cm|mm|pt|pc|em|ex|px|%)* [\-]{0,1}\d+(in|cm|mm|pt|pc|em|ex|px|%)*/',
                        $bg['p'])
                    )
                    {
                        $bg['p'] = false;
            ...
            elseif ($k === 'LIST-STYLE')
            {
                ...
                if (
                preg_match('/(lower-roman|upper-roman|lower-latin|lower-alpha|upper-latin|upper-alpha|decimal|disc|circle|square|arabic-indic|bengali|devanagari|gujarati|gurmukhi|kannada|malayalam|oriya|persian|tamil|telugu|thai|urdu|cambodian|khmer|lao|cjk-decimal|hebrew)/i',
                    $v, $m)
                )
                {
    ...
    function setCSSboxshadow($v)
    {
        ...
        foreach ($ss as $s)
        {
            ...
            if (isset($p[0]))
            {
                $new['x'] = $this->sizeConverter->convert(trim($p[0]),
                    $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
            }
            if (isset($p[1]))
            {
                $new['y'] = $this->sizeConverter->convert(trim($p[1]),
                    $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
            }
            if (isset($p[2]))
            {
                ...
                {
                    $new['blur'] = $this->sizeConverter->convert(trim($p[2]),
                        $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
                }
                else
                {
                    $new['col'] = $this->colorConverter->convert(preg_replace('/\*/', ',', $p[2]),
                ...
                if (isset($p[3]))
                {
                    ...
                    {
                        $new['spread'] = $this->sizeConverter->convert(trim($p[3]),
                            $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
                    }
                    else
                    {
                        $new['col'] = $this->colorConverter->convert(preg_replace('/\*/', ',', $p[3]),
                            $this->mpdf->PDFAXwarnings);
                    }
                    if (isset($p[4]))
                    {
                        $new['col'] = $this->colorConverter->convert(preg_replace('/\*/', ',', $p[4]),
                            $this->mpdf->PDFAXwarnings);
                    }
                }
            }
    ...
    function setCSStextshadow($v)
    {
        ...
        foreach ($ss as $s)
        {
            ...
            if (isset($p[0]))
            {
                $new['x'] = $this->sizeConverter->convert(trim($p[0]), $this->mpdf->FontSize, $this->mpdf->FontSize,
                    false);
            }
            if (isset($p[1]))
            {
                $new['y'] = $this->sizeConverter->convert(trim($p[1]), $this->mpdf->FontSize, $this->mpdf->FontSize,
                    false);
            }
            if (isset($p[2]))
            {
    ...
    function parseCSSbackground($s)
    {
        ...
        else
        {
            ...
            { /* -- END BACKGROUNDS -- */
                ...
                // mPDF 5.6.05
                if (
                preg_match('/^\s*(#[0-9a-fA-F]{3,6}|(rgba|rgb|device-cmyka|cmyka|device-cmyk|cmyk|hsla|hsl|spot)\(.*?\)|[a-zA-Z]{3,})\s+(url\(.*)/i',
                    $s, $m)
                )
                {
                    $bg['c'] = strtolower($m[1]);
            ...
            elseif (
            preg_match('/^\s*(#[0-9a-fA-F]{3,6}|(rgba|rgb|device-cmyka|cmyka|device-cmyk|cmyk|hsla|hsl|spot)\(.*?\)|[a-zA-Z]{3,})/i',
                $s, $m)
            )
            {
                $bg['c'] = strtolower($m[1]);
    ...
    function MergeCSS($inherit, $tag, $attr)
    {
        ...
        { // NB looks at $this->tablecascadeCSS-1 for cascading CSS
            ...
            { // mPDF 6
                ...
                foreach ($classes as $class)
                {
                    $this->_set_mergedCSS($this->tablecascadeCSS[$this->tbCSSlvl - 1]['CLASS>>' . $class], $p, false,
                        9);
                }
                // STYLESHEET nth-child SELECTOR e.g. tr:nth-child(odd)  td:nth-child(2n+1)
                if ($tag === 'TR' || $tag === 'TD' || $tag === 'TH')
            ...
            foreach ($classes as $class)
            {
                $this->_set_mergedCSS($this->tablecascadeCSS[$this->tbCSSlvl - 1][$tag . '>>CLASS>>' . $class], $p,
                    false, 9);
            }
            $this->_set_mergedCSS($this->tablecascadeCSS[$this->tbCSSlvl - 1][$tag . '>>ID>>' . $attr['ID']], $p, false,
                9);
        }
