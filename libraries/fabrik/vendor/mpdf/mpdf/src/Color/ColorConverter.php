...
class ColorConverter
{
    ...
    public function lighten($c)
    {
        ...
        {
            $ret = [
                4, max(0, ord($c{1}) - 20), max(0, ord($c{2}) - 20), max(0, ord($c{3}) - 20), max(0, ord($c{4}) - 20)
            ];
        }
    ...
    public function darken($c)
    {
        ...
        {
            $ret = [
                4, min(100, ord($c{1}) + 20), min(100, ord($c{2}) + 20), min(100, ord($c{3}) + 20), min(100,
                    ord($c{4}) + 20)
            ];
        }
        elseif ($c{0} == static::MODE_GRAYSCALE)
    ...
    public function colAtoString($c)
    {
        ...
        {
            return 'rgba(' . ord($c{1}) . ', ' . ord($c{2}) . ', ' . ord($c{3}) . ', ' . sprintf('%0.2F',
                    ord($c{4}) / 100) . ')';
        }

        if ($c{0} == static::MODE_CMYKA)
        {
            return 'cmyka(' . ord($c{1}) . ', ' . ord($c{2}) . ', ' . ord($c{3}) . ', ' . ord($c{4}) . ', ' . sprintf('%0.2F',
                    ord($c{5}) / 100) . ')';
        }
    ...
    private function ensureBinaryColorFormat($color)
    {
        ...
        if (
        !in_array($color[0],
            [static::MODE_GRAYSCALE, static::MODE_SPOT, static::MODE_RGB, static::MODE_CMYK, static::MODE_RGBA, static::MODE_CMYKA])
        )
        {
