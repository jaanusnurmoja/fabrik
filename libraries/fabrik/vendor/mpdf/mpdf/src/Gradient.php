...
class Gradient
{
    ...
    {
        $s = ' q ';
        $s .= sprintf(' %.3F %.3F %.3F %.3F re W n ', $x * Mpdf::SCALE, ($this->mpdf->h - $y) * Mpdf::SCALE,
            $w * Mpdf::SCALE, -$h * Mpdf::SCALE);
        $s .= sprintf(' %.3F 0 0 %.3F %.3F %.3F cm ', $w * Mpdf::SCALE, $h * Mpdf::SCALE, $x * Mpdf::SCALE,
            ($this->mpdf->h - ($y + $h)) * Mpdf::SCALE);
    ...
    public function Gradient($x, $y, $w, $h, $type, $stops = [], $colorspace = 'RGB', $coords = '', $extend = '', $return = false, $is_mask = false)
    {
        ...
        $s .= sprintf(' %.3F %.3F %.3F %.3F re W n', $x * Mpdf::SCALE, ($this->mpdf->h - $y) * Mpdf::SCALE,
                $w * Mpdf::SCALE, -$h * Mpdf::SCALE) . "\n";
        $s .= sprintf(' %.3F 0 0 %.3F %.3F %.3F cm', $usew * Mpdf::SCALE, $useh * Mpdf::SCALE, $usex * Mpdf::SCALE,
                ($this->mpdf->h - ($usey + $useh)) * Mpdf::SCALE) . "\n";
        ...
        {
            if (
                isset($stops[$i]['offset']) && preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $stops[$i]['offset'],
                    $m)
            )
            {
                $tmp = $this->sizeConverter->convert($m[1], $this->mpdf->w, $this->mpdf->FontSize, false);
        ...
        for ($i = 0; $i < count($stops); $i++)
        {
            ...
            if ($colorspace === 'CMYK')
            {
                $this->mpdf->gradients[$n]['stops'][$i]['col'] = sprintf('%.3F %.3F %.3F %.3F',
                    ord($stops[$i]['col']{1}) / 100, ord($stops[$i]['col']{2}) / 100, ord($stops[$i]['col']{3}) / 100,
                    ord($stops[$i]['col']{4}) / 100);
            }
            ...
            else
            {
                $this->mpdf->gradients[$n]['stops'][$i]['col'] = sprintf('%.3F %.3F %.3F',
                    ord($stops[$i]['col']{1}) / 255, ord($stops[$i]['col']{2}) / 255, ord($stops[$i]['col']{3}) / 255);
            }
    ...
    private function parseMozLinearGradient($m, $repeat)
    {
        ...
        if (
            preg_match('/(left|center|right|bottom|top|deg|grad|rad)/i',
        ...
        { // default values
            ...
            elseif (
                !isset($starty) && isset($first[1]) && preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $first[1],
                    $m)
            )
            {
                $tmp = $this->sizeConverter->convert($m[1], $this->mpdf->w, $this->mpdf->FontSize, false);
    ...
    private function parseMozRadialGradient($m, $repeat)
    {
        ...
        elseif (
        preg_match('/(circle|ellipse|closest-side|closest-corner|farthest-side|farthest-corner|contain|cover)/i',
            $bgr[0])
        )
        {
            $startStops = 1;
        ...
        { // default values
            ...
            if (
            preg_match('/(closest-side|closest-corner|farthest-side|farthest-corner|contain|cover)/i', $shape_size, $m)
            )
            {
                $size = $m[1];
                if ($size === 'contain')
