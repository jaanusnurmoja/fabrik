...
class Helpers
{
    ...
    public static function build_url($protocol, $host, $base_path, $url)
    {
        ...
        if (
            mb_strpos($url, "://") !== false || mb_substr($url, 0, 1) === "#" || mb_strpos($url,
                "data:") === 0 || mb_strpos($url, "mailto:") === 0 || mb_strpos($url, "tel:") === 0
        )
        {
            return $url;
        }
        ...
        if (!in_array(mb_strtolower($protocol), ["http://", "https://", "ftp://", "ftps://"]))
        {
            ...
            //($url[1] !== ':' || ($url[2]!=='\\' && $url[2]!=='/'))
            if (
                $url[0] !== '/' && (strtoupper(substr(PHP_OS, 0,
                        3)) !== 'WIN' || (mb_strlen($url) > 1 && $url[0] !== '\\' && $url[1] !== ':'))
            )
            {
    ...
    {
        if (
        !preg_match('/^data:(?P<mime>[a-z0-9\/+-.]+)(;charset=(?P<charset>[a-z0-9-])+)?(?P<base64>;base64)?\,(?P<data>.*)?/is',
            $data_uri, $match)
        )
        {
            return false;
    ...
    public static function imagecreatefrombmp($filename, $context = null)
    {
        ...
        // read image header
        $meta += unpack('Vheadersize/Vwidth/Vheight/vplanes/vbits/Vcompression/Vimagesize/Vxres/Vyres/Vcolors/Vimportant',
            fread($fh, 40));
        $bytes_read += 40;
            ...
            while ($x < $meta['width'])
            {
                switch ($meta['bits'])
                {
                    ...
                    case 1:
                        ...
                        break;
                    default:
                        trigger_error('imagecreatefrombmp: ' . $filename . ' has ' . $meta['bits'] . ' bits and this is not supported!',
                            E_USER_WARNING);
                        return false;
                }
        ...
        foreach ([' ', '.', ',', '!', '?', '-', '+'] as $s)
        {
            ...
            while (($pos = mb_strpos($str, $s, $pos)) !== false)
            {
                ...
                if ($pos !== false && $pos < $max_len)
                {
                    ...
                    if ($pos + 1 < $max_len)
                    {
                        $str = mb_substr($str, 0, $pos) . mb_strtoupper(mb_substr($str, $pos, 1)) . mb_substr($str,
                                $pos + 1);
                    }
                    else
                    {
