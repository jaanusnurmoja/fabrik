...
class Parser
{
    ...
    public function __construct($sText, Settings $oParserSettings = null, $iLineNo = 1)
    {
        ...
        foreach (
            explode('/',
                Size::ABSOLUTE_SIZE_UNITS . '/' . Size::RELATIVE_SIZE_UNITS . '/' . Size::NON_SIZE_UNITS) as $val
        )
        {
            $iSize = strlen($val);
            if (!isset($this->aSizeUnits[$iSize]))
            {
                $this->aSizeUnits[$iSize] = [];
            }
    ...
    private function parseListItem(CSSList $oList, $bIsRoot = false)
    {
        if ($this->comes('@'))
        {
            ...
            if ($oAtRule instanceof Charset)
            {
                ...
                {
                    throw new UnexpectedTokenException('@charset must be the first parseable token in a document', '',
                        'custom', $this->iLineNo);
                }
    ...
    private function parseAtRule()
    {
        ...
        elseif ($this->identifierIs($sIdentifier, 'keyframes'))
        {
    ...
    private function parsePrimitiveValue()
    {
        ...
        $this->consumeWhiteSpace();
        if (
            is_numeric($this->peek()) || ($this->comes('-.') && is_numeric($this->peek(1,
                    2))) || (($this->comes('-') || $this->comes('.')) && is_numeric($this->peek(1, 1)))
        )
        {
            $oValue = $this->parseNumericValue();
        }
        elseif ($this->comes('#') || $this->comes('rgb', true) || $this->comes('hsl', true))
        {
            $oValue = $this->parseColorValue();
        }
        elseif ($this->comes('url', true))
        {
            $oValue = $this->parseURLValue();
        }
        elseif ($this->comes("'") || $this->comes('"'))
        {
    ...
    private function parseColorValue()
    {
        ...
        if ($this->comes('#'))
        {
            ...
            $aColor = [
                'r' => new Size(intval($sValue[0] . $sValue[1], 16), null, true,
                    $this->iLineNo), 'g' => new Size(intval($sValue[2] . $sValue[3], 16), null, true,
                    $this->iLineNo), 'b' => new Size(intval($sValue[4] . $sValue[5], 16), null, true, $this->iLineNo)
            ];
        }
        else
        {
    ...
    private function consumeUntil($aEnd, $bIncludeEnd = false, $consumeEnd = false, array &$comments = [])
    {
