...
class JDocumentRendererrss extends JDocumentRenderer
{
    ...
    public function render($name, $params = null, $content = null)
    {
        ...
        $url = $uri->toString(['scheme', 'user', 'pass', 'host', 'port']);
        ...
        $feed .= "		<lastBuildDate>" . htmlspecialchars($now->toRFC822(), ENT_COMPAT,
                'UTF-8') . "</lastBuildDate>\n";
        ...
        if ($data->editor != "")
        {
            $feed .= "		<managingEditor>" . htmlspecialchars($data->editor, ENT_COMPAT,
        ...
        for ($i = 0; $i < count($data->items); $i++)
        {
            ...
            $feed .= "		<item>\n";
            $feed .= "			<title><![CDATA[" . htmlspecialchars(strip_tags(html_entity_decode($data->items[$i]->title,
                    ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') . "]]></title>\n";
            ...
            $feed .= "			<description><![CDATA[" . html_entity_decode($this->_relToAbs($data->items[$i]->description),
                    ENT_COMPAT, 'UTF-8') . "]]></description>\n";

            if ($data->items[$i]->author != "")
            {
                $feed .= "			<author>" . htmlspecialchars($data->items[$i]->author, ENT_COMPAT,
                        'UTF-8') . "</author>\n";
            }
            ...
            */
            ...
            if ($data->items[$i]->date != "")
            {
                ...
                $feed .= "			<pubDate>" . htmlspecialchars($itemDate->toRFC822(), ENT_COMPAT,
                        'UTF-8') . "</pubDate>\n";
            }

            if ($data->items[$i]->guid != "")
            {
                $feed .= "			<guid>" . htmlspecialchars($data->items[$i]->guid, ENT_COMPAT,
                        'UTF-8') . "</guid>\n";
            }
