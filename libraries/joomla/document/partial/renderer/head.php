...
{
    public $excludeJsFiles = [
        '/jquery.js',
        '/bootstrap.js'
    ];

    public $keeperJsFiles = [
     ...
     *
     * FABRIK - removes any common js files already included on the main page, so they
     * don't get run again when the popup window opens, which (for example) would create
     * a new jQuery instance, blowing away everything already loaded in the main page instance
     *
    ...
    public function fetchHead($document)
    {
        ...
        if (isset($document->_metaTags['name']['tags']))
        {
            ...
            $document->_metaTags['name']['tags'] = implode(', ',
        ...
        foreach ($document->_metaTags as $type => $tag)
        {
            foreach ($tag as $name => $content)
            {
                ...
                {
                    $buffer .= $tab . '<meta ' . $type . '="' . $name . '" content="' . htmlspecialchars($content,
                            ENT_COMPAT, 'UTF-8') . '" />' . $lnEnd;
                }
            }
        }
        ...
        foreach ($document->_styleSheets as $src => $attribs)
        {
            ...
            foreach ($attribs as $attrib => $value)
            {
                ...
                if (in_array($attrib, ['type', 'mime']) && $document->isHtml5() && in_array($value, $defaultCssMimes))
                {
                    continue;
                }
        ...
        foreach ($document->_scripts as $src => $attribs)
        {
            ...
            foreach ($attribs as $attrib => $value)
            {
                ...
                // Don't add type attribute if document is HTML5 and it's a default mime type. 'mime' is for B/C.
                if (in_array($attrib, ['type', 'mime']) && $document->isHtml5() && in_array($value, $defaultJsMimes))
                {
                    continue;
                }
