...
class PlgFabrik_ListWebservice extends PlgFabrik_List
{
    ...
    public function topButton_result()
    {
        if ($this->canUse())
        {
            ...
            $img = FabrikHelperHTML::image($imageName, 'list', $tmpl, ['alt' => $label]);
    ...
    {
        $opts = FArrayHelper::getValue($args, 0, []);
        $model = $this->getModel();
    ...
    public function process($opts = [])
    {
        ...
        $opts = ['driver' => $driver, 'endpoint' => $params->get('webservice_url'), 'credentials' => $credentials];
    ...
    protected function getMap($formModel)
    {
        ...
        for ($i = 0; $i < $n; $i++)
        {
            ...
            $return[] = ['from' => $from[$i], 'to' => $tid, 'value' => $value[$i], 'match' => $match[$i], 'eval' => (bool)$eval[$i]];
        }
    ...
    protected function getCredentials()
    {
        ...
        $return = [];
        $keys = isset($credentials->webservice_credentials_key) ? $credentials->webservice_credentials_key : [];
        $vals = isset($credentials->webservice_credentials_value) ? $credentials->webservice_credentials_value : [];
        $n = count($keys);
