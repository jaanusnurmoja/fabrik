...
class PlgFabrik_ListRadius_search extends PlgFabrik_List
{
    ...
    protected function searchSelectList($type)
    {
        ...
        elseif ($default_search == 'mylocation')
        {
    ...
    protected function distanceSelectList($default)
    {
        ...
        $select = JHtml::_('select.genericlist', $options, $selectName, 'class="input-small"', 'value', 'text',
            $default);
    ...
    public function onMakeFilters(&$args)
    {
        ...
        $type = $this->app->getUserStateFromRequest($typeKey, 'radius_search_type' . $this->renderOrder,
            [$layoutData->defaultSearch]);
        ...
        $active = $this->app->getUserStateFromRequest($layoutData->baseContext . 'radius_search_active',
            'radius_search_active' . $this->renderOrder, [$layoutData->startActive]);

        if ($layoutData->usePopup)
        {
            $layoutData->address = $this->app->getUserStateFromRequest($layoutData->baseContext . 'geocode' . $this->renderOrder,
                'radius_search_geocode_field' . $this->renderOrder);
            $lat = $this->app->getUserStateFromRequest($layoutData->baseContext . 'lat' . $this->renderOrder,
                'radius_search_lat' . $this->renderOrder);
            $lon = $this->app->getUserStateFromRequest($layoutData->baseContext . 'lon' . $this->renderOrder,
                'radius_search_lon' . $this->renderOrder);

        }
        else
        {
            // don't use session for simple mode
            $active = $this->app->input->get('radius_search_active' . $this->renderOrder, [0], 'array');
            if ($active[0] == 0)
            {
                $layoutData->address = '';
            ...
            {
                $layoutData->address = $this->app->getUserStateFromRequest($layoutData->baseContext . 'geocomplete' . $this->renderOrder,
                    'radius_search_geocomplete_field' . $this->renderOrder);
                $lat = $this->app->getUserStateFromRequest($layoutData->baseContext . 'lat' . $this->renderOrder,
                    'radius_search_geocomplete_lat' . $this->renderOrder);
                $lon = $this->app->getUserStateFromRequest($layoutData->baseContext . 'lon' . $this->renderOrder,
                    'radius_search_geocomplete_lon' . $this->renderOrder);
    ...
    private function placeCoordinates($place)
    {
        ...
        if (is_object($row))
        {
            $coords = explode(':', str_replace(['(', ')'], '', $row->$mapName));
    ...
    protected function getQuery($params)
    {
        ...
        {
            $input->set('radius_search_active' . $this->renderOrder, [0]);
    ...
    protected function getSearchLatLon()
    {
        ...
        $type = $this->app->input->get('radius_search_type' . $this->renderOrder, [], 'array');
        $type = FArrayHelper::getValue($type, 0);

        switch ($type)
        {
            case 'place':
                $place = $this->app->getUserStateFromRequest($baseContext . 'radius_search_place' . $this->renderOrder,
                    'radius_search_place' . $this->renderOrder);
                list($latitude, $longitude) = $this->placeCoordinates($place);
                break;
            default:
                $latitude = $this->app->getUserStateFromRequest($baseContext . 'lat' . $this->renderOrder,
                    'radius_search_lat' . $this->renderOrder);
                $longitude = $this->app->getUserStateFromRequest($baseContext . 'lon' . $this->renderOrder,
                    'radius_search_lon' . $this->renderOrder);
                break;
            case 'geocode':
                $latitude = $this->app->getUserStateFromRequest($baseContext . 'lat' . $this->renderOrder,
                    'radius_search_geocode_lat' . $this->renderOrder);
                $longitude = $this->app->getUserStateFromRequest($baseContext . 'lon' . $this->renderOrder,
                    'radius_search_geocode_lon' . $this->renderOrder);
                break;
            case 'geocomplete':
                $latitude = $this->app->getUserStateFromRequest($baseContext . 'lat' . $this->renderOrder,
                    'radius_search_geocomplete_lat' . $this->renderOrder);
    ...
    protected function getValue()
    {
        ...
        $baseContext = $this->getSessionContext();
        $v = $this->app->getUserStateFromRequest($baseContext . 'radius_search_distance' . $this->renderOrder,
            'radius_search_distance' . $this->renderOrder, '');
    ...
    public function loadJavascriptClass()
    {
        ...
        if ($params->get('place', 1) == 1)
        {
            ...
            FabrikHelperHTML::autoComplete("radius_search_place{$this->renderOrder}", $el->getElement()->id, $formId,
                $el->getElement()->plugin, $opts);
        }
