...
class PlgFabrik_ListEmail extends PlgFabrik_List
{
    ...
    public function getToField()
    {
        ...
        elseif ($toType == 'table' || $toType == 'table_picklist')
        {
            ...
            if ($toType == 'table_picklist')
            {
                ...
                $html .= JHTML::_('select.genericlist', $results, 'email_to_selectfrom[]', $attribs, 'email', 'name',
                    '', 'email_to_selectfrom');
                $html .= '<br /><a href="#" class="btn btn-small" id="email_add">' . FabrikHelperHTML::icon('icon-plus') . ' ' . FText::_('COM_FABRIK_ADD') . ' &gt;&gt;</a>';
                $html .= '</div>';
                $html .= '<div class="span6">';
                $html .= JHTML::_('select.genericlist', $empty, 'list_email_to[]', $attribs, 'email', 'name', '',
                    'list_email_to');
            ...
            else
            {
                ...
                $html = JHTML::_('select.genericlist', $results, 'list_email_to[]', $attribs, 'email', 'name', '',
                    'list_email_to');
            }
    ...
    private function _upload()
    {
        ...
        $files = $input->files->get('attachment', []);
    ...
    private function _send($row, $mailTo)
    {
        ...
        else
        {
            ...
            return FabrikWorker::sendMail($emailFrom, $fromName, $mailTo, $thisSubject, $thisMsg, 1, $cc, $bcc,
                $this->filepath,
    ...
    private function mailMerged($firstRow, $mergedMsg, $sent, $notSent)
    {
        ...
        if ($toHow == 'single')
        {
            ...
            if ($sent > 0)
            {
                $res = FabrikWorker::sendMail($emailFrom, $fromName, $thisTos, $thisSubject, $mergedMsg, true, $cc,
                    $bcc, $this->filepath,
                    $replyEmail, $replyEmailName);
            }
        }
        else
        {
            foreach ($thisTos as $thisTo)
            {
                if (FabrikWorker::isEmail($thisTo))
                {
                    $res = FabrikWorker::sendMail($emailFrom, $fromName, $thisTo, $thisSubject, $mergedMsg, true, $cc,
                        $bcc, $this->filepath,
                        $replyEmail, $replyEmailName);
     ...
     *
     */
    private function _replyEmailName($data = [])
    {
        $w = new FabrikWorker;
        $params = $this->getParams();
        ...
        return [$replyEmail, $replyEmailName];
    }
     ...
     *
     */
    private function _fromEmailName($data = [])
    {
        $w = new FabrikWorker;
        $params = $this->getParams();
