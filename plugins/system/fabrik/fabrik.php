...
class PlgSystemFabrik extends JPlugin
{
    ...
    public function __construct(&$subject, $config)
    {
        ...
        else
        {
            ...
            if (!$loaded)
            {
                ...
                {
                    $app->enqueueMessage('Fabrik cannot find files required for this version of Joomla.  <b>DO NOT</b> use the Fabrik backend admin until this is resolved.  Please visit <a href="http://fabrikar.com/forums">our web site</a> and check for announcements about this version',
                        'error');
                }
            }
        }
    ...
    public static function js()
    {
        ...
        if ($session->has('fabrik.js.cacheids'))
        {
            ...
            $cacheIds = $session->get('fabrik.js.cacheids', []);
            ...
            if (!empty($cacheId))
            {
                ...
                $script = $cache->call(['PlgSystemFabrik', 'buildJs'], $cacheId);
            }
            else
            {
    ...
    {
        $session = JFactory::getSession();
        $config = (array)$session->get('fabrik.js.config', []);
        $config = implode("\n", $config);

        $js = (array)$session->get('fabrik.js.scripts', []);
        $js = implode("\n", $js);

        $jLayouts = (array)$session->get('fabrik.js.jlayouts', []);
  ...
  cache: true
    ...
    public static function onDoContentSearch($text, $params, $phrase = '', $ordering = '')
    {
        ...
        foreach ($ids as $id)
        {
            ...
            $elementModel = $listModel->getFormModel()->getElement($params->get('search_description', $table->label),
                true);
    ...
    function onExtensionAfterSave($option, $data)
    {
        ...
        $table->load(['name' => 'Fabrik - Premium']);
        $table->save([
            'type'     => 'collection',
            'name'     => 'Fabrik - Premium',
            'enabled'  => 1,
