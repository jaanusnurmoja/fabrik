...
class FabrikModelFusionchart extends FabrikFEModelVisualization
{
    ...
    protected function setChartMessages()
    {
        ...
        {
            $this->FC->setChartMessage("PBarLoadingText=" . $params->get('fusionchart_message_loading',
                    'Please Wait.The chart is loading...'));
        }

        if ($params->get('fusionchart_message_parsing_data'))
        {
            $this->FC->setChartMessage("ParsingDataText=" . $params->get('fusionchart_message_parsing_data',
                    'Reading Data. Please Wait'));
        }

        if ($params->get('fusionchart_message_nodata'))
        {
            $this->FC->setChartMessage("ChartNoDataText=" . $params->get('fusionchart_message_nodata',
                    'No data to display.'));
        }
    }
    ...
    public function getFusionchart()
    {
        ...
        $calc_prefixes = ['sum___', 'avg___', 'med___', 'cnt___'];
        $calc_prefixmap = ['sum___' => 'sums', 'avg___' => 'avgs', 'med___' => 'medians', 'cnt___' => 'count'];
        ...
        switch ($chartType)
        {
            ...
            case 'SPLINEAREA':
                ...
                if ($chartType == 'PIE2D' || $chartType == 'PIE3D')
                {
                    ...
                    $alphas = $params->get('fusionchart_elalpha', []);
                ...
                elseif ($chartType == 'FUNNEL3D')
                {
                    $strParam .= ';is2D=0';
                    $chartType = 'FUNNEL';
                }
                elseif ($chartType === 'MAPS')
                {
                ...
                else
                {
                    ...
                    $gsums = FArrayHelper::getValue($chartCumulatives, $datasetKey,
                        '0') == '0' ? $gdata[$datasetKey] : $this->gcumulatives[$datasetKey];
            ...
            case 'SCROLLSTACKEDCOLUMN2D':
                if ($this->c > 0)
                {
                    ...
                    foreach ($gdata as $key => $chartdata)
                    {
                        $cdata = FArrayHelper::getValue($chartCumulatives, $key,
                            '0') == '0' ? $gdata[$key] : $this->gcumulatives[$key];
                        $datasetLabel = $this->axisLabels[$key];
    ...
    protected function trendLine(&$gdata = null)
    {
        ...
        elseif (!$found && ($trendstart != '' && $trendend != ''))
        {
    ...
    protected function buildTrendLine($startval, $endval, $nbe = null)
    {
        ...
        if (isset($nbe))
        {
            $trendParams['displayvalue'] = FArrayHelper::getValue($this->axisLabels, $nbe,
                $params->get('fusionchart_trendlabel', ''));
        }
    ...
    private function addDataset($dataset, $label, $params = [])
    {
    ...
    private function makeChartData($value, $params = [])
    {
        if ($value === null)
        {
            if (empty($params))
    ...
    private function addChartData($value, $params = [])
    {
