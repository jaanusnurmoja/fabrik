...
class FabrikModelApprovals extends FabrikFEModelVisualization
{
    ...
    public function getRows()
    {
        ...
        for ($x = 0; $x < count($ids); $x++)
        {
            ...
            $this->asField($formModel, $approveEls[$x], $asfields, ['alias' => 'approve']);
            $this->asField($formModel, $titles[$x], $asfields, ['alias' => 'title']);
            $this->asField($formModel, $users[$x], $asfields, ['alias' => 'user']);
            $this->asField($formModel, $contents[$x], $asfields, ['alias' => 'content']);

            $query->select($db->quote($item->label) . " AS type, " . $item->db_primary_key . ' AS pk, ' . implode(',',
                    $asfields))
