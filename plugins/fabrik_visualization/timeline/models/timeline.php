...
class FabrikModelTimeline extends FabrikFEModelVisualization
{
    ...
    public function onAjax_getEvents()
    {
        ...
        $lists = $params->get('timeline_table', []);
        ...
        if ($start <= $totals[$currentList])
        {
            ...
            if ($start + $this->step > $totals[$currentList])
            {
                ...
                else
                {
                    ...
                    $res->events = array_merge($res->events,
                        $this->jsonEvents($nextListId, $totals[$nextListId], 0, $c));
                }
            }
        }
        else
    ...
    protected function jsonEvents($listId, $total, $start, $c)
    {
        ...
        $templates = (array)$params->get('timeline_detailtemplate', []);
        $startdates = (array)$params->get('timeline_startdate', []);
        $enddates = (array)$params->get('timeline_enddate', []);
        $labels = (array)$params->get('timeline_label', []);
        $colours = (array)$params->get('timeline_colour', []);
        $textColours = (array)$params->get('timeline_text_color', []);
        $classNames = (array)$params->get('timeline_class', []);
        $evals = (array)$params->get('eval_template', []);
            ...
            foreach ($data as $group)
            {
                if (is_array($group))
                {
                    foreach ($group as $row)
                    {
                        ...
                        $event->start = array_key_exists($startdate . '_raw',
                            $row) ? $row->{$startdate . '_raw'} : $row->$startdate;
                        $event->end = $event->start;

                        if (trim($enddate) !== '')
                        {
                            $end = array_key_exists($enddate . '_raw',
                                $row) ? $row->{$enddate . '_raw'} : @$row->$enddate;
    ...
    protected function getTotal()
    {
        ...
        $lists = $params->get('timeline_table', []);
        $totals = [];
        $where = $this->app->input->get('where', [], 'array');
    ...
    protected function getBandInfo()
    {
        ...
        $intervals = FArrayHelper::getValue($bands, 'timelne_band_interval_unit', []);
        $widths = FArrayHelper::getValue($bands, 'timeline_band_width', []);
        $overviews = FArrayHelper::getValue($bands, 'timeline_band_as_overview', []);
        $bgs = FArrayHelper::getValue($bands, 'timeline_band_background_colour', []);
        $data = [];
    ...
    public function setListIds()
    {
        ...
        {
            $params = $this->getParams();
            $this->listids = (array)$params->get('timeline_table', []);
        }
    }
}
