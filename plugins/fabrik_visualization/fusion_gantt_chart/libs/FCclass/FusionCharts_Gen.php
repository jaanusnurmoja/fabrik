...
#############============ For Future Use (start)============================
    ...
    var $MSSDataset       = [];       # dataset array for MSStackedColumn2D
    var $MSSDatasetParams = [];  # MSSDataset parameter setting
    ...
    # lineset
    var $lineSet      = [];         # lineSet array
    var $lineSetParam = [];    # lineSet Parameter setting array
    ...
    # vtrendLines array
    var $vtrendLines = [];     # vtrendLines array
    var $vtLineCounter;             # vtrendLines array counter

    # style array
    var $styles = [];           # styles array
...
##------------ PUBLIC FUNCTIONS ----------------------------------------------------------------
        ...
        if ($this->DataBaseType == "mysql")
        {
            ...
            while ($row = mysql_fetch_array($query_result))
            {
                ...
                else
                {
                    ...
                    $this->addChartData($this->encodeSpecialChars($data),
                        "name=" . $this->encodeSpecialChars($label) . ";" . $strParam, "");
                }
            }
        }
        elseif ($this->DataBaseType == "oracle")
    ...
    function renderChart($isHTML = false, $display = true)
    {
        ...
        if ($isHTML == false)
        {
            if ($display)
            {
                print $this->renderChartJS($this->SWFFile . $this->chartMSG, "", $this->strXML, $this->chartID,
                    $this->width, $this->height, $this->JSC["debugmode"], $this->JSC["registerwithjs"], $this->wMode);
            }
            else
            {
                return $this->renderChartJS($this->SWFFile . $this->chartMSG, "", $this->strXML, $this->chartID,
                    $this->width, $this->height, $this->JSC["debugmode"], $this->JSC["registerwithjs"], $this->wMode);
            }
        ...
        else
        {
            if ($display)
            {
                print $this->renderChartHTML($this->SWFFile . $this->chartMSG, "", $this->strXML, $this->chartID,
                    $this->width, $this->height, $this->JSC["debugmode"], $this->JSC["registerwithjs"], $this->wMode);
            }
            else
            {
                return $this->renderChartHTML($this->SWFFile . $this->chartMSG, "", $this->strXML, $this->chartID,
                    $this->width, $this->height, $this->JSC["debugmode"], $this->JSC["registerwithjs"], $this->wMode);
            }
        }
    ...
    {
        print $this->renderChartJS($this->SWFFile, "", $dataXML, $this->chartID, $this->width, $this->height,
            $this->JSC["debugmode"], $this->JSC["registerwithjs"], $this->wMode);
    }
...
##------------ PRIVATE FUNCTIONS ----------------------------------------------------------------
    ...
    function setSSMSDataArray($value = "", $setParam = "", $vlineParam = "")
    {
        ...
        if ($vlineParam == "")
        {
            ...
            {
                if (
                    $this->seriesType == 1 && (gettype(strpos($this->chartType,
                            "line")) == "boolean" && gettype(strpos($this->chartType, "area")) == "boolean")
                )
                {
            ...
            else
            {
                if (
                    $this->seriesType == 1 && (gettype(strpos($this->chartType,
                            "pie")) == "boolean" && gettype(strpos($this->chartType,
                            "line")) == "boolean" && gettype(strpos($this->chartType, "area")) == "boolean")
                )
                {
                    if (strpos(strtolower($strParam), " color") === false)
    ...
    function addGanttCategorySet($catParam = "")
    {
        ...
        $this->GT_categories[$this->GT_categories_Counter] = [];
        $strParam = "";
    ...
    function addGanttDatacolumn($catParam = "")
    {
        ...
        $this->GT_datatable[$this->GT_datatable_Counter] = [];
        $strParam = "";
    ...
    function addLineset($seriesName, $strParam)
    {
        $this->createLineset();
        $this->lineSetCounter++;
        $this->lineSet[$this->lineCounter][$this->lineSetCounter] = [];
    ...
    function addMSSSubDataset($seriesName, $strParam)
    {
        ...
        $this->MSSDataset[$this->MSSDatasetCounter][$this->MSSSubDatasetCounter] = [];
    ...
    function createMSSDataset()
    {
        ...
        $this->MSSDataset[$this->MSSDatasetCounter] = [];
        $this->MSSDatasetParams[$this->MSSDatasetCounter] = [];
    }
    ...
    {
        $this->MSSSetCounter++;
        $this->MSSDataset[$this->MSSDatasetCounter][$this->MSSSubDatasetCounter][$this->MSSSetCounter] = [];
    }
    ...
    {
        $this->styles[$styleID][$subStyle][$subParam] = [];
    }
