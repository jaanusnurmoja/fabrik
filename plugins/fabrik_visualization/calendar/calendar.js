...
var fabrikCalendar = new Class({
    ...
    _makeEventRelDiv: function (entry, opts, aDate, target) {
        ...
        if (entry.link !== '' && this.options.readonly === false && this.options.j3 === false) {
            x = new Element('a', {
                'href': entry.link, 'class': 'fabrikEditEvent',
                'events': {
                    'click': function (e) {
        ...
        for (var i = 1; i < trs.length; i++) {
            ...
            tds.each(function (td) {
                ...
                td.adopt(
                    new Element('div', {
                        'class': 'date',
                        'styles': {'background-color': this._getColor('#E8EEF7', firstDate)}
    ...
    renderMonthView: function () {
        ...
        if (!this.mothView) {
            ...
            for (var i = 0; i < 6; i++) {
                ...
                for (d = 0; d < 7; d++) {
                    ...
                    tr.adopt(new Element('td', {
                        'class': 'day ' + (firstDate.getTime()) + extraClass,
                        'styles': {
            ...
            this.mothView = new Element('div', {
                'class': 'monthView', 'styles': {
                    'position': 'relative'
                }
            }).adopt(
                new Element('table', {
    ...
    render: function (options) {
        ...
        this.ajax.updateEvents = new Request({
