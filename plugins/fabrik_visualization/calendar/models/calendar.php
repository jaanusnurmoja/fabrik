...
class FabrikModelCalendar extends FabrikFEModelVisualization
{
        ...
        if (is_null($this->events))
        {
            ...
            for ($i = 0; $i < count($tables); $i++)
            {
                ...
                if ($tables[$i] != 'undefined')
                {
                    ...
                    $this->events[$tables[$i]][] = [
                        'startdate'   => $startDate, 'enddate' => $endDate, 'startShowTime' => $startShowTime,
    ...
    public function getDateLimitsMsg()
    {
        ...
        if ($min !== '' && $max === '')
        {
            $msg = '<br />' . JText::sprintf('PLG_VISUALIZATION_CALENDAR_LIMIT_AFTER',
                    JFactory::getDate($min)->format($f));
        }

        if ($min === '' && $max !== '')
        {
            $msg = '<br />' . JText::sprintf('PLG_VISUALIZATION_CALENDAR_LIMIT_BEFORE',
