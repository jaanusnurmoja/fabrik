            ...
            foreach ($allData as $data)
            {
                foreach ($data as $row)
                {
                    ...
                    if (!empty($dateElement))
                    {
                        ...
                        $str .= "			<pubDate>" . htmlspecialchars($pubDate->toRFC822(), ENT_COMPAT,
                                'UTF-8') . "</pubDate>\n";
                    }
    ...
    public function getJs()
    {
        ...
        if ($params->get('media_which_player', 'jw') == 'jw')
        {
            ...
            $opts->jw_playlist_url = COM_FABRIK_LIVESITE
                ...
                . $this->getId();
            $opts->jw_skin = COM_FABRIK_LIVESITE . 'plugins/fabrik_visualization/media/libs/jw/skins/' . $params->get('media_jw_skin',
                    'snel.zip');
        }
