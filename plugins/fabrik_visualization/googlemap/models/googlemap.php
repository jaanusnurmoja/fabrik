...
class FabrikModelGooglemap extends FabrikFEModelVisualization
{
        ...
        if ($opts->use_overlays)
        {
            ...
            if (!empty($overlayCode))
            {
                ...
                if (is_array($overlayArray))
                {
                    ...
                    {
                        $opts->overlay_preserveviewports = array_merge($opts->overlay_preserveviewports,
                            $overlayArray['preserveViewports']);
                    }

                    if (array_key_exists('suppressInfoWindows', $overlayArray))
                    {
                        $opts->overlay_suppressinfowindows = array_merge($opts->overlay_suppressinfowindows,
    ...
    protected function getPolyline()
    {
        ...
        foreach ($listModels as $listModel)
        {
            ...
            $mapsElements = Lizt::getElements($listModel, ['plugin' => 'googlemap', 'published' => 1]);
            ...
            */
            ...
            foreach ($data as $groupKey => $group)
            {
                foreach ($group as $row)
                {
                    ...
                    {
                        $rowData[trim($matches[0], '{}')] = sprintf('%.' . $matches[1] . 'f',
                                $v[0]) . ',' . sprintf('%.' . $matches[1] . 'f', $v[1]);
                    }
                    ...
                    $title = $titleElement == '' ? '' : html_entity_decode(strip_tags($row->$titleElement), ENT_COMPAT,
                        'UTF-8');
                    ...
                    if (
                        empty($html) && (array_key_exists('fabrik_view', $rowData) || array_key_exists('fabrik_edit',
                                $rowData))
                    )
                    {
                    ...
                    if ($iconImg == '')
                    {
                        ...
                        if ($iconImg != '')
                        {
                            ...
                            if (
                                strstr($iconImg, 'http://') || strstr($iconImg,
                                    'https://') || JFile::exists(JPATH_BASE . $iconImg)
                            )
                            {
                    ...
                    if (array_key_exists($v[0] . $v[1], $icons))
                    {
                        ...
                        {
                            $groupedIcons[] = [
                                $v[0], $v[1], $html, $iconImg, $width, $height, 'groupkey' => $groupKey, 'listid' => $listId,
    ...
    private function markerSize($iconImg)
    {
        if (!array_key_exists($iconImg, $this->markerSizes))
        {
            ...
            $this->markerSizes[$iconImg] = [$width, $height];
        }
    ...
    public function getStaticMap()
    {
        ...
        {
            $first = $icons[0];
            $bounds = ['lat' => [$first[0], $first[0]], 'lon' => [$first[1], $first[1]]];
            $c = 1;
