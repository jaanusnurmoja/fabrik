...
// Get labels
$query->select('params')->from('i2qtd_fabrik_elements')->where('id = 15');
$db->setQuery($query);
...
$query->clear();
$query->select('COUNT( * ) AS value, `doelgroep_scholier` AS label')->from('fab_userinfo')->group('doelgroep_scholier');
$db->setQuery($query);
$data = [];
