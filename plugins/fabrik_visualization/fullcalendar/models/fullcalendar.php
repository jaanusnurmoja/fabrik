...
class FabrikModelFullcalendar extends FabrikFEModelVisualization
{
    ...
    public function &getEventLists()
    {
        if (is_null($this->eventLists))
        {
            ...
            $query->select('id AS value, label AS text')->from('#__{package}_lists')->where('id IN (' . implode(',',
                    $lists) . ')');
            $db->setQuery($query);
                ...
                if (is_array($formdata))
                {
                    foreach ($formdata as $row)
                    {
                        if ($row->startdate != '')
                        {
                            ...
                            $row->allday = is_string($row->allday) ? filter_var($row->allday, FILTER_VALIDATE_BOOLEAN,
                                FILTER_NULL_ON_FAILURE) : (bool)$row->allday;
                            ...
                            if (!empty($row->status))
                            {
                                $row->status = FabrikString::getRowClass($row->status,
                                    FabrikString::shortColName($status));
                            }
                            else
                            {
                                $row->status = '';
    ...
    public function getDateLimitsMsg()
    {
        ...
        if ($min !== '' && $max === '')
        {
            $msg = '<br />' . JText::sprintf('PLG_VISUALIZATION_FULLCALENDAR_LIMIT_AFTER',
                    JFactory::getDate($min)->format($f));
        }

        if ($min === '' && $max !== '')
        {
            $msg = '<br />' . JText::sprintf('PLG_VISUALIZATION_FULLCALENDAR_LIMIT_BEFORE',
                    JFactory::getDate($max)->format($f));
        }
