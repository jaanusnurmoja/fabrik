...
class FabrikViewFullcalendar extends JViewLegacy
{
    ...
    private function iniJs()
    {
        ...
        $shim = $model->getShim();
        $paths = ['fullcalendar' => 'plugins/fabrik_visualization/fullcalendar/libs/fullcalendar/fullcalendar.min'];

        $shim['fullcalendar'] = (object)[
            'deps' => ['lib/moment/moment']
        ];
        ...
        if ($lang != '' && $lang != 'en-gb')
        {
            $shim['lang'] = (object)[
                'deps' =>
                    ['lib/moment/moment', 'fullcalendar']
            ];
