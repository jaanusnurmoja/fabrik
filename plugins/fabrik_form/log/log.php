                        ...
                        if (!$groupModel->isJoin())
                        {
                            if ($logDelete === '2')
                            {
                                ...
                                foreach ($elementModels as $elementModel)
                                {
                                    ...
                                    elseif ($mode === 'include' && !in_array($fullKey, $fields))
                                    {
                                        continue;
                                    }
                                    ...
                                    if (isset($row->$fullKeyRaw) && !$this->dataEmpty($row->$fullKeyRaw))
                                    {
                                        ...
                                        {
                                            $pks[$row->__pk_val]['row_changes'][$fullKeyRaw] = [
                        ...
                        else
                        {
                            if ($deleteJoinedRows)
                            {
                                ...
                                if (!\Fabrik\Helpers\ArrayHelper::emptyIsh($thisPks))
                                {
                                    ...
                                    {
                                        if (
                                        !array_key_exists($thisPk,
                                            $pks[$row->__pk_val]['joins'][$joinId]['row_changes'])
                                        )
                                        {
                                        ...
                                        foreach ($elementModels as $elementModel)
                                        {
                                            ...
                                            {
                                                if (
                                                !array_key_exists($fullKeyRaw,
                                                    $pks[$row->__pk_val]['joins'][$joinId]['row_changes'][$thisPk])
                                                )
                                                {
                                                    $pks[$row->__pk_val]['joins'][$joinId]['row_changes'][$thisPk][$fullKeyRaw] = [
                                                        'time_date'   => $date->format('Y-m-d H:i:s'),
                                                        'form_id'     => $formModel->getId(),
                                                        'list_id'     => $formModel->getListModel()->getId(),
                                                        'element_id'  => $elementModel->getId(),
