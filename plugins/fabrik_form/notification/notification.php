...
class PlgFabrik_FormNotification extends PlgFabrik_Form
{
    ...
    protected function getRef($listId = 0)
    {
        ...
        return $db->q($input->getInt('listid', $listId) . '.' . $input->getInt('formid', 0) . '.' . $input->get('rowid',
                '', 'string'));
    }
    ...
    protected function process($add, $why)
    {
        ...
        else
        {
            ...
            foreach ($userIds as $userId)
            {
                $ok = true;
                $query->clear('set');
                $fields2 = array_merge($fields, ['user_id = ' . $userId]);
    ...
    public function onAfterProcess()
    {
        ...
        $query = $db->getQuery(true);
        $fields = ['reference = ' . $ref, 'event = ' . $event, 'date_time = ' . $date];
