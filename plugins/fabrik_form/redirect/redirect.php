...
class PlgFabrik_FormRedirect extends PlgFabrik_Form
{
    ...
    public function onLastProcess()
    {
        ...
        $surl = (array)$this->session->get($context . 'url', []);
        $stitle = (array)$this->session->get($context . 'title', []);
        $smsg = (array)$this->session->get($context . 'msg', []);
        $sshowsystemmsg = (array)$this->session->get($context . 'showsystemmsg', []);
        ...
        $this->data['thanks_message'] = FText::_($w->parseMessageForPlaceHolder($params->get('thanks_message'),
            $this->data));
        ...
        if ($this->data['jump_page'] != '')
        {
            ...
            $this->session->set($context . 'redirect_content_popup_width',
                $params->get('redirect_content_popup_width', '300'));
            $this->session->set($context . 'redirect_content_popup_height',
                $params->get('redirect_content_popup_height', '300'));
            $this->session->set($context . 'redirect_content_popup_x_offset',
                $params->get('redirect_content_popup_x_offset', '0'));
            $this->session->set($context . 'redirect_content_popup_y_offset',
                $params->get('redirect_content_popup_y_offset', '0'));
            $this->session->set($context . 'redirect_content_popup_title',
                $params->get('redirect_content_popup_title', ''));
            $this->session->set($context . 'redirect_content_popup_reset_form',
                $params->get('redirect_content_popup_reset_form', '1'));
        }
        else
        {
    ...
    protected function buildJumpPage()
    {
        ...
        $reserved = ['format', 'view', 'layout', 'task'];
        $queryvars = [];
