...
class PlgFabrik_FormMailgun extends PlgFabrik_Form
{
    ...
    public function onAfterProcess()
    {
        ...
        $content = $contentTemplate != '' ? FabrikHelperHTML::getContentTemplate($contentTemplate, 'both',
            $runContentPlugins) : '';
        ...
        @list($emailFrom, $emailFromName) = explode(":",
            $w->parseMessageForPlaceholder($params->get('mailgun_from'), $this->data, false), 2);
        ...
        // Changes by JFQ
        @list($returnPath, $returnPathName) = explode(":",
            $w->parseMessageForPlaceholder($params->get('mailgun_return_path'), $this->data, false), 2);
        ...
        $subject = preg_replace_callback('/&#([0-9a-fx]+);/mi', [$this, 'replace_num_entity'], $subject);
        ...
        $dbEmailTo = array_map([$this->_db, 'quote'], $emailTo);
        ...
        foreach ($emailTo as $email)
        {
            ...
            if (FabrikWorker::isEmail($email))
            {
                ...
                $thisSubject = strip_tags($w->parseMessageForPlaceholder($subject, $this->data, true, false,
                    $thisUser));
                ...
                */
                ...
                try
                {
                    ...
                    */
                    ...
                    $messageBldr->setFromAddress($emailFrom, ['full_name' => $emailFromName]);
            ...
            {
                $this->app->enqueueMessage(JText::sprintf('PLG_FORM_EMAIL_DID_NOT_SEND_EMAIL_INVALID_ADDRESS', $email),
                    'notice');
            }
        }
    ...
    protected function _getTextEmail()
    {
        ...
        foreach ($groupModels as &$groupModel)
        {
            ...
            foreach ($elementModels as &$elementModel)
            {
                ...
                $key = (!array_key_exists($element->name, $data)) ? $elementModel->getFullName(true,
                    false) : $element->name;
