...
class PlgFabrik_FormPaypal extends PlgFabrik_Form
{
    ...
    public function onAfterProcess()
    {
        ...
        {
            $shippingAmount = FArrayHelper::getValue($this->data,
                FabrikString::safeColNameToArrayKey($params->get('paypal_shipping_cost_element')));
        ...
        if (trim($item) == '')
        {
            $itemRaw = FArrayHelper::getValue($this->data,
                FabrikString::safeColNameToArrayKey($params->get('paypal_item_element') . '_raw'));
    ...
    public function onThanks()
    {
        ...
        $retMsg = $params->get('paypal_return_msg', []);
    ...
    public function onIpn()
    {
        ...
        $ipnSubscriberIDField = $params->get('paypal_ipn_subscr_id_element', []);
        ...
        $buyer_address = $input->get('address_status', '', 'string') . ' - ' . $input->get('address_street', '',
                'string')
            ...
            else
            {
                ...
                while (!feof($fp))
                {
                    ...
                    if (JString::strcmp($tres, "VERIFIED") === 0)
                    {
                        ...
                        {
                            $this->doLog('form.paypal.ipndebug.ipn_no_txn_fields',
                                "No IPN txn or status fields specified, can't test for reversed, refunded or cancelled");
                        }
                        ...
                        if ($status == 'ok')
                        {
                            ...
                            if ($ipn !== false)
                            {
                                ...
                                if (method_exists($ipn, $ipnFunction))
                                {
                                    ...
                                    if ($status != 'ok')
                                    {
                                        $this->doLog('form.paypal.ipndebug.ipn_function_not_ok',
                                            "The IPN function $ipnFunction did not return ok");
                                        break;
                                    }
                                }
                                ...
                                if (method_exists($ipn, $txnTypeFunction))
                                {
                                    ...
                                    if ($status != 'ok')
                                    {
                                        $this->doLog('form.paypal.ipndebug.ipn_txn_type_function_not_ok',
                                            "The IPN txn type function $txnTypeFunction did not return ok");
                                        break;
                                    }
                                }
                            }
                            ...
                            {
                                $this->doLog('form.paypal.ipndebug.ipn_cannot_load',
