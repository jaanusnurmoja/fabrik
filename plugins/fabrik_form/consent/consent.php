...
class PlgFabrik_FormConsent extends PlgFabrik_Form
{
    ...
    public function onBeforeProcess()
    {
        ...
        {
            $formModel->errors['consent_required'] = [FText::_('PLG_FORM_CONSENT_PLEASE_CONFIRM_CONSENT')];
        ...
        {
            $formModel->errors['consent_remove'] = [FText::_('PLG_FORM_CONSENT_REMOVE_CONSENT')];
            $formModel->formErrorMsg = FText::_('PLG_FORM_CONSENT_PLEASE_CONFIRM_CONSENT');
            return false;
        }
    }
    ...
    protected function savePrivacy($data, $userId, $status)
    {
        ...
        $columns = ['id', 'date_time', 'list_id', 'form_id', 'row_id', 'user_id', 'consent_message', 'update_record', 'ip', 'newsletter_engine', 'sublist_id', 'subid'];
        $values = [
            'NULL',
    ...
    protected function sendUpdateWarning($email)
    {
        ...
        {
            $this->app->enqueueMessage(JText::sprintf('PLG_FORM_CONSENT_DID_NOT_SEND_EMAIL_INVALID_ADDRESS', $email),
                'warning');
        }
