...
class PlgFabrik_FormJ2Store extends PlgFabrik_Form
{
    ...
    public function onAfterProcess()
    {
        ...
        $productTable->load(['product_source' => $source, 'product_source_id' => $this->getModel()->getInsertId()]);
    ...
    private function storeVariant($data, $productId)
    {
        ...
        $table->load(['product_id' => $productId]);
    ...
    private function storeFiles($placeholderData, $productId)
    {
        ...
        $rows = [];
        $fields = ['j2store_product_file_display_name', 'j2store_product_file_save_name'];

        foreach ($fields as $field)
        {
            $key = str_replace('j2store_', '', $field);
            $fileDisplayName = $params->get($field);
            $simpleFileDisplayName = str_replace(['{', '}'], ['', ''], $fileDisplayName);
            $displayNames = ArrayHelper::getValue($placeholderData, $simpleFileDisplayName, [$fileDisplayName]);
            $i = 0;
    ...
    private function storeImages($data, $productImageId)
    {
        ...
        $table->load(['product_id' => $data['j2store_product_id']]);
        $table->bind($images);
    ...
    public function getEndContent()
    {
        ...
        if ($product->get_product_by_source($this->j2StoreSource(), $id))
        {
            ...
            $this->html = $layout->render((object)['product' => $product]);
        }
    }
        ...
        foreach ($data as $group)
        {
            foreach ($group as $row)
            {
                ...
                if ($product->get_product_by_source($source, $id))
                {
                    ...
                    $row->j2store = $layout->render((object)['product' => $product]);
                }
                else
                {
    ...
    public function onGetPluginRowHeadings($args)
    {
        ...
        $args[0]['headingClass']['j2store'] = ['class' => '', 'style' => ''];
        $args[0]['cellClass']['j2store'] = ['class' => '', 'style' => ''];
    }
