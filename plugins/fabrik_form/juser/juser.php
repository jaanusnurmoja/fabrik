...
class PlgFabrik_FormJUser extends plgFabrik_Form
{
            ...
            if ($count === 0)
            {
                try
                {
                    ...
                    $query->select('DISTINCT u.*, ug.group_id')->from($fabrikDb->quoteName('#__users') . 'AS u')
                        ->join('LEFT',
                            '#__user_usergroup_map AS ug ON ug.user_id = u.id')->group('u.id')->order('u.id ASC');
                    $fabrikDb->setQuery($query);
                    ...
                    foreach ($origUsers as $o_user)
                    {
                        ...
                        $query->clear();
                        $fields = [
                            $this->getFieldName('juser_field_userid', true) => $o_user->id,
                    ...
                    foreach ($rows as $row)
                    {
                        if (isset($row->$userIdField))
                        {
                            if (!empty($row->$userIdField))
                            {
                                ...
                                if (!$user->delete())
                                {
                                    $this->app->enqueueMessage('Unable to delete user id ' . $row->$userIdField,
                                        'error');
                                }
                            }
                        }
        ...
        if ($params->get('juser_field_usertype') != '')
        {
            ...
            else
            {
                ...
                if ($params->get('juser_group_preserve', '0') === '1')
                {
                    ...
                    $whitelist = $params->get('juser_group_whitelist', []);
    ...
    private function filterGroupIds($data, $me, $groupIds)
    {
        ...
        $authLevels = array_merge((array)$params->get('juser_group_whitelist', []), $me->getAuthorisedGroups());

        foreach ($groupIds as $groupId)
        {
            if (
                in_array($groupId, $authLevels) || $me->authorise('core.admin',
                    'com_users') || $groupId == $defaultGroup
            )
            {
                $data[] = $groupId;
            }
    ...
    protected function check($post)
    {
        ...
        if ($post['name'] == '')
        {
            ...
            $this->raiseError($formModel->errors, $this->namefield,
                FText::_('JLIB_DATABASE_ERROR_PLEASE_ENTER_YOUR_NAME'));
            $ok = false;
        }

        if ($post['username'] == '')
        {
            $this->raiseError($formModel->errors, $this->usernamefield,
                FText::_('JLIB_DATABASE_ERROR_PLEASE_ENTER_A_USER_NAME'));
            $ok = false;
        }
        ...
        {
            $this->raiseError($formModel->errors, $this->usernamefield,
                JText::sprintf('VALID_AZ09', FText::_('Username'), 2));
            $ok = false;
        }
