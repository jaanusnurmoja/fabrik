...
class PlgFabrik_FormStripe extends PlgFabrik_Form
{
    ...
    public function onBeforeStore()
    {
        ...
        else
        {
            ...
            elseif ($params->get('stripe_cost_eval', 0) == 1)
            {
                $amount = @eval($amount);
            ...
            elseif ($params->get('stripe_item_eval', 0) == 1)
            {
                $item = @eval($item);
            ...
            if (trim($item) == '')
            {
                $itemRaw = ArrayHelper::getValue($this->data,
                    StringHelper::safeColNameToArrayKey($params->get('stripe_item_element') . '_raw'));
    ...
    public function getBottomContent()
    {
        ...
        if (!empty($customerId))
        {
            ...
            if ($params->get('stripe_customers_allow_update_cc', '0') === '1')
            {
                ...
                $opts->panelLabel = FText::_(
                    $params->get('stripe_customers_update_button_name',
                        "PLG_FORM_STRIPE_CUSTOMERS_UPDATE_CC_BUTTON_NAME")
                );
                $dep = new stdClass;
                $dep->deps = [
                    'stripe'
            ...
            $layoutData->updateButtonName = FText::_($params->get('stripe_customers_update_button_name',
                "PLG_FORM_STRIPE_CUSTOMERS_UPDATE_CC_BUTTON_NAME"));
            ...
            $layoutData->bottomText = FText::_($params->get('stripe_charge_bottom_text_existing',
                'PLG_FORM_STRIPE_CHARGE_BOTTOM_TEXT_EXISTING'));
        ...
        else
        {
            ...
            $layoutData->currencyCode = $currencyCode;
            $layoutData->langTag = JFactory::getLanguage()->getTag();
            $layoutData->bottomText = FText::_($params->get('stripe_charge_bottom_text_new',
                'PLG_FORM_STRIPE_CHARGE_BOTTOM_TEXT_NEW'));
            ...
            if ($formModel->failedValidation())
            {
                ...
                $layoutData->failedValidationMsg = FText::_($params->get('stripe_charge_failed_validation_text',
                    'PLG_FORM_STRIPE_CHARGE_FAILED_VALIDATION_TEXT'));
