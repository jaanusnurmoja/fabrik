...
class PlgFabrik_FormComment extends PlgFabrik_Form
{
    ...
    private function getAddCommentForm($reply_to = 0, $master = false)
    {
        ...
        $layoutData->name = trim($input->get('ide_people___voornaam', '',
                'cookie') . ' ' . $input->get('ide_people___achternaam', '', 'cookie'));
        ...
        if ($layoutData->wysiwyg)
        {
            ...
            $layoutData->editor = $editor->display($layoutData->id, '', '100%', '100%', $cols, $rows, $buttons,
                $layoutData->id);
        }
    ...
    private function writeComment($params, $comment)
    {
        ...
        $input = $this->app->input;
        $layoutData = new stdClass;
        $layoutData->insrc = FabrikHelperHTML::image("star_in.png", 'form', @$this->tmpl, [], true);
        ...
        if ($this->doThumbs())
        {
            ...
            $layoutData->thumbs = $thumb->render([]);
        }
        else
        {
    ...
    protected function addNotificationEvent($row)
    {
        ...
        $query->insert('#__{package}_notification_event')
            ->set(['event = ' . $event, 'user_id = ' . $userId, 'reference = ' . $ref, 'date_time = ' . $date]);
        $db->setQuery($query);
        ...
        {
            JLog::add('Couldn\'t save fabrik comment notification event: ' + $db->stderr(true), JLog::WARNING,
                'fabrik');
    ...
    protected function saveNotificationToPlugin($row, $comments)
    {
        ...
        if ((int)$params->get('comment-internal-notify') == 1 && $shouldSubscribe)
        {
            $query->insert('#__{package}_notification')
                ->set(['reason = ' . $db->q('commentor'), 'user_id = ' . $userId, 'reference = ' . $ref, 'label = ' . $label]);
            $db->setQuery($query);
            ...
            if (isset($rowData->$owner) && !empty($rowData->$owner))
            {
                ...
                if (is_numeric($userId))
                {
                    $query->clear->insert('#__{package}_notification')
                        ->set(['reason = ' . $db->q('owner'), 'user_id = ' . $userId, 'reference = ' . $ref, 'label = ' . $label]);
                    $db->setQuery($query);
                    ...
                    {
                        JLog::add('Couldn\'t save fabrik comment notification: ' + $db->stderr(true), JLog::WARNING,
                            'fabrik');
        ...
        if ($params->get('comment-notify-admins') == 1)
        {
            ...
            foreach ($rows as $row)
            {
                if ($row->id != $userId)
                {
                    $query->clear();
                    $fields = [
                        'reason = ' . $db->quote('admin observing a comment'), 'user_id = ' . $row->id, 'reference = ' . $ref,
                        'label = ' . $label
                    ];
                    ...
                    {
                        JLog::add('Couldn\'t save fabrik comment notification for admin: ' + $db->stderr(true),
                            JLog::WARNING, 'fabrik');
                    }
                }
            }
    ...
    protected function sentNotifications($row, $comments)
    {
        ...
        if ((int)$params->get('comment-internal-notify') == 1)
        {
            ...
            foreach ($comments as $comment)
            {
                ...
                {
                    FabrikWorker::sendMail($this->app->get('mailfrom'), $this->app->get('fromname'), $comment->email,
                        $title, $message, true);
        ...
        if (!empty($owner))
        {
            ...
            if (isset($rowData->$owner) && !empty($rowData->$owner))
            {
                ...
                {
                    FabrikWorker::sendMail($this->app->get('mailfrom'), $this->app->get('fromname'), $ownerEmail,
                        $title, $message, true);
        ...
        if ($params->get('comment-notify-admins') == 1)
        {
            ...
            foreach ($rows as $row)
            {
                ...
                {
                    FabrikWorker::sendMail($this->app->get('mailfrom'), $this->app->get('fromname'), $row->email,
                        $title, $message, true);
                    $sentTo[] = $row->email;
                }
            }
