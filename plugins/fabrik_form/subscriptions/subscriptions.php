...
class PlgFabrik_FormSubscriptions extends PlgFabrik_Form
{
    ...
    protected function getItemName()
    {
        ...
        return [$data['jos_fabrik_subs_users___plan_id_raw'], $data['jos_fabrik_subs_users___plan_id'][0]];
    }
    ...
    {
        $input = $this->app->input;
        JLog::add($input->server->getString('REQUEST_URI') . ' ' . http_build_query($_REQUEST), JLog::INFO,
            'fabrik.ipn.start');
                            ...
                            if ($status)
                            {
                                ...
                                if ($ipn !== false)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            $status = $ipn->$txn_type_function($listModel, $request, $set_list,
                                                $err_msg);
                                ...
                                if (!empty($set_list))
                                {
                                    ...
                                    $query = $db->getQuery(true);
                                    $query->update('#__fabrik_subs_invoices')->set(implode(',',
                                        $set_array))->where('id = ' . $db->quote($invoiceId));
                                    $db->setQuery($query);
