...
class FabrikSubscriptionsIPN
{
    ...
    public function __construct()
    {
        ...
        // Add the logger.
        JLog::addLogger(['text_file' => 'fabrik.subs.log.php']);
    }
    ...
    protected function activateSubscription($listModel, $request, &$set_list, &$err_msg, $recurring = true)
    {
        ...
        {
            $msg = $subUser->get('id') . ' set to ' . implode(', ',
    ...
    public function txn_type_subscr_cancel($listModel, $request, &$set_list, &$err_msg)
    {
        ...
        if (!$invoice)
        {
            $this->log('fabrik.ipn.txn_type_subscr_cancel invoice not found',
                $invoice . ' not found so didnt cancel a subscription');
            return false;
        }
        $sub = $this->getSubscriptionFromInvoice($invoice);
        if ($sub === false)
        {
            $this->log('fabrik.ipn.txn_type_subscr_cancel subscription not found',
                $invoice . ' not found so didnt cancel a subscription');
            return false;
        }
        $sub->cancel();
    ...
    public function txn_type_subscr_eot($listModel, $request, &$set_list, &$err_msg)
    {
        ...
        if ($sub->recurring != 1)
        {
            $this->log('fabrik.ipn.txn_type_subscr_eot',
                'not expiring as sub is not recurring (so eot is triggered on sub signup)');
            /*
    ...
    public function fallbackPlan($sub)
    {
        ...
        $this->log('fabrik.ipn. fallback',
            ' getting fallback sub plan :  ' . (int)$sub->plan . ' = ' . (int)$plan->fall_back_plan);
        $fallback = false;
        ...
        if ($fallback)
        {
            ...
            $mail->sendMail('rob@pollen-8.co.uk', 'fabrikar subs', 'rob@pollen-8.co.uk',
                'fabrikar sub: fall back plan created', $msg, true);
        }
    }
