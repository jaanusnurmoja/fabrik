...
class PlgFabrik_FormArticle extends PlgFabrik_Form
{
    ...
    protected function saveArticle($id, $catId)
    {
        ...
        $params = $this->getParams();
        $data = ['articletext' => $this->buildContent(), 'catid' => $catId, 'state' => 1, 'language' => '*'];
        $attributes = [
            'title'        => '',
            'publish_up'   => '',
            'publish_down' => '',
        ...
        $dispatcher->trigger('onContentBeforeSave', ['com_content.article', $item, $isNew]);
        ...
        $dispatcher->trigger('onContentAfterSave', ['com_content.article', $item, $isNew]);
    ...
    protected function findElementData($elementId, $default = '')
    {
        ...
        if ($elementModel = $formModel->getElement($elementId, true))
        {
            ...
            if (is_a($elementModel, 'PlgFabrik_ElementDate'))
            {
                ...
                $value = $elementModel->storeDatabaseFormat($value, []);
            }
        }
    ...
    protected function generateNewTitle($id, $catId, &$data)
    {
        ...
        while ($table->load(['alias' => $alias, 'catid' => $catId]))
        {
        ...
        if ($title <> $data['title'])
        {
            ...
            $listModel->updateRows([$pk], $titleField, $title);
        }
