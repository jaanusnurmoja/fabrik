...
class PlgFabrik_FormConfirmation extends PlgFabrik_Form
{
    ...
    public function onBeforeStore()
    {
        ...
        $session->set('com_' . $this->package . '.form.' . $formModel->getId() . '.session.hash',
            $sessionModel->getHash());

        // Set an error so we can reshow the same form for confirmation purposes
        $formModel->errors['confirmation_required'] = [FText::_('PLG_FORM_CONFIRMATION_PLEASE_CONFIRM_YOUR_DETAILS')];
