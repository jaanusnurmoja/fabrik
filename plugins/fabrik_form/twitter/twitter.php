...
class PlgFabrik_FormTwitter extends PlgFabrik_Form
{
    ...
    public function onTweet()
    {
        ...
        $connection = new TwitterOAuth($consumerKey, $consumerSecret, $_SESSION['oauth_token'],
        ...
        $connection = new TwitterOAuth($consumerKey, $consumerSecret, $access_token['oauth_token'],
    ...
    protected function sendTweet($connection)
    {
        ...
        $url = $this->session->get($context . 'url', [$url]);
        $url = array_shift($url);
    ...
    private function _process()
    {
        ...
        $this->session->set('com_' . $this->package . '.form.twitter.showmessage',
            $params->get('twitter-show-success-msg', 0));
    ...
    private function bitlifyMessage($msg)
    {
        ...
        $re = "#(?i)\b((?:[a-z][\w-]+:(?:/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)"
            . "(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'\".,<>?������]))#";
        $msg = preg_replace_callback($re, [&$this, 'bitlifyCallback'], $msg);
    ...
    public function onUpdateAdmin()
    {
        ...
        $counter = $input->get('repeatCounter');
        $pairs = [
            'twitter_oauth_token' => 'oauth_token', 'twitter_oauth_token_secret' => 'oauth_token_secret',
