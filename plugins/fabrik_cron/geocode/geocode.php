        ...
        foreach ($data as $gkey => $group)
        {
            if (is_array($group))
            {
                foreach ($group as $rkey => $row)
                {
                    ...
                    if ($geocode_from === '1')
                    {
                        ...
                        if ($do_geocode)
                        {
                            ...
                            if (!empty($full_addr))
                            {
                                ...
                                {
                                    $logMsg = sprintf('Error (%s), id %s , no geocode result for: %s', $res['status'],
                                        $row->$primary_key_element_long, $full_addr);
                            ...
                            else
                            {
                                FabrikWorker::log('plg.cron.geocode.information',
                                    'empty address, id = ' . $row->$primary_key_element_long);
                            }
                        }
                    }
                    // 2 - convert separate lat/lon elements to a map element, no geocoding
                    elseif ($geocode_from === '2')
                    {
                        $total_attempts++;
                    ...
                    elseif ($geocode_from === '4' || $geocode_from === '5')
                    {
                        if ($geocode_batch_limit > 0 && $total_attempts >= $geocode_batch_limit)
                        {
                            FabrikWorker::log('plg.cron.geocode.information', 'reached batch limit');
                        ...
                        if ($do_geocode)
                        {
                            ...
                            if (!empty($lat) && !empty($long))
                            {
                                ...
                                {
                                    $logMsg = sprintf('Error (%s), id %s , no geocode result for: %s', $res['status'],
                                        $row->$primary_key_element_long, $lat . "," . $long);
                                    FabrikWorker::log('plg.cron.geocode.information', $logMsg);
                                }
                            ...
                            {
                                FabrikWorker::log('plg.cron.geocode.information',
                                    'empty lat/lng, id = ' . $row->$primary_key_element_long);
                            }
                        }
                    }
