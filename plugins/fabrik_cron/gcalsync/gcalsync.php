    ...
    public function process(&$data, &$listModel)
    {
        ...
        if (preg_match('#feeds/(.*?)/(\w+-\w+|\w+)/(\w+)#', $gcal_url, $matches))
        {
            ...
            */
            ...
            if (
                empty($gcal_label_element_long) || empty($gcal_start_date_element_long) || empty($gcal_end_date_element_long)
            ...
            {
                throw new RuntimeException('Please install the Zend gdata from library http://framework.zend.com/download/gdata',
                    500);
            ...
            */
            ...
            foreach ($gcal_event_ids as $id => $event)
            {
                if (!array_key_exists($id, $our_event_ids))
                {
                    ...
                    $row = [];
                    $row[$gcal_start_date_element] = strftime('%Y-%m-%d %H:%M:%S',
                        strtotime($event->when[0]->startTime));
