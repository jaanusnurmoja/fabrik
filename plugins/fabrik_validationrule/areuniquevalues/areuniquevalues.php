...
class PlgFabrik_ValidationruleAreUniqueValues extends PlgFabrik_Validationrule
{
    ...
    public function validate($data, $repeatCounter)
    {
        ...
        $query->select('COUNT(*)')->from($lookupTable)->where($db->qn($elementModel->getFullName(false,
                false)) . ' = ' . $data);
        ...
        if (!empty($otherField))
        {
            ...
            $v = FArrayHelper::getValue($formData, $otherFullName . '_raw',
                FArrayHelper::getValue($formData, $otherFullName, ''));
        ...
        if (!empty($otherField2))
        {
            // $$$ the array thing needs fixing, for now just grab 0
            $formData = $elementModel->getForm()->formData;
            $v = FArrayHelper::getValue($formData, $otherFullName2 . '_raw',
                FArrayHelper::getValue($formData, $otherFullName2, ''));
