...
class plgCommunityFabrik extends CApplications
{
    ...
    function onProfileDisplay()
    {
        ...
        if (!empty($view) && !empty($id))
        {
            ...
            $callback = [$className, '_getFabrikHTML'];

            $content = $cache->call($callback, $view, $id, $rowid, $usekey, $layout, $element, $additional,
                $this->userparams, $user->id);
        }
        else
        {
    ...
    static function _getFabrikHTML($view, $id, $rowid, $usekey, $layout, $element, $additional, $params, $userId)
    {
        ...
        //echo $plugin_cmd;
        return $jsFabrik->jsFabrikRender(["{$plugin_cmd}"]);
    }
}
