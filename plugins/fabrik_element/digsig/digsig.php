...
class PlgFabrik_ElementDigsig extends PlgFabrik_Element
{
    ...
    public function render($data, $repeatCounter = 0)
    {
        ...
        if (!$this->isEditable())
        {
            if ($format === 'pdf')
            {
                ...
                $layout = new JLayoutFile('fabrik-element-digsig-details-pdf', $basePath,
                    ['debug' => false, 'component' => 'com_fabrik', 'client' => 'site']);
            }
            else
            {
                $layout = new JLayoutFile('fabrik-element-digsig-details', $basePath,
                    ['debug' => false, 'component' => 'com_fabrik', 'client' => 'site']);
            }
        ...
        {
            $layout = new JLayoutFile('fabrik-element-digsig-form', $basePath,
                ['debug' => false, 'component' => 'com_fabrik', 'client' => 'site']);
        }
    ...
    public function onAjax_signature_to_image()
    {
        ...
        $opts = [
            'imageSize' => [$digsig_width, $digsig_height]
        ];
    ...
    public function formJavascriptClass(&$srcs, $script = '', &$shim = [])
    {
        ...
        $digsigShim->deps = [$folder . 'jquery.signaturepad'];
