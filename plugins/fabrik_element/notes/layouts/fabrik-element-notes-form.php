...
if ($d->canUse) :
    ...
    if ($d->primaryKey > 0) :
        if ($d->fieldType == 'field') :?>
            <input class="fabrikinput inputbox text span12" name="<?php
            echo $d->name; ?>"/>
        <?php
        else:
            ?>
            <textarea class="fabrikinput inputbox text span12" name="<?php
            echo $d->name; ?>" cols="50" rows="3"/></textarea>
        <?php
        endif;
        ?>

        <input type="button" class="button btn" value="<?php
        echo FText::_('PLG_ELEMENT_NOTES_ADD'); ?>"/>
    <?php
    else :
