...
class PlgFabrik_ElementCascadingdropdown extends PlgFabrik_ElementDatabasejoin
{
    ...
    public function elementJavascript($repeatCounter)
    {
        ...
        if ($this->getDisplayType() === 'auto-complete')
        {
            ...
            FabrikHelperHTML::autoComplete($id, $this->getElement()->id, $this->getFormModel()->getId(),
                'cascadingdropdown', $autoOpts);
        }
        ...
        $qsValue = $input->get($fullName, [], 'array');
        $qsValue = FArrayHelper::getValue($qsValue, 0, null);
        $qsWatchValue = $input->get($watchName, [], 'array');
        ...
        $opts->def = $useQsValue ? $qsValue : $this->getValue([], $repeatCounter);
        ...
        $opts->autoCompleteOpts = $opts->displayType == 'auto-complete'
            ? FabrikHelperHTML::autoCompleteOptions($opts->id, $this->getElement()->id, $formId,
                'cascadingdropdown') : null;
        $this->elementJavascriptJoinOpts($opts);

        $data = $this->getFormModel()->data;

        // Was otherwise using the none-raw value.
        $opts->value = $this->getValue($data, $repeatCounter, ['raw' => true]);
        ...
        return ['FbCascadingdropdown', $id, $opts];
    }
    ...
    public function getJoinLabelColumn($useStep = false)
    {
        ...
        if (
            ($params->get('cascadingdropdown_label_concat') != '') && $this->app->input->get('override_join_val_column_concat',
                '0') !== '1'
        )
        {
    ...
    public function render($data, $repeatCounter = 0)
    {
        ...
        $imageOpts = ['alt' => FText::_('PLG_ELEMENT_CALC_LOADING'), 'style' => 'display:none;padding-left:10px;', 'class' => 'loader'];
            ...
            switch ($displayType)
            {
                ...
                case 'dropdown':
                    ...
                    else
                    {
                        ...
                        $maxWidth = $params->get('max-width',
                            '') === '' ? '' : ' style="max-width:' . $params->get('max-width') . ';"';
                        $advancedClass = $this->getAdvancedSelectClass();
                        $attributes = 'class="' . $class . ' ' . $advancedClass . '" ' . $disabled . ' size="1"' . $maxWidth;
                        $html[] = JHTML::_('select.genericlist', $tmp, $name, $attributes, 'value', 'text', $default,
                            $id);
                    }
                    break;
            }
    ...
    protected function _getOptionVals($data = [], $repeatCounter = 0, $incWhere = true, $opts = [])
    {
        ...
        if ($filterView == 'table')
        {
            array_unshift($this->optionVals[$sqlKey],
                JHTML::_('select.option', $params->get('cascadingdropdown_noselectionvalue', ''),
        ...
        if ($this->canUse() && in_array($this->getDisplayType(), ['multilist', 'dropdown']))
        {
            foreach ($this->optionVals[$sqlKey] as $key => &$opt)
            {
    ...
    {
        $params = $this->getParams();
        return JHTML::_('select.option', $params->get('cascadingdropdown_noselectionvalue', ''),
    ...
    protected function showPleaseSelect()
    {
        ...
        if (in_array($this->getDisplayType(), ['checkbox', 'multilist', 'radio']))
        {
            return false;
        }
    ...
    protected function getWatchElement()
    {
        if (!isset($this->watchElement))
        {
            ...
            if (!$this->watchElement)
            {
                ...
                if (!$this->watchElement)
                {
                    throw new RuntimeException('No watch element found for cdd: ' . $this->getElement()->id . ', trying to find children of: ' . $watch,
                        500);
                }
            }
        }
    ...
    protected function buildQuery($data = [], $incWhere = true, $opts = [])
    {
        ...
        else
        {
            if (isset($formModel->data) || isset($formModel->formData))
            {
                ...
                else
                {
                     ...
                     */
                    if (
                        is_array($formModel->formDataWithTableName) && array_key_exists($watch,
                            $formModel->formDataWithTableName)
                    )
                    {
                        $whereVal = $watchElement->getValue($formModel->formDataWithTableName, $repeatCounter,
                            $watchOpts);
                    }
                    else
                    {
        ...
        if (!is_null($whereVal) && $whereKey != '')
        {
            ...
            if (is_array($whereVal))
            {
                ...
                $where .= count($whereVal) == 0 || !preg_match('/\w/',
                    implode(',', $whereVal)) ? '4 = -4' : $whereKey . ' IN ' . '(' . str_replace(',,', ',\'\',',
                        implode(',', $whereVal)) . ')';
            }
            else
            {
        ...
        $placeholders = is_null($whereVal) ? [] : ['whereval' => $whereVal, 'wherekey' => $whereKey];
        $join = $this->getJoin();
    ...
    protected function getLabelOrConcatVal()
    {
        ...
        else
        {
            ...
            $val = $w->parseMessageForPlaceHolder($val, []);
    ...
    public function finalCopyCheck($elementMap)
    {
        ...
        {
            throw new RuntimeException('cascade dropdown: no id ' . $oldObserverId . ' found in ' . implode(",",
                    array_keys($elementMap)));
        }
    ...
    {
        return ['cascadingdropdown_observe'];
    }
