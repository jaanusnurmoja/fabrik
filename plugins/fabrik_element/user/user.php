...
class PlgFabrik_ElementUser extends PlgFabrik_ElementDatabasejoin
{
    ...
    public function render($data, $repeatCounter = 0)
    {
        ...
        if ($params->get('update_on_edit') || !$rowId)
        {
            ...
            {
                $userId = $this->getValue($data, $repeatCounter, ['raw' => 1]);
        ...
        elseif ($this->inRepeatGroup && $this->newGroup)
        {
        ...
        else
        {
            ...
            else
            {
                ...
                if ($id === '')
                {
                    $id = $this->getValue($data, $repeatCounter, ['raw' => 1]);
                }
        ...
        */
        ...
        else
        {
            if ($this->updateOnEdit())
            {
                ...
                {
                    $formModel->formDataWithTableName[$thisFullName][$repeatCounter] = [$data[$element->name]];
                    $formModel->formDataWithTableName[$thisFullName . '_raw'][$repeatCounter] = [$data[$element->name]];
                }
                else
                {
                    $formModel->formDataWithTableName[$thisFullName] = [$data[$element->name]];
                    $formModel->formDataWithTableName[$thisFullName . '_raw'] = [$data[$element->name]];
                }
            ...
            else
            {
                ...
                elseif ($this->getGroupModel()->canRepeat())
                {
    ...
    public function getValue($data, $repeatCounter = 0, $opts = [])
    {
        ...
        if (empty($data) || !array_key_exists($key, $data))
        {
            ...
            if (!in_array($input->get('task'), ['processForm', 'view', '', 'form.process', 'process']))
            {
                return '';
            }
    ...
    public function getFilter($counter = 0, $normal = true, $container = '')
    {
        ...
        if (in_array($element->filter_type, ['range', 'dropdown', '', 'checkbox']))
        {
    ...
    protected function getLabelOrConcatVal()
    {
        ...
        if ($displayParam == 'gid')
        {
            ...
            {
                $this->app->enqueueMessage(JText::sprintf('PLG_ELEMENT_USER_NOTICE_GID', $this->getElement()->id),
                    'notice');
                $displayMessage = true;
            }
        }
