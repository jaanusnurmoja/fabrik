...
class PlgFabrik_ElementTextarea extends PlgFabrik_Element
{
    ...
    protected function tagify($data)
    {
        ...
        $icon = FabrikHelperHTML::image($img, 'form', @$this->tmpl, ['alt' => 'tag']);
        ...
        if (!$this->isEditable())
        {
            ...
            else
            {
                ...
                if (
                    $value !== ''
                    &&
                    ((int)$params->get('textarea-truncate-where',
                            0) === 2 || (int)$params->get('textarea-truncate-where', 0) === 3)
                )
                {
