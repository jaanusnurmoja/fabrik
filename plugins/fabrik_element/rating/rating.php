...
class PlgFabrik_ElementRating extends PlgFabrik_Element
{
    ...
    protected function getRatingAverage($data, $listId, $formId, $rowId, $ids = [])
    {
        ...
        if (!isset($this->avgs))
        {
            ...
            $query->select('row_id, AVG(rating) AS r, COUNT(rating) AS total')->from(' #__{package}_ratings')
                ->where(['rating <> -1', 'listid = ' . (int)$listId, 'formid = ' . (int)$formId, 'element_id = ' . (int)$elementId]);
    ...
    protected function getCreatorId($listId, $formId, $rowId, $ids = [])
    {
    ...
    protected function canRate($rowId = null, $ids = [])
    {
        $params = $this->getParams();
    ...
    public function render($data, $repeatCounter = 0)
    {
        ...
        $imgOpts = ['icon-class' => 'small', 'style' => $css, 'data-rating' => -1];
    ...
    public function elementJavascript($repeatCounter)
    {
        ...
        if (!FabrikWorker::j3())
        {
            $opts->insrc = FabrikHelperHTML::image("star.png", 'form', @$this->tmpl, [], true);
            $opts->outsrc = FabrikHelperHTML::image("star-empty.png", 'form', @$this->tmpl, [], true);
            $opts->clearoutsrc = $clearsrc = FabrikHelperHTML::image("remove-sign-out.png", 'form', @$this->tmpl, [],
                true);
            $opts->clearinsrc = $clearsrc = FabrikHelperHTML::image("remove-sign.png", 'form', @$this->tmpl, [], true);
        }
