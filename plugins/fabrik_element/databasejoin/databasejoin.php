...
class PlgFabrik_ElementDatabasejoin extends PlgFabrik_ElementList
{
    ...
    protected function filterValueList_Exact($normal, $tableName = '', $label = '', $id = '', $incjoin = true)
    {
        ...
        else
        {
            ...
            if ($this->app->input->get('method') === 'autocomplete_options')
            {
				...
				*/
                $params = $this->getParams();
                $this->autocomplete_where = $this->_autocompleteWhere($params->get('dbjoin_autocomplete_how',
                    'contains'), $label, $v);
    ...
    public function getJoinLabelColumn($useStep = false)
    {
        ...
        if (is_object($join) && ($params->get($this->concatLabelParam) != ''))
        {
            if ($this->app->input->get('override_join_val_column_concat', '0') !== '1')
            {
                ...
                $w = new FabrikWorker;
                $val = $w->parseMessageForPlaceHolder($val, [], false, false, null, false);
    ...
    protected function getJoin()
    {
        ...
        else
        {
            ...
            if ($this->join->load(['element_id' => $element->get('id')]))
            {
                if (is_string($this->join->params))
                {
                    $this->join->params = new Registry($this->join->params);
        ...
        $data = ['id' => 0, 'group_id' => $element->get('group_id')];
        $this->join = $this->updateFabrikJoin($data, $element->get('id'), $this->getDbName(),
            $this->getJoinValueFieldName(), $this->getLabelParamVal());
		 ...
		 */
        if (!in_array($input->get('task'), ['inlineedit', 'form.inlineedit']) && $input->get('format') !== 'raw')
        {
     ...
     */
    protected function _getOptionVals($data = [], $repeatCounter = 0, $incWhere = true, $opts = [])
    {
        $params = $this->getParams();
        $db = $this->getDb();
        ...
        $db->setQuery($sql);
        FabrikHelperHTML::debug((string)$db->getQuery(),
            $this->getElement()->name . ' databasejoin element: get options query');
     ...
     */
    protected function _getOptions($data = [], $repeatCounter = 0, $incWhere = true, $opts = [])
    {
        $params = $this->getParams();
        $this->getDb();
        $tmp = [];
        ...
        {
            array_unshift($tmp, JHTML::_('select.option', $params->get('database_join_noselectionvalue', ''),
     ...
     */
    protected function buildQuery($data = [], $incWhere = true, $opts = [])
    {
     ...
     */
    protected function buildQueryWhere($data = [], $incWhere = true, $thisTableAlias = null, $opts = [], $query = false)
    {
        $params = $this->getParams();
        $element = $this->getElement();
        ...
        {
            $where = ($this->mustApplyWhere($whereAccess,
                    $element->id) && $incWhere) ? $params->get('database_join_where_sql') : '';
        }
        ...
        if (!empty($this->autocomplete_where))
        {
            ...
            if (
                ($mode == 'filter' && $filterType == 'auto-complete')
                || ($mode == 'form' && $displayType == 'auto-complete')
                || ($mode == 'filter' && $displayType == 'auto-complete')
            )
            {
                $where .= JString::stristr($where,
                    'WHERE') ? ' AND ' . $this->autocomplete_where : ' WHERE ' . $this->autocomplete_where;
            }
        }
    ...
    protected function getLabelOrConcatVal()
    {
        ...
        if (
            $params->get($this->concatLabelParam) == '' || $this->app->input->get('override_join_val_column_concat',
                '0') === '1'
        )
        {
    ...
    public function render($data, $repeatCounter = 0)
    {
        ...
        if ($this->isJoin() && in_array($displayType, ['checkbox', 'multilist']))
        {
            $this->hasSubElements = true;
        }

        $default = (array)$this->getValue($data, $repeatCounter, ['raw' => true]);
        $defaultLabels = (array)$this->getValue($data, $repeatCounter, ['raw' => false]);
        ...
        if (!$formModel->isEditable() || !$this->isEditable())
        {
            ...
            if (
                FArrayHelper::getValue($defaultLabels, 0) === $params->get('database_join_noselectionlabel',
                    FText::_('COM_FABRIK_PLEASE_SELECT'))
            )
            {
			...
			*/
            ...
            $displayData->control = count($defaultLabels) < 2 ? implode(' ',
                $defaultLabels) : '<ul><li>' . implode('<li>', $defaultLabels) . '</li></ul>';
            ...
            if ($this->canUse())
            {
                ...
                switch ($displayType)
                {
                    ...
                    case 'radio':
                        if ($this->isJoin())
                        {
                            $this->renderCheckBoxList($data, $repeatCounter, $html, $tmp,
                                FArrayHelper::getValue($default, 0));
                        }
                        else
                        {
                            $this->renderRadioList($data, $repeatCounter, $html, $tmp,
                                FArrayHelper::getValue($default, 0));
                        }
                        break;
                    case 'checkbox':
     ...
     * @return  void
     */
    protected function renderDescription($options = [], $default = [])
    {
        $params = $this->getParams();
        ...
        if ($params->get('join_desc_column', '') !== '')
        {
            ...
            FabrikHelperHTML::jLayoutJs($descDivLayout, $descDivLayout, $displayData, [$this->layoutBasePath()]);
        ...
        if ($this->app->input->get('print', '') !== '1' && $params->get('databasejoin_readonly_link') == 1)
        {
            if ($popUrl = $this->popUpFormUrl())
            {
                for ($i = 0; $i < count($defaultLabels); $i++)
                {
                    ...
                    $url = $popUrl . $value;
                    $defaultLabels[$i] = '<a href="' . JRoute::_($url) . '">' . FArrayHelper::getValue($defaultLabels,
                            $i) . '</a>';
                }
            }
        }
    ...
    protected function renderDropdownList($data, $repeatCounter, &$html, $tmp, $defaultValue)
    {
        ...
        if (FabrikWorker::j3())
        {
            ...
            $displayData->editable = $this->isEditable();
            $displayData->attributes = 'class="fabrikinput form-control inputbox input ' . $this->getAdvancedSelectClass() . ' ' . $params->get('bootstrap_class',
                    'input-large') . '" size="1"';
        ...
        else
        {
            ...
            $attributes = 'class="fabrikinput inputbox input ' . $advancedClass . ' ' . $params->get('bootstrap_class',
                    'input-large') . '" size="1"';
    ...
    protected function renderRadioList($data, $repeatCounter, &$html, $tmp, $defaultValue)
    {
        ...
        FabrikHelperHTML::jLayoutJs($singleLayout . '_' . $id, $singleLayout, $displayData, [$this->layoutBasePath()]);
        $rowOptsLayout = 'fabrik-element-' . $this->getPluginName() . '-form-rowopts';
        FabrikHelperHTML::jLayoutJs($rowOptsLayout, $rowOptsLayout, $displayData, [$this->layoutBasePath()]);
        ...
        {
            $html[] = FabrikHelperHTML::aList('radio', $tmp, $thisElName, $attributes, $defaultValue, 'value', 'text',
                $displayData->optsPerRow, $displayData->editable);
        }
    ...
    protected function renderAutoComplete($data, $repeatCounter, &$html, $default)
    {
        ...
        $label = (array)$this->getValue($data, $repeatCounter, ['valueFormat' => 'label']);
		...
		*/
        ...
        $class = ' class="fabrikinput inputbox autocomplete-trigger ' . $params->get('bootstrap_class',
                'input-large') . '"';
        ...
        $html[] = '<input type="text" size="' . $params->get('dbjoin_autocomplete_size',
                '20') . '" name="' . $autoCompleteName . '" id="' . $id
            . '-auto-complete" value="' . htmlspecialchars(FArrayHelper::getValue($label,
                0)) . '"' . $class . $placeholder . '/>';
    ...
    protected function renderMultiSelectList($data, $repeatCounter, &$html, $tmp, $default)
    {
        ...
        else
        {
            ...
            $html[] = FabrikHelperHTML::aList('multilist', $tmp, $elName, $attributes, $default, 'value', 'text',
                $optsPerRow, $this->isEditable());
        }
    }
    ...
    protected function renderCheckBoxList($data, $repeatCounter, &$html, $tmp, $default)
    {
        ...
        {
            $html[] = FabrikHelperHTML::aList($type, $tmp, $name, $attributes, $default, 'value', 'text',
                $displayData->optsPerRow, $displayData->editable);
        }
     ...
     */
    public function getTitlePart($data, $repeatCounter = 0, $opts = [])
    {
        // Get raw value
        $opts['raw'] = '1';
     ...
     */
    public function getEmailValue($value, $data = [], $repeatCounter = 0)
    {
        /**
         ...
         */
        $tmp = $this->_getOptions($data, $repeatCounter, true, ['emailValue' => true]);
    ...
    protected function listPreformat(&$data, $thisRow)
    {
        ...
        $values = isset($thisRow->$raw) ? FabrikWorker::JSONtoData($thisRow->$raw, true) : [];
    ...
    {
        $match = $this->isExactMatch(['match' => $this->getElement()->filter_exact_match]);
    ...
    public function getFilter($counter = 0, $normal = true, $container = '')
    {
        ...
        $this->filterDisplayValues = [$default];
        $rows = [];

        if (in_array($element->get('filter_type'), ['range', 'dropdown', '', 'checkbox', 'multiselect']))
        {
            ...
            foreach ($rows as &$r)
            {
                ...
                // Encode if necessary
                if (!in_array($element->get('filter_type'), ['checkbox']))
                {
            ...
            if (!in_array($element->filter_type, ['checkbox', 'multiselect']))
            {
    ...
    protected function filterValueList_All($normal, $tableName = '', $label = '', $id = '', $incjoin = true)
    {
		...
		*/
        ...
        $where = $this->buildQueryWhere([], true, null, ['mode' => 'filter']);
    ...
    public function getFilterValue($value, $condition, $eval)
    {
        ...
        {
            $stringComparisons = ['begins', 'contains', 'equals', 'ends'];
    ...
    protected function checkboxRows($groupBy = null, $condition = null, $value = null, $where = null, $offset = 0, $limit = 0)
    {
        ...
        $this->buildQueryWhere([], true, $tableAlias, ['mode' => 'filter'], $query);
    ...
    public function elementJavascript($repeatCounter)
    {
        ...
        if ($this->getParams()->get('database_join_display_type', 'dropdown') == 'auto-complete')
        {
            ...
            FabrikHelperHTML::autoComplete($id, $this->getElement()->get('id'), $this->getFormModel()->getId(),
                'databasejoin', $autoOpts);
        }

        $opts = $this->elementJavascriptOpts($repeatCounter);

        return ['FbDatabasejoin', $id, $opts];
    }
    ...
    protected function elementJavascriptOpts($repeatCounter = 0)
    {
        ...
        $arSelected = $this->getValue($data, $repeatCounter, ['raw' => true]);
        $table = $params->get('join_db_name');
        ...
        $opts->autoCompleteOpts = $opts->displayType == 'auto-complete'
            ? FabrikHelperHTML::autoCompleteOptions($opts->id, $this->getElement()->get('id'),
                $this->getFormModel()->getId(), 'databasejoin') : null;
    ...
    public function onSave($data)
    {
        ...
        if (!$this->isJoin())
        {
            $this->updateFabrikJoins($data, $this->getDbName(), $this->getJoinValueFieldName(),
                $this->getLabelParamVal());
        }
            ...
            if (is_numeric($id))
            {
                if ($id == '')
                {
                    JError::raiseWarning(500,
                        'Unable to get table for cascading dropdown (ignore if creating a new element)');
    ...
    protected function updateFabrikJoin($data, $elementId, $tableJoin, $keyCol, $label)
    {
		...
		*/
        if ($data['id'] == 0)
        {
            $key = ['element_id' => $data['id'], 'list_id' => 0];
        }
        else
        {
            $key = ['element_id' => $data['id']];
        }
     ...
     *
     */
    public static function cacheAutoCompleteOptions($elementModel, $search, $opts = [])
    {
        ...
        // default to "contains" for backward compat.
        $elementModel->autocomplete_where = $elementModel->_autocompleteWhere($params->get('dbjoin_autocomplete_how',
            'contains'), $c, $search);
        $opts = ['mode' => 'filter'];
        $tmp = $elementModel->_getOptions([], 0, true, $opts);
    ...
    {
        if (in_array($this->getDisplayType(), ['checkbox', 'multilist']))
        {
            return true;
        }
        else
    ...
    public function buildQueryElementConcat($jKey, $addAs = true)
    {
        ...
        $params = $this->getParams();
        $jKey = $this->getLabelOrConcatVal();
        $where = $this->buildQueryWhere([], true, $params->get('join_db_name'));
    ...
    public function getJoinRepeatCount($data, $oJoin)
    {
        ...
        if ($displayType === 'multilist')
        {
            ...
            return count(FArrayHelper::getValue($data, $repeatName, []));
        }
        else
        {
    ...
    public function canIncludeInSearchAll($advancedMode)
    {
        if ($advancedMode)
        {
            ...
            $field = ArrayHelper::fromObject(FArrayHelper::getValue($fields, $this->getLabelParamVal(), []));
            $type = FArrayHelper::getValue($field, 'Type', '');
            $notAllowed = ['int', 'double', 'decimal', 'date', 'serial', 'bit', 'boolean', 'real'];
