...
class PlgFabrik_ElementGooglemap extends PlgFabrik_Element
{
    ...
    public function renderListData($data, stdClass &$thisRow, $opts = [])
    {
        ...
        foreach ($data as $i => &$d)
        {
            ...
            else
            {
                if (!$params->get('fb_gm_staticmap_tableview'))
                {
                    $d = $params->get('fb_gm_staticmap_tableview_type_coords',
                        'num') == 'dms' ? $this->_dmsformat($d) : $this->_microformat($d);
                }
            }
    ...
    public function elementJavascript($repeatCounter)
    {
        ...
        $opts->auto_center = (bool)$params->get('fb_gm_auto_center', false) && (bool)$params->get('fb_gm_draggable',
                '1');
        $opts->styles = Googlemap::styleJs($params);

        if ($opts->geocode == '2')
        {
            foreach (['addr1', 'addr2', 'city', 'state', 'zip', 'country'] as $which_field)
            {
                $field_id = '';
        ...
        $opts->radius_write_element = $opts->use_radius ? $this->_getFieldId('fb_gm_radius_write_element',
            $repeatCounter) : false;
        $opts->radius_read_element = $opts->use_radius ? $this->_getFieldId('fb_gm_radius_read_element',
            $repeatCounter) : false;
        $opts->radius_ro_value = $opts->use_radius ? $this->_getFieldValue('fb_gm_radius_read_element', $data,
            $repeatCounter) : false;
    ...
    protected function _staticMap($v, $w = null, $h = null, $z = null, $repeatCounter = 0, $tableView = false, $data = [])
    {
        $id = $this->getHTMLId($repeatCounter);
        $params = $this->getParams();
        ...
        if (
            (!$tableView && $params->get('fb_gm_staticmap') == '1') || ($tableView && $params->get('fb_gm_staticmap_tableview',
                    '0') === '1')
        )
        {
     ...
     */
    protected function getIndEmailValue($value, $data = [], $repeatCounter = 0)
    {
