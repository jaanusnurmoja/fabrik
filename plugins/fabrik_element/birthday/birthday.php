...
class PlgFabrik_ElementBirthday extends PlgFabrik_Element
{
    ...
    public function render($data, $repeatCounter = 0)
    {
         ...
         */
        $aNullDates = ['0000-00-000000-00-00', '0000-00-00 00:00:00', '0000-00-00', '', $this->_db->getNullDate()];
        ...
        $daySys = ['01', '02', '03', '04', '05', '06', '07', '08', '09'];
        $daySimple = ['1', '2', '3', '4', '5', '6', '7', '8', '9'];
    ...
    {
        return [
            FText::_('January'), FText::_('February'), FText::_('March'), FText::_('April'), FText::_('May'), FText::_('June'),
    ...
    private function _yearOptions()
    {
        $params = $this->getParams();
        $years = [
            JHTML::_('select.option', '', FText::_($params->get('birthday_yearlabel', 'PLG_ELEMENT_BIRTHDAY_YEAR')))
        ];
        $years = [
            JHTML::_(
                'select.option',
    ...
    public function renderListData($data, stdClass &$thisRow, $opts = [])
    {
         ...
         */
        $data = $groupModel->isJoin() ? FabrikWorker::JSONtoData($data, true) : [$data];
        $data = (array)$data;
        $format = [];
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (
                                in_array(substr(($thisYear - $year), -1),
                                    $jubileum) || in_array(substr(($thisYear - $year), -2), $jubileum)
                            )
                            {
                        ...
                        {
                            if (
                                in_array(substr(($nextYear - $year), -1),
                                    $jubileum) || in_array(substr(($nextYear - $year), -2), $jubileum)
                            )
                            {
    ...
    public function getFilter($counter = 0, $normal = true, $container = '')
    {
        ...
        if (
            $element->filter_type === 'dropdown' && $params->get('list_filter_layout', 'individual') === 'day_mont_year'
        )
        {
            ...
            $this->filterDisplayValues = [$layoutData->default];

            $return = [];
            $return[] = $layout->render($layoutData);
            $return[] = $normal ? $this->getFilterHiddenFields($counter, $elName, false,
                $normal) : $this->getAdvancedFilterHiddenFields();
        ...
        if ($type === 'prefilter' || $type === 'menuPrefilter')
        {
            switch ($condition)
            {
                ...
                case 'laterthisyear':
                    ...
                    break;
                case 'today':
                    $search = [date('Y'), date('n'), date('j')];
                    return $this->_dayMonthYearFilterQuery($key, $search);
                    break;
                case 'yesterday':
                    $today = new DateTime();
                    $today->sub(new DateInterval('P1D'));
                    $search = ['', $today->format('n'), $today->format('j')];
                    return $this->_dayMonthYearFilterQuery($key, $search);
                    break;
                case 'tomorrow':
                    $today = new DateTime();
