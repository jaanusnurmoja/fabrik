...
class PlgFabrik_ElementThumbs extends PlgFabrik_Element
{
    ...
    protected function getThumbsCount($data, $listId, $formId, $rowId)
    {
        ...
        return json_encode([$resup, $resdown]);
    }
    ...
    public function render($data, $repeatCounter = 0)
    {
        ...
        if (
            $input->get('view') == 'form' && ((bool)$params->get('rate_in_from',
                    false) === false || $this->getFormModel()->isNewRecord())
        )
        {
            return '';
        }
    ...
    public function elementJavascript($repeatCounter)
    {
        ...
        $opts->noAccessMsg = trim(FText::_($params->get('thumbs_no_access_msg',
            FText::_('PLG_ELEMENT_THUMBS_NO_ACCESS_MSG_DEFAULT'))));
        $opts->row_id = $rowId;
    ...
    public function elementListJavascript()
    {
        ...
        $opts = new stdClass;
        $opts->canUse = $this->canUse();
        $opts->noAccessMsg = FText::_($params->get('thumbs_no_access_msg',
            FText::_('PLG_ELEMENT_THUMBS_NO_ACCESS_MSG_DEFAULT')));
        $opts->listid = $list->id;
        $opts->formid = $this->getFormModel()->getId();
