...
class PlgFabrik_ElementImage extends PlgFabrik_Element
{
     ...
     * @var array
     */
    protected $ignoreFolders = ['cache', 'lib', 'install', 'modules', 'themes', 'upgrade', 'locks', 'smarty', 'tmp'];
    ...
    public function getDefaultValue($data = [])
    {
        if (!isset($this->default))
        {
            ...
            if ($element->eval == "1")
            {
                ...
                FabrikWorker::logEval($this->default,
                    'Caught exception on eval in ' . $element->name . '::getDefaultValue() : %s');
            }
        }
    ...
    public function renderListData($data, stdClass &$thisRow, $opts = [])
    {
        ...
        if ($data === '' || empty($data) || !$pathset)
        {
            ...
            if (!strstr($iPath, '/'))
            {
                ...
                $data = (array)FabrikHelperHTML::image($iPath, 'list', @$this->tmpl, [], true);
            }
            else
            {
                $data = (array)$iPath;
            ...
            else
            {
                ...
                foreach ($data[$key] as $k => $v)
                {
                    $retvals[] = preg_replace("#^$selectImage_root_folder#", '',
                            $data[$key . '_folder'][$k]) . $data[$key . '_image'][$k];
                }
