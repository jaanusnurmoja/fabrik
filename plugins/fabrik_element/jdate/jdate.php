...
class PlgFabrik_ElementJdate extends PlgFabrik_ElementList
{
    ...
    public function render($data, $repeatCounter = 0)
    {
        ...
        $format = $params->get('jdate_form_format', $params->get('jdate_table_format', 'Y-m-d'));
        ...
        else
        {
            ...
            $time = ($params->get('jdate_showtime', 0)) ? ' ' . $localDate->format($timeFormat, true) : '';
        ...
        // Build HTML widget
        if ($params->get('jdate_showtime', 0) && !$element->hidden)
        {
        ...
        $class = 'fabrikinput inputbox input ' . $params->get('bootstrap_class', 'input-medium');
        $element->width = (int)$element->width < 0 ? 1 : (int)$element->width;
        $calOpts = ['class' => $class, 'size' => $element->width, 'maxlength' => '19'];
        ...
        {
            $calOpts['showTime'] = '1';
            $calOpts['timeFormat'] = $params->get('jdate_time_24', '1') ? '24' : '12';
            $format .= ' ' . $timeFormat;
        }
    ...
    protected function timeButton($timeElName, $time, &$str)
    {
        ...
        $str[] = '<input type="text" class="' . $class . '" ' . $readOnly . ' size="' . $timeLength . '" value="' . $time . '" name="'
            . $timeElName . '" />';
        $opts = ['alt' => FText::_('PLG_ELEMENT_JDATE_TIME'), 'class' => 'timeButton'];
        ...
        $layoutData = (object)[
            ...
            'aria'  => ' aria-label="' . FText::_('PLG_ELEMENT_JDATE_ARIA_LABEL_TIME') . '"'
        ];
    ...
    protected function shouldApplyTz($view = 'form')
    {
        ...
        $params = $this->getParams();
        $storeAsLocal = (bool)$params->get('jdate_store_as_local', 0);
    ...
    private function _indStoreDBFormat($val)
    {
        ...
        if ($listModel->importingCSV && $params->get('jdate_csv_offset_tz', '0') == '1')
        {
            return $val;
        }
    ...
    public function elementJavascript($repeatCounter)
    {
        ...
        $opts->showtime = (!$element->hidden && $params->get('jdate_showtime', 0)) ? true : false;
        $opts->timelabel = FText::_('PLG_ELEMENT_JDATE_TIME_LABEL', true);
        $opts->typing = (bool)$params->get('jdate_allow_typing_in_field', true);
        $opts->timedisplay = $params->get('jdate_timedisplay', 1);
        //$opts->dateTimeFormat = $params->get('jdate_time_format', '');
    ...
    public function getValidationWatchElements($repeatCounter)
    {
        $return = [];
        $id = $this->getHTMLId($repeatCounter);
        $return[] = ['id' => $id, 'triggerEvent' => 'blur'];
    ...
    protected function formatContainsTime($format)
    {
        ...
        $times = ['%H', '%I', '%l', '%M', '%p', '%P', '%r', '%R', '%S', '%T', '%X', '%z', '%Z'];
    ...
    public function getFilterValue($value, $condition, $eval)
    {
        ...
        $exactTime = $this->formatContainsTime($params->get('jdate_table_format'));
        ...
        switch ($filterType)
        {
            ...
            default:
                ...
                {
                    if (!$params->get('jdate_showtime', 0) || $exactTime == false)
                    {
    ...
    {
        $months = [
            FText::_('JANUARY_SHORT'), FText::_('JANUARY'), FText::_('FEBRUARY_SHORT'), FText::_('FEBRUARY'), FText::_('MARCH_SHORT'),
    ...
    public function getFilter($counter = 0, $normal = true, $container = '')
    {
        ...
        if (in_array($fType, ['dropdown', 'checkbox', 'multiselect']))
        {
    ...
    public function getFilterRO($data)
    {
        ...
        {
            foreach (explode(' AND ', FArrayHelper::getValue($data, $this->getFullName(true, false), [])) as $d)
            {
                $return[] = $this->getROElement(trim($d, "'"));
            }

            return FText::_('COM_FABRIK_BETWEEN') . '<br />' . implode('<br />' . FText::_('COM_FABRIK_AND') . "<br />",
                    $return);
        }
    ...
    protected function singleFilter($default, $v, $type = 'text')
    {
        ...
        $imageOpts = $displayData->j3 ? ['alt' => 'calendar'] : [
            'alt'   => 'calendar',
        ...
        $displayData->jCal = $this->calendar($rawValue, $v, $this->getFilterHtmlId(0), $format,
    ...
    protected function rangeFilter($default, $v)
    {
        ...
        $imageOpts = $displayData->j3 ? ['alt' => 'calendar'] : ['alt' => 'calendar', 'class' => 'calendarbutton', 'id' => $from->id . '_cal_img'];
        $from->img = FabrikHelperHTML::image('calendar', 'form', @$this->tmpl, $imageOpts);

        $displayData->from = $from;
        $displayData->jCalFrom = $this->calendar($rawValues[0], $v . '[0]', $this->getFilterHtmlId(0), $format,
            $this->filterCalendarOpts(), 0);
        ...
        $imageOpts = $displayData->j3 ? ['alt' => 'calendar'] : ['alt' => 'calendar', 'class' => 'calendarbutton', 'id' => $to->id . '_cal_img'];
        $to->img = FabrikHelperHTML::image('calendar', 'form', @$this->tmpl, $imageOpts);

        $displayData->to = $to;
        $displayData->filterType = $this->getFilterType();
        $displayData->jCalTo = $this->calendar($rawValues[1], $v . '[1]', $this->getFilterHtmlId(1), $format,
            $this->filterCalendarOpts(), 0);
    ...
    public function prepareCSVData(&$data, $key, $isRaw = false)
    {
        ...
        $params = $this->getParams();
        $csvFormat = $params->get('jdate_csv_offset_tz', '0');

        // 0 is "format format", 1 is "list format"
        if ($csvFormat === '0' || $csvFormat === '3')
        {
            $paramName = $csvFormat === '0' ? 'jdate_form_format' : 'jdate_table_format';
     ...
     */
    protected function getAvgQuery(&$listModel, $labels = [])
    {
     ...
     * @return string
     */
    protected function getSumQuery(&$listModel, $labels = [])
    {
        $label = count($labels) == 0 ? "'calc' AS label" : 'CONCAT(' . implode(', " & " , ', $labels) . ')  AS label';
        $table = $listModel->getTable();
        $db = $listModel->getDb();
    ...
    protected function strftimeTFormatToMySQL(&$format)
    {
         ...
         */
        $search = [
            '%e', '%j', '%u', '%V', '%W', '%h', '%B', '%C', '%g', '%G', '%M', '%P', '%r', '%R', '%T', '%X', '%z', '%Z', '%D', '%F', '%s',
            '%x', '%A'
        ];

        $replace = [
            '^@e', '^@j', '^@w', '^@U', '^@U', '^@b', '^@M', '', '^@y', '^@Y', '^@i', '^@p', '^@I:^@i:^@S ^@p', '^@H:^@i',
     ...
     */
    public function getDefaultProperties($properties = [])
    {
    ...
    public function filterJS($normal, $container)
    {
        ...
        $opts->type = $type;
        $opts->ids = $type == 'field' ? [$id] : [$id, $id2];
        $opts->buttons = $type == 'field' ? [$id . '_cal_img'] : [$id . '_cal_img', $id2 . '_cal_img'];
    ...
    protected function filterCalendarOpts()
    {
        $params = $this->getParams();
        $class = $this->filterClass();
        $calOpts = ['class' => $class, 'maxlength' => '19', 'size' => 16];
    ...
    public function formJavascriptClass(&$srcs, $script = '', &$shim = [])
    {
        ...
        $deps = array_key_exists($key, $shim) ? $shim[$key]->deps : [];
        $params = $this->getParams();
     ...
     * @return string
     */
    public function getFrontDefaultValue($data = [])
    {
        $params = $this->getParams();
        $db = JFactory::getDbo();
        $alwaysToday = $params->get('jdate_alwaystoday', false);
...
class FabJDate extends JDate
{
    ...
    public function __construct($date = 'now', $tz = null)
    {
        ...
        {
            JDEBUG ? $app->enqueueMessage('date format unknown for ' . $orig . ' replacing with today\'s date',
                'notice') : '';
            $date = 'now';
    ...
    static public function strftimeFormatToDateFormat($format)
    {
        ...
        $replace = [
            'j', 'z', 'w', 'W', 'W', 'M', 'F', 'Y', 'y', 'Y', 'i', 'a', '"g:i:s a', 'H:i', 'H:i:s', 'H:i:s', 'O', 'O', 'm/d/y"', 'Y-m-d', 'U',
            'Y-m-d', 'l', 'Y', 'm', 'd', 'H', 's'
        ];
