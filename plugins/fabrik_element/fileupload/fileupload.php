...
JLog::addLogger(['text_file' => 'fabrik.element.fileupload.log.php'], $logLvl, ['com_fabrik.element.fileupload']);
...
class PlgFabrik_ElementFileupload extends PlgFabrik_Element
{
    ...
    public function ignoreOnUpdate($val)
    {
        ...
        else
        {
            if ($groupModel->isJoin())
            {
                ...
                $fileJoinData = FArrayHelper::getValue($_FILES['join']['name'], $joinId, []);
    ...
    public function elementJavascript($repeatCounter)
    {
        ...
        $value = $this->getValue([], $repeatCounter);
        ...
        $rawValues = count($value) == 0 ? [] : FArrayHelper::array_fill(0, count($value), 0);
        ...
        $opts->previewButton = Html::image($icon, 'form', @$this->tmpl,
            ['alt' => FText::_('PLG_ELEMENT_FILEUPLOAD_VIEW')]);
        $opts->resizeButton = Html::image($resize, 'form', @$this->tmpl,
            ['alt' => FText::_('PLG_ELEMENT_FILEUPLOAD_RESIZE')]);
        $opts->files = $oFiles;
    ...
    public function validate($data = [], $repeatCounter = 0)
    {
        ...
        {
            $name = $this->getFullName(true, false);
            $files = $input->files->get($name, [], 'raw');
        ...
        if (!$this->_fileUploadSizeOK($fileSize))
        {
            ...
            $ok = false;
            $size = $fileSize / 1024;
            $errors[] = JText::sprintf('PLG_ELEMENT_FILEUPLOAD_FILE_TOO_LARGE', $params->get('ul_max_file_size'),
                $size);
        }
    ...
    public function processUpload()
    {
        ...
        $formModel = $this->getFormModel();
        $name = $this->getFullName(true, false);
        $myFileDirs = $input->get($name, [], 'array');
    ...
    protected function filesToDelete()
    {
        $input = $this->app->input;
        $groupModel = $this->getGroup();
        $deletedFiles = $input->get('fabrik_fileupload_deletedfile', [], 'array');
        $gid = $groupModel->getId();

        return FArrayHelper::getValue($deletedFiles, $gid, []);
    }
    ...
    protected function filesToKeep(array $deletedImages)
    {
        ...
        if ($groupModel->isJoin())
        {
            ...
            $formGroupIds = FArrayHelper::getValue($formModel->formData, $pkName, [], 'array');
        ...
        if ($storage->exists($filePath))
        {
            switch ($params->get('ul_file_increment', 0))
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    JLog::add('Ind upload Delete file: ' . $filePath . '; user = ' . $this->user->get('id'),
                        JLog::WARNING, 'com_fabrik.element.fileupload');
    ...
    public function getStorage()
    {
        ...
        {
            $params = $this->getParams();
            $storageType = JFilterInput::getInstance()->clean($params->get('fileupload_storage_type',
    ...
    public function render($data, $repeatCounter = 0)
    {
        ...
        if ($isAjax)
        {
            ...
            elseif (is_object($value) && isset($value->file))
            {
                $value = $value->file;
            }
        }
            ...
            foreach ($values as $v)
            {
                ...
                if ($render->output != '')
                {
                    if ($this->isEditable())
                    {
                        ...
                        {
                            $render->output = '<span class="fabrikUploadDelete" data-role="delete_span">' . $this->deleteButton($v,
        ...
        if (($params->get('fu_show_image') == '0' && !$this->isAjax()) && $this->isEditable())
        {
            foreach ($values as $v)
            {
                ...
                if ($render->output != '')
                {
                    ...
                    {
                        $render->output = '<span class="fabrikUploadDelete" id="' . $id . '_delete_span">' . $this->deleteButton($v,
                                $repeatCounter) . $render->output . '</span>';
                    }
        ...
        if (
            $params->get('fileupload_storage_type',
                'filesystemstorage') == 'filesystemstorage' && $params->get('upload_allow_folderselect') == '1'
        )
        {
    ...
    protected function plupload($str, $repeatCounter, $values)
    {
        ...
        Html::jLayoutJs('fabrik-progress-bar', 'fabrik-progress-bar', (object)['context' => '', 'animated' => true]);
        Html::jLayoutJs('fabrik-progress-bar-success', 'fabrik-progress-bar',
            (object)['context' => 'success', 'value' => 100]);
        Html::jLayoutJs('fabrik-icon-delete', 'fabrik-icon', (object)['icon' => 'icon-delete']);
    ...
    public function onAjax_deleteFile()
    {
        ...
        $join->load(['element_id' => $input->getInt('element_id')]);
        $this->setId($input->getInt('element_id'));
        $this->getElement();
        ...
        if ($join->table_join != '')
        {
            ...
            JLog::add('Delete join image entry: ' . $db->getQuery() . '; user = ' . $this->user->get('id'),
                JLog::WARNING, 'com_fabrik.element.fileupload');
            $db->execute();
        }
     ...
     * @return  string  HTML
     */
    public function buildCarousel($id = 'carousel', $data = [], $thisRow = null, $nav = true)
    {
        $rendered = '';
    ...
    private function renameFile($filename, $repeatCounter)
    {
        ...
        if (!empty($php))
        {
            ...
            FabrikWorker::logEval($filename,
                'Eval exception : ' . $this->getElement()->name . '::renameFile() : ' . $filename . ' : %s');
        }
