...
class PlgFabrik_ElementLockrow extends PlgFabrik_Element
{
        ...
        if (!empty($value))
        {
            ...
            if ($allowReedit === '0' || $allowReedit === '2')
            {
                ...
                if ($value !== $origValue)
                {
                    if ($expireMinutes < 0)
                    {
                        $this->app->enqueueMessage(FText::sprintf('PLG_ELEMENT_LOCKROW_SUBMIT_WRONG_LOCK_EXPIPRED_MSG',
                            abs($expireMinutes)));
                    }
                    else
                    {
                        $this->app->enqueueMessage(FText::sprintf('PLG_ELEMENT_LOCKROW_SUBMIT_WRONG_LOCK_MSG',
                            abs($expireMinutes)));
                    }
        ...
        if (!empty($value))
        {
            ...
            if ((int)$thisUserId === (int)$lockingUserId)
            {
                if ($ttl === 0 || $timeNow < $ttlTime)
                {
                    ...
                    elseif ($params->get('lockrow_allow_user_reedit', '1') === '2')
                    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            $expireTime = $ttlTime = $timeNow;
                            $expireMinutes = round($expireTime / 60);
                            $app->enqueueMessage(FText::sprintf('PLG_ELEMENT_LOCKROW_OWN_LOCKED_MSG',
                                abs($expireMinutes)));
                        }
    ...
    public function elementListJavascript()
    {
        ...
        foreach ($gKeys as $gKey)
        {
            ...
            {
                $rowLocks[$rowkey->__pk_val] = isset($rowkey->$elNameRaw) ? $this->showLocked($rowkey->$elNameRaw,
                    $userId) : false;
                $canUnlocks[$rowkey->__pk_val] = isset($rowkey->$elNameRaw) ? $this->canUnlock($rowkey->$elNameRaw,
                    $userId) : false;
