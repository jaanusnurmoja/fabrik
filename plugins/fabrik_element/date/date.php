...
class PlgFabrik_ElementDate extends PlgFabrik_ElementList
{
    ...
    public function render($data, $repeatCounter = 0)
    {
        ...
        else
        {
            ...
            $time = ((bool)$element->hidden || $params->get('date_showtime', 0)) ? $localDate->format($timeFormat,
                true) : '';
    ...
    protected function timeButton($timeElName, $time, &$str)
    {
        ...
        $str[] = '<input type="text" class="' . $class . '" ' . $readOnly . ' size="' . $timeLength . '" value="' . $time . '" name="'
            . $timeElName . '" />';
        $opts = ['alt' => FText::_('PLG_ELEMENT_DATE_TIME'), 'class' => 'timeButton'];
    ...
    public function calendar($value, $name, $id, $format = '%Y-%m-%d', $attribs = null, $repeatCounter = 0)
    {
        ...
        $opts = $j3 ? ['alt' => 'calendar'] : ['alt' => 'calendar', 'class' => 'calendarbutton', 'id' => $id . '_cal_img'];
    ...
    public function elementJavascript($repeatCounter)
    {
        ...
        $opts->showSeconds = $opts->whichTimePicker === 'clock' ? false : $params->get('date_show_seconds',
                '1') === '1';
    ...
    private function getTimeFormat()
    {
        ...
        if ($params->get('date_showtime', '0'))
        {
            ...
            {
                if (
                    $params->get('date_which_time_picker', 'nunbers') !== 'clock'
            ...
            {
                if (
                    $params->get('date_which_time_picker', 'wicked') !== 'clock'
                    && $params->get('date_show_seconds', '1')
                )
                {
                    $timeFormat = 'g:i:s A';
        ...
        elseif ((bool)$this->getElement()->hidden)
        {
            $timeFormat = 'H:i:s';
        }
