...
class PlgFabrik_ElementCalc extends PlgFabrik_Element
{
    ...
    private function _getV($data, $repeatCounter)
    {
        ...
        if (!$params->get('calc_on_save_only', true) || $task == 'form.process' || $task == 'process')
        {
            ...
            FabrikWorker::logEval($res,
                'Eval exception : ' . $this->getElement()->name . ' (id ' . $this->getId() . ')::_getV() : ' . $default . ' : %s');
    ...
    public function preFormatFormJoins($data, $row)
    {
        ...
        else
        {
            ...
            FabrikWorker::logEval($res,
                'Eval exception : ' . $element->name . ' (id ' . $this->getId() . ')::preFormatFormJoins() : ' . $cal . ' : %s');
    ...
    public function render($data, $repeatCounter = 0)
    {
        ...
        if (in_array($this->app->input->get('format', 'html'), ['html', 'partial']))
        {
            $opts = ['alt' => FText::_('PLG_ELEMENT_CALC_LOADING'), 'style' => 'display:none;padding-left:10px;', 'class' => 'loader'];
    ...
    protected function getSumQuery(&$listModel, $labels = [])
    {
        ...
        {
            $db = $listModel->getDb();
            $label = count($labels) == 0 ? "'calc' AS label" : 'CONCAT(' . implode(', " & " , ',
                    $labels) . ')  AS label';
     ...
     * @return string
     */
    protected function getMedianQuery(&$listModel, $labels = [])
    {
        $fields = $listModel->getDBFields($this->getTableName(), 'Field');
        $name = $this->getElement()->name;
        $field = FArrayHelper::getValue($fields, $name, false);
